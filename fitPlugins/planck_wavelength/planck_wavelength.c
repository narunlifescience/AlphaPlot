...
int function_f(const gsl_vector *params, void *void_data, gsl_vector *f) {
  ...
  for (i = 0; i < d->n; i++)
    gsl_vector_set(
        f, i, (a / (pow(d->X[i], 5) * (exp(b / d->X[i]) - 1.0)) - d->Y[i]) /
                  d->sigma[i]);
  return GSL_SUCCESS;
}
...
int function_df(const gsl_vector *params, void *void_data, gsl_matrix *J) {
  ...
  for (i = 0; i < d->n; i++) {
    ...
    gsl_matrix_set(J, i, 1,
                   a / (pow(x, 3) * pow(exp(b / x) - 1, 2) * exp(b / x) * b));
  }
  return GSL_SUCCESS;
}

int function_fdf(const gsl_vector *params, void *void_data, gsl_vector *f,
                 gsl_matrix *J) {
...
double function_d(const gsl_vector *params, void *void_data) {
  ...
  function_f(params, void_data, f);
  for (i = 0; i < d->n; i++) result += pow(gsl_vector_get(f, i), 2);
