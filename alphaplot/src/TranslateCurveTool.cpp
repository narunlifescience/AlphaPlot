...
TranslateCurveTool::TranslateCurveTool(Graph *graph, ApplicationWindow *app,
  ...
  if (status_target)
    connect(this, SIGNAL(statusText(const QString &)), status_target,
            status_slot);
  // d_graph->plotWidget()->canvas()->setCursor(QCursor(QPixmap(vizor_xpm), -1,
  // -1));
...
void TranslateCurveTool::selectCurvePoint(QwtPlotCurve *curve,
  ...
  connect((ScreenPickerTool *)d_sub_tool,
          SIGNAL(selected(const QwtDoublePoint &)), this,
          SLOT(selectDestination(const QwtDoublePoint &)));
  emit statusText(
      tr("Curve selected! Move cursor and click to choose a point and "
...
void TranslateCurveTool::selectDestination(const QwtDoublePoint &point) {
  ...
  if (((PlotCurve *)d_selected_curve)->type() == Graph::Function) {
    ...
    if (tab->columnType(col) != AlphaPlot::Numeric) {
      QMessageBox::warning(
          d_graph, tr("Warning"),
    ...
    for (int i = row_start; i < row_end; i++) {
      if (!tab->column(col)->isInvalid(i))
        tab->column(col)
            ->setValueAt(i, (d_dir == Horizontal ? d_selected_curve->x(i)
