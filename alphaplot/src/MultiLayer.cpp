...
void LayerButton::mousePressEvent(QMouseEvent *event) {
  if (!isChecked()) emit clicked(this);
  if (event->button() == Qt::RightButton) emit showContextMenu();
}
...
Graph *MultiLayer::layer(int num) { return (Graph *)graphsList.at(num - 1); }
...
void MultiLayer::findBestLayout(int &rows, int &cols) {
  ...
  if (NumGraph % 2 == 0)  // NumGraph is an even number
  {
    ...
    if (NumGraph < 8) rows = NumGraph / 4 + 1;
    if (NumGraph >= 8) rows = NumGraph / 4;
...
void MultiLayer::wheelEvent(QWheelEvent *e) {
  ...
  if (resize && (resize && (e->modifiers() == Qt::AltModifier ||
...
QString MultiLayer::saveToString(const QString &geometry) {
  ...
  s += "Margins\t" + QString::number(left_margin) + "\t" +
       QString::number(right_margin) + "\t" + QString::number(top_margin) +
       "\t" + QString::number(bottom_margin) + "\n";
  s += "Spacing\t" + QString::number(rowsSpace) + "\t" +
       QString::number(colsSpace) + "\n";
  s += "LayerCanvasSize\t" + QString::number(l_canvas_width) + "\t" +
       QString::number(l_canvas_height) + "\n";
  s += "Alignement\t" + QString::number(hor_align) + "\t" +
       QString::number(vert_align) + "\n";
...
void MultiLayer::setMargins(int lm, int rm, int tm, int bm) {
  if (left_margin != lm) left_margin = lm;
  if (right_margin != rm) right_margin = rm;
  if (top_margin != tm) top_margin = tm;
  if (bottom_margin != bm) bottom_margin = bm;
}

void MultiLayer::setSpacing(int rgap, int cgap) {
  if (rowsSpace != rgap) rowsSpace = rgap;
  if (colsSpace != cgap) colsSpace = cgap;
}

void MultiLayer::setLayerCanvasSize(int w, int h) {
  if (l_canvas_width != w) l_canvas_width = w;
  if (l_canvas_height != h) l_canvas_height = h;
}

void MultiLayer::setAlignement(int ha, int va) {
  if (hor_align != ha) hor_align = ha;

  if (vert_align != va) vert_align = va;
}
