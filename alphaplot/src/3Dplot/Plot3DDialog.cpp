...
void Plot3DDialog::initGeneralPage() {
  ...
  connect(boxResolution, SIGNAL(valueChanged(int)), this,
          SIGNAL(updateResolution(int)));
  connect(boxDistance, SIGNAL(valueChanged(int)), this,
          SIGNAL(adjustLabels(int)));
  connect(boxMeshLineWidth, SIGNAL(valueChanged(int)), this,
          SIGNAL(updateMeshLineWidth(int)));
  connect(boxOrthogonal, SIGNAL(toggled(bool)), this, SIGNAL(setOrtho(bool)));
  connect(boxLegend, SIGNAL(toggled(bool)), this,
          SIGNAL(showColorLegend(bool)));
  connect(boxZoom, SIGNAL(valueChanged(double)), this,
          SLOT(changeZoom(double)));
  connect(boxXScale, SIGNAL(valueChanged(double)), this,
          SLOT(changeZoom(double)));
  connect(boxYScale, SIGNAL(valueChanged(double)), this,
          SLOT(changeZoom(double)));
  connect(boxZScale, SIGNAL(valueChanged(double)), this,
          SLOT(changeZoom(double)));
...
void Plot3DDialog::disableGridOptions() { btnGrid->setDisabled(true); }
...
void Plot3DDialog::disableLegend() { boxLegend->setDisabled(true); }

void Plot3DDialog::setLabelsDistance(int dist) { boxDistance->setValue(dist); }

void Plot3DDialog::pickDataColorMap() {
  QString fn =
      QFileDialog::getOpenFileName(this, QString(), d_plot->colorMap(),
                                   tr("Colormap files") + " (*.map *.MAP)");
  if (!fn.isEmpty()) emit setDataColorMap(fn);
}

QColor Plot3DDialog::pickFromColor() {
  QColor c = QColorDialog::getColor(fromColor, this);
  if (!c.isValid()) return fromColor;
...
QColor Plot3DDialog::pickLabelColor() {
  ...
  if (!c.isValid()) return labelColor;
...
QColor Plot3DDialog::pickTitleColor() {
  ...
  if (!c.isValid()) return titleColor;
...
void Plot3DDialog::setTitle(const QString& title) { boxTitle->setText(title); }

void Plot3DDialog::setTitleFont(const QFont& font) { titleFont = font; }

QColor Plot3DDialog::pickMeshColor() {
  QColor c = QColorDialog::getColor(meshColor, this);
  if (!c.isValid()) return meshColor;
...
void Plot3DDialog::setColors(const QColor& title, const QColor& mesh,
                             const QColor& axes, const QColor& num,
                             const QColor& label, const QColor& bg,
                             const QColor& grid) {
  titleColor = title;
  meshColor = mesh;
  axesColor = axes;
...
void Plot3DDialog::showLegend(bool show) { boxLegend->setChecked(show); }

void Plot3DDialog::changeZoom(double) {
  if (generalDialog->currentWidget() != (QWidget*)general) return;
...
void Plot3DDialog::changeTransparency(int val) {
  if (generalDialog->currentWidget() != (QWidget*)colors) return;
...
QStringList Plot3DDialog::scaleOptions(int axis, double start, double end,
  ...
  for (int i = 0; i < 5; i++) scales[5 * axis + i] = l[i];
  return l;
}
...
void Plot3DDialog::setAxesFonts(const QFont& xf, const QFont& yf,
                                const QFont& zf) {
  xAxisFont = xf;
  yAxisFont = yf;
  zAxisFont = zf;
...
void Plot3DDialog::setResolution(int r) { boxResolution->setValue(r); }

void Plot3DDialog::setZoom(double zoom) { boxZoom->setValue(zoom * 100); }
...
void Plot3DDialog::showTitleTab() { generalDialog->setCurrentIndex(2); }

void Plot3DDialog::showAxisTab() { generalDialog->setCurrentIndex(1); }
