...
void Graph3D::addData(Table* table, int xcol, int ycol) {
  ...
  for (i = 0; i < r; i++) {
    if (!table->column(xcol)->isInvalid(i) &&
        !table->column(ycol)->isInvalid(i))
      xmesh++;
  }
...
void Graph3D::addMatrixData(Matrix* m) {
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) data_matrix[i][j] = m->cell(i, j);
  }

  sp->makeCurrent();
  sp->loadFromData(data_matrix, rows, cols, m->xStart(), m->xEnd(), m->yStart(),
                   m->yEnd());
  ...
  if (d_autoscale || first_time) findBestLayout();
  update();
}
...
void Graph3D::changeDataColumn(Table* table, const QString& colName) {
  ...
  plotAssociation =
      table->colName(xCol) + "(X)," + table->colName(yCol) + "(Y),";
...
void Graph3D::addData(Table* table, int xCol, int yCol, int zCol, double xl,
  ...
  for (i = 0; i < r; i++) {
    if (!table->column(xCol)->isInvalid(i) &&
      ...
      xv = table->cell(i, xCol);
      yv = table->cell(i, yCol);
      if (xv >= xl && xv <= xr && yv >= yl && yv <= yr) columns++;
    }
  }
...
void Graph3D::updateMatrixData(Matrix* m) {
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) data[i][j] = m->cell(i, j);
  }

  sp->loadFromData(data, rows, cols, m->xStart(), m->xEnd(), m->yStart(),
                   m->yEnd());
...
void Graph3D::setTickLengths(const QStringList& lst) {
  ...
  if (int(lst.count()) > 6) tick_length.removeAll(tick_length.first());
...
void Graph3D::updateTickLength(int axis, double majorLength,
                               double minorLength) {
  double majorl, minorl;
  switch (axis) {
    case 0:
...
void Graph3D::updateScale(int axis, const QStringList& options) {
  ...
  if (*min != options[0].toDouble() || *max != options[1].toDouble()) {
    ...
    *max = options[1].toDouble();
    if (func) {
      ...
      func->setMaxZ(zMax);
      func->create();
      sp->createCoordinateSystem(Triple(xMin, yMin, zMin),
  ...
  if (majors != newMaj)
    for (int i = 0; i < 4; i++) targetAxes[i]->setMajors(newMaj);

  newMin = options[3].toInt();
  if (minors != newMin)
    for (int i = 0; i < 4; i++) targetAxes[i]->setMinors(newMin);
...
void Graph3D::updateScales(double xl, double xr, double yl, double yr,
  ...
  for (i = 0; i < r; i++) {
    if (!worksheet->column(xcol)->isInvalid(i) &&
        !worksheet->column(ycol)->isInvalid(i)) {
...
void Graph3D::setColors(const QColor& meshColor, const QColor& axesColor,
                        const QColor& numColor, const QColor& labelColor,
                        const QColor& bgColor, const QColor& gridColor) {
...
void Graph3D::updateColors(const QColor& meshColor, const QColor& axesColor,
                           const QColor& numColor, const QColor& labelColor,
                           const QColor& bgColor, const QColor& gridColor) {
  if (meshCol != meshColor) {
    sp->setMeshColor(Qt2GL(meshColor));
...
void Graph3D::setFramed() {
  if (sp->coordinates()->style() == FRAME) return;
...
void Graph3D::setBoxed() {
  if (sp->coordinates()->style() == BOX) return;
...
void Graph3D::setNoAxes() {
  if (sp->coordinates()->style() == NOCOORD) return;
...
void Graph3D::setFilledMesh() {
  if (sp->plotStyle() == FILLEDMESH) return;
...
void Graph3D::setHiddenLineGrid() {
  if (sp->plotStyle() == HIDDENLINE) return;
...
void Graph3D::setLineGrid() {
  if (sp->plotStyle() == WIREFRAME) return;
...
void Graph3D::setCrossMesh() {
  if (!sp || pointStyle == HairCross) return;
...
void Graph3D::setBarsPlot() {
  if (pointStyle == VerticalBars) return;
...
void Graph3D::setFloorData() {
  if (sp->floorStyle() == FLOORDATA) return;
...
void Graph3D::setFloorIsolines() {
  if (sp->floorStyle() == FLOORISO) return;
...
void Graph3D::setEmptyFloor() {
  if (sp->floorStyle() == NOFLOOR) return;
...
void Graph3D::setMeshLineWidth(int lw) {
  if ((int)sp->meshLineWidth() == lw) return;
...
void Graph3D::print() {
  ...
  if (dialog.exec()) {
    if (IO::save(sp, "alphaplot.png", "PNG")) {
      QPixmap p;
      p.load("alphaplot.png", "PNG", Qt::ColorOnly);
...
void Graph3D::setPointOptions(double size, bool s) {
  if (pointSize == size && smooth == s) return;
...
void Graph3D::updatePoints(double size, bool sm) {
  if (pointStyle == Dots && pointSize == size && smooth == sm) return;
...
void Graph3D::updateCones(double rad, int quality) {
  if (pointStyle == Cones && conesRad == rad && conesQuality == quality) return;
...
void Graph3D::updateCross(double rad, double linewidth, bool smooth,
                          bool boxed) {
...
void Graph3D::setStyle(const QStringList& st) {
  ...
  if (st[3] == "filledmesh")
    ...
    if (st[6] == "1") crossHairSmooth = true;

    crossHairBoxed = false;
    if (st[7] == "1") crossHairBoxed = true;
...
void Graph3D::setZoom(double val) { sp->setZoom(val); }

void Graph3D::updateZoom(double val) {
  if (sp->zoom() == val) return;
...
void Graph3D::updateTitle(const QString& s, const QColor& color,
                          const QFont& font) {
  if (title != s) {
    title = s;
    sp->setTitle(title);
...
void Graph3D::setOptions(const QStringList& lst) {
  legendOn = false;
  if (lst[1].toInt() == 1) legendOn = true;
  sp->showColorLegend(legendOn);
...
void Graph3D::setDataColors(const QColor& cMin, const QColor& cMax) {
  if (cMin == fromColor && cMax == toColor) return;
...
void Graph3D::animate(bool on) {
  if (on)
    d_timer->start(
        animation_redraw_wait);  // Wait this many msecs before redraw
  else
    d_timer->stop();
}
...
void Graph3D::copy(Graph3D* g) {
  ...
  setColors(g->meshColor(), g->axesColor(), g->numColor(), g->labelColor(),
            g->bgColor(), g->gridColor());
