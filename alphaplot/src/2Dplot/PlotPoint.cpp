...
PlotPoint::PlotPoint(QCustomPlot *parentPlot, int halfSize)
  ...
  setSelectable(
      true);  // plot moves only selectable points, see Plot::mouseMoveEvent
...
QPointF PlotPoint::pos() const { return mCenterTracer->position->coords(); }

const QColor &PlotPoint::color() const { return brush().color(); }
