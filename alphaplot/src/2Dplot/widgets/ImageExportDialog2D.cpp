...
void ImageExportDialog2D::initAdvancedOptions() {
  ...
  d_vector_width = new QSpinBox();
  d_vector_width->setRange(0, 5000);
  d_vector_width->setValue(0);
  vector_layout->addWidget(new QLabel(tr("Image Width")), 1, 0);
  vector_layout->addWidget(d_vector_width, 1, 1);
  d_vector_height = new QSpinBox();
  d_vector_height->setRange(0, 5000);
  d_vector_height->setValue(0);
  vector_layout->addWidget(new QLabel(tr("Image Width")), 2, 0);
  ...
  d_raster_resolution = new QSpinBox();
  d_raster_resolution->setRange(20, 1500);
  d_raster_resolution->setValue(300);
  raster_layout->addWidget(d_raster_resolution, 1, 1);
  d_raster_width = new QSpinBox();
  d_raster_width->setRange(0, 5000);
  d_raster_width->setValue(0);
  raster_layout->addWidget(new QLabel(tr("Image Width")), 2, 0);
  raster_layout->addWidget(d_raster_width, 2, 1);
  d_raster_height = new QSpinBox();
  d_raster_height->setRange(0, 5000);
  d_raster_height->setValue(0);
  raster_layout->addWidget(new QLabel(tr("Image Height")), 3, 0);
  raster_layout->addWidget(d_raster_height, 3, 1);
  d_raster_scale = new QDoubleSpinBox();
  d_raster_scale->setValue(1.0);
  d_raster_scale->setRange(1.0, 10.0);
  raster_layout->addWidget(new QLabel(tr("Image Scale")), 4, 0);
  raster_layout->addWidget(d_raster_scale, 4, 1);
  d_raster_quality = new QSpinBox();
  d_raster_quality->setValue(100);
  d_raster_quality->setRange(1, 100);
  raster_layout->addWidget(new QLabel(tr("Image quality")), 5, 0);
  raster_layout->addWidget(d_raster_quality, 5, 1);
}

void ImageExportDialog2D::updateAdvancedOptions(const QString &filter) {
