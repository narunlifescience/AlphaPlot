...
void ColorMapEditor::updateColorMap() {
  ...
  for (int i = 1; i < rows - 1; i++) {
    ...
    double val =
        (table->item(i, 0)->text().toDouble() - min_val) / range.width();
...
void ColorMapEditor::insertLevel() {
  ...
  double val = 0.5 * (table->item(row, 0)->text().toDouble() +
                      table->item(row - 1, 0)->text().toDouble());
...
void ColorMapEditor::showColorDialog(int row, int col) {
  ...
  if (!color.isValid() || color == c) return;
...
void ColorMapEditor::validateLevel(int row, int col) {
  ...
  if (row == 0 || row == table->rowCount() - 1) {
    QMessageBox::critical(this, tr("Input Error"),
                          tr("Sorry, you cannot edit this value!"));
    table->blockSignals(true);
    if (!row)
  ...
  QString s =
      table->item(row, 0)->text().remove("-").remove(".").remove(",").remove(
          "+");
