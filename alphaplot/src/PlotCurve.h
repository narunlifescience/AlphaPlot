...
class DataCurve : public PlotCurve {
 public:
  ...
  QList<QVector<double> > convertData(const QList<Column *> &cols,
  ...
  /**
           ...
           *  - &lt;id of X column> "(X)," &lt;id of Y column> "(Y)" [ ","
   * &lt;id of error column> ("(xErr)" | "(yErr)") ]
           *  - &lt;id of Xstart column> "(X)," &lt;id of Ystart column>"(Y),"
   * &lt;id of Xend column> "(X)," &lt;id of Yend column> "(Y)"\n
           *    (denoting start and end coordinates for the #VectXYXY style)
           *  - &lt;id of Xstart column> "(X)," &lt;id of Ystart column> "(Y),"
   * &lt;id of angle column> "(A)," &lt;id of magnitude column> "(M)"\n
           *    (denoting start coordinates, angle in radians and length for the
   * #VectXYAM style)
           *
           * Column ids are of the form '&lt;name of table> "_" &lt;name of
   * column>'.
           */
 ...
 protected:
   ...
   */
  QString d_x_column;
  //! First row of #d_tabel to be plotted, provided all relevant columns have
  //! valid entries.
  int d_start_row;
  //! Last row of #d_tabel to be plotted, provided all relevant columns have
  //! valid entries.
  int d_end_row;
 ...
 private:
   ...
   *
   * Besides #d_start_row and #d_end_row, this takes into account rows which had
   * invalid entries in
   * one of the relevant columns the last time convertData() was called.
   * Usually, changes in
   * validity should cause the curve to be updated via updateData(); however,
   * subclasses of
   * DataCurve may depend on more columns than X and Y (in particular, this is
   * true of vector
   * curves); so DataCurve cannot (within the current architecture) compute this
   * mapping on demand
