...
QwtText QwtTextScaleDraw::label(double value) const {
  ...
  if (!scDiv.contains(value)) return QwtText();
...
TimeScaleDraw::TimeScaleDraw(const QTime& t, const QString& format)
    : t_origin(t), t_format(format) {}

QString TimeScaleDraw::origin() { return t_origin.toString("hh:mm:ss.zzz"); }
...
QString DateScaleDraw::origin() { return t_origin.toString(); }
...
DateTimeScaleDraw::DateTimeScaleDraw(const QDateTime& origin,
                                     const QString& format)
    : d_origin(origin), d_format(format) {}

QString DateTimeScaleDraw::origin() { return d_origin.toString(d_format); }

QwtText DateTimeScaleDraw::label(double value) const {
  QDate date = QDate::fromJulianDay(qRound(value));
  QTime time =
      QTime(12, 0, 0, 0).addMSecs(int((value - floor(value)) * 86400000.0));
...
QwtText QwtSupersciptsScaleDraw::label(double value) const {
  ...
  if (list[0].toDouble() == 0.0) return QString("0");
