...
#endif
{
  ...
  for (int i = 0; i < columns; i++) {
    Column *new_col = new Column(QString::number(i + 1), AlphaPlot::Numeric);
...
void Table::insertColumns(int before, QList<Column *> new_cols) {
  ...
  WAIT_CURSOR;
  beginMacro(
      QObject::tr("%1: insert %2 column(s)").arg(name()).arg(new_cols.size()));
  int pos = before;
...
void Table::removeColumn(Column *col) { removeColumns(columnIndex(col), 1); }
...
int Table::columnCount() const { return d_table_private->columnCount(); }

int Table::rowCount() const { return d_table_private->rowCount(); }
...
void Table::setColumnCount(int new_size) {
  ...
  if (old_size == new_size || new_size < 0) return;
...
void Table::clear() {
  ...
  int cols = columnCount();
  for (int i = 0; i < cols; i++) column(i)->clear();
  endMacro();
  RESET_CURSOR;
}
...
void Table::clearMasks() {
  ...
  int cols = columnCount();
  for (int i = 0; i < cols; i++) column(i)->clearMasks();
  endMacro();
  RESET_CURSOR;
}
#endif
    ...
    // selection will be expanded to the needed size from the current cell
    {
      ...
      if (last_col >= columnCount()) {
        ...
        for (int i = 0; i < last_col + 1 - columnCount(); i++) {
          Column *new_col = new Column(QString::number(i + 1), AlphaPlot::Text);
          new_col->setPlotDesignation(AlphaPlot::Y);
          cols << new_col;
        }
        appendColumns(cols);
...
void Table::sortTable() {
  ...
  for (int i = 0; i < columnCount(); i++) cols.append(column(i));
...
void Table::setSelectedColumnsAsX() { setSelectionAs(AlphaPlot::X); }

void Table::setSelectedColumnsAsY() { setSelectionAs(AlphaPlot::Y); }

void Table::setSelectedColumnsAsZ() { setSelectionAs(AlphaPlot::Z); }

void Table::setSelectedColumnsAsYError() { setSelectionAs(AlphaPlot::yErr); }

void Table::setSelectedColumnsAsXError() { setSelectionAs(AlphaPlot::xErr); }
  ...
  foreach (Column *col, cols) {
    if (col->dataType() == AlphaPlot::TypeDouble) {
      ...
      for (int row = 0; row < col->rowCount(); row++) {
        if (col->valueAt(row) > max) max = col->valueAt(row);
      }
    ...
    if (d_view->formulaModeActive())
      ...
      foreach (Interval<int> i, d_view->selectedRows().intervals())
        if (i.end() == col_ptr->rowCount() - 1)
          ...
          QStringList empties;
          for (int j = 0; j < i.size(); j++) empties << QString();
...
void Table::createActions() {
  ...
  action_unmask_selection = new QAction(
      QIcon(QPixmap(":/unmask.xpm")), tr("&Unmask", "unmask selection"), this);
...
#ifndef LEGACY_CODE_0_2_x
  action_clear_masks =
      new QAction(QIcon(QPixmap(":/unmask.xpm")), tr("Clear Masks"), this);
...
bool Table::export_to_TeX(QString fileName, TeXTableSettings &tex_settings) {
  ...
  if (d_view->selectedColumnCount() != 0) {
    ...
    for (int i = 0; i < columnCount(); i++) {
      if (d_view->isColumnSelected(i)) columns_list << column(i);
    }
    ...
    // Get all columns
    for (int i = 0; i < columnCount(); i++) columns_list << column(i);
...
int Table::colX(int col) {
  for (int i = col - 1; i >= 0; i--) {
    if (column(i)->plotDesignation() == AlphaPlot::X) return i;
  }
  int cols = columnCount();
...
int Table::colY(int col) {
  ...
  if (column(col)->plotDesignation() == AlphaPlot::xErr ||
    ...
    for (int i = col - 1; i >= 0; i--) {
      if (column(i)->plotDesignation() == AlphaPlot::Y) return i;
    }
    for (int i = col + 1; i < cols; i++) {
      if (column(i)->plotDesignation() == AlphaPlot::Y) return i;
    }
...
void Table::setPlotMenu(QMenu *menu) { d_plot_menu = menu; }
...
QString Table::text(int row, int col) {
  ...
  if (!col_ptr) return QString();
  if (col_ptr->isInvalid(row)) return QString();
...
void Table::handleModeChange(const AbstractColumn *col) {
  ...
  if (index != -1) d_table_private->updateHorizontalHeader(index, index);
}
...
void Table::handleRowsAboutToBeInserted(const AbstractColumn *col, int before,
  ...
  if (before <= col->rowCount() && new_size > rowCount()) setRowCount(new_size);
}

void Table::handleRowsInserted(const AbstractColumn *col, int before,
                               int count) {
  Q_UNUSED(count);
  int index = columnIndex(static_cast<const Column *>(col));
...
bool Table::load(XmlStreamReader *reader) {
  if (reader->isStartElement() && reader->name() == "table") {
    ...
    if (cols != columnCount())
      reader->raiseWarning(
          tr("columns attribute and number of read columns do not match"));
...
void Table::Private::insertColumns(int before, QList<Column *> cols) {
  ...
  if ((count < 1) || (before > d_column_count)) return;
...
void Table::Private::removeColumns(int first, int count) {
  if ((count < 1) || (first >= d_column_count)) return;
...
int Table::Private::numColsWithPD(AlphaPlot::PlotDesignation pd) {
  ...
  for (int i = 0; i < d_column_count; i++)
    if (d_columns.at(i)->plotDesignation() == pd) count++;
...
void Table::Private::updateVerticalHeader(int start_row) {
  ...
  for (; i < d_row_count; i++) d_vertical_header_data << i + 1;
...
QVariant Table::Private::headerData(int section, Qt::Orientation orientation,
  ...
  switch (orientation) {
    case Qt::Horizontal:
      if (section >= d_horizontal_header_data.size()) return QVariant();
      switch (role) {
        case Qt::DisplayRole:
        case Qt::ToolTipRole:
        case Qt::EditRole:
      ...
      break;
    case Qt::Vertical:
      if (section >= d_vertical_header_data.size()) return QVariant();
      switch (role) {
        case Qt::DisplayRole:
        case Qt::ToolTipRole:
