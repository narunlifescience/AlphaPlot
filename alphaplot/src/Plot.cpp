...
void Plot::drawItems(QPainter *painter, const QRect &rect,
  ...
  for (int i = 0; i < QwtPlot::axisCnt; i++) {
    ...
    bool min =
        (minorTicksType == ScaleDraw::In || minorTicksType == ScaleDraw::Both);
    bool maj =
        (majorTicksType == ScaleDraw::In || majorTicksType == ScaleDraw::Both);

    if (min || maj) drawInwardTicks(painter, rect, map[i], i, min, maj);
  }
}
...
void Plot::drawInwardTicks(QPainter *painter, const QRect &rect,
  ...
  switch (axis) {
    case QwtPlot::yLeft:
      x = x1;
      low = y1;
      if (axisEnabled(QwtPlot::xTop)) low += majTickLength;
      high = y2;
      if (axisEnabled(QwtPlot::xBottom)) high -= majTickLength;
      if (min) {
    ...
    case QwtPlot::xBottom:
      y = y2;
      low = x1;
      if (axisEnabled(QwtPlot::yLeft)) low += majTickLength;
      high = x2;
      if (axisEnabled(QwtPlot::yRight)) high -= majTickLength;
      if (min) {
        for (j = 0; j < minTicks; j++) {
...
int Plot::axesLinewidth() const {
  for (int axis = 0; axis < QwtPlot::axisCnt; axis++) {
    ...
    if (scale) return scale->penWidth();
  }
  return 0;
}

int Plot::minorTickLength() const { return minTickLength; }

int Plot::majorTickLength() const { return majTickLength; }

void Plot::setTickLength(int minLength, int majLength) {
  if (majTickLength == majLength && minTickLength == minLength) return;
...
int Plot::insertMarker(QwtPlotMarker *m) {
  marker_key++;
  if (!d_markers.contains(marker_key)) d_markers.insert(marker_key, m);
  m->setRenderHint(QwtPlotItem::RenderAntialiased,
                   ((Graph *)parent())->antialiasing());
...
int Plot::insertCurve(QwtPlotItem *c) {
  curve_key++;
  if (!d_curves.contains(curve_key)) d_curves.insert(curve_key, c);
...
void Plot::removeCurve(int index) {
  ...
  if (c->rtti() == QwtPlotItem::Rtti_PlotSpectrogram) {
    ...
    if (colorAxis) colorAxis->setColorBarEnabled(false);
  }
...
void Plot::setMajorTicksType(int axis, int type) {
  ScaleDraw *sd = (ScaleDraw *)axisScaleDraw(axis);
  if (sd) sd->setMajorTicksStyle((ScaleDraw::TicksStyle)type);
}
...
void Plot::setMinorTicksType(int axis, int type) {
  ScaleDraw *sd = (ScaleDraw *)axisScaleDraw(axis);
  if (sd) sd->setMinorTicksStyle((ScaleDraw::TicksStyle)type);
}
