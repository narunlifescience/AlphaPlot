...
void CurvesDialog::showPlotAssociations() {
  ...
  if (app) app->showPlotAssociations(curve);
}
...
void CurvesDialog::showFunctionDialog() {
  ...
  if (app) app->showFunctionDialog(d_graph, currentRow);
}
...
void CurvesDialog::showCurrentFolder(bool currentFolder) {
  ...
  if (currentFolder) {
    ...
    available->addItems(app->columnsList(AlphaPlot::Y));
}

void CurvesDialog::closeEvent(QCloseEvent *e) {
  ApplicationWindow *app = (ApplicationWindow *)this->parent();
  if (app) app->d_add_curves_dialog_size = this->size();
