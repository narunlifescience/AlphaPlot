...
bool FunctionDialog::acceptParametric() {
  ...
  QString xformula =
      boxXFunction->currentText().replace(QChar::ParagraphSeparator, "\n");
  QString yformula =
...
bool FunctionDialog::acceptPolar() {
  ...
  QString rformula =
      boxPolarRadius->currentText().replace(QChar::ParagraphSeparator, "\n");
  QString tformula =
  ...
  if (!graph)
    result = app->newFunctionPlot(type, formulas, boxPolarParameter->text(),
                                  ranges, boxPolarPoints->value());
  else {
    if (curveID >= 0)
      result = graph->modifyFunctionCurve(app, curveID, type, formulas,
      ...
      result = graph->addFunctionCurve(app, type, formulas,
                                       boxPolarParameter->text(), ranges,
                                       boxPolarPoints->value());
  }
  if (!result) boxPolarRadius->setFocus();
  return result;
}
