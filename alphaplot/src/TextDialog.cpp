...
TextDialog::TextDialog(TextType type, QWidget* parent, Qt::WFlags fl)
  ...
  if (textType == TextDialog::TextMarker) {  // TODO: Sometime background
                                             // features for axes lables should
                                             // be implemented
    ...
    connect(backgroundBtn, SIGNAL(clicked()), this,
   ...
   * caution: This code is still the old Qt3 code
     QLabel* rotate=new QLabel(tr( "Rotate (deg.)" ),GroupBox1,
   "TextLabel1_2",0);
     rotate->hide();
...
void TextDialog::setTextColor(QColor c) { colorBtn->setColor(c); }

void TextDialog::pickTextColor() {
  QColor c = QColorDialog::getColor(colorBtn->color(), this);
  if (!c.isValid() || c == colorBtn->color()) return;
...
void TextDialog::pickBackgroundColor() {
  ...
  if (!c.isValid() || c == backgroundBtn->color()) return;

  backgroundBtn->setColor(c);
}

void TextDialog::setFont(const QFont& fnt) { selectedFont = fnt; }
