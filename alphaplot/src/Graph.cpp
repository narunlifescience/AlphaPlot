...
Graph::Graph(QWidget *parent, QString name, Qt::WFlags f) : QWidget(parent, f) {
  ...
  connect(titlePicker, SIGNAL(doubleClicked()), this,
          SIGNAL(viewTitleDialog()));
  ...
  connect(scalePicker, SIGNAL(axisTitleRightClicked(int)), this,
          SLOT(showAxisTitleMenu(int)));
  connect(scalePicker, SIGNAL(axisRightClicked(int)), this,
          SLOT(showAxisContextMenu(int)));
  connect(scalePicker, SIGNAL(xAxisTitleDblClicked()), this,
          SIGNAL(xAxisTitleDblClicked()));
  connect(scalePicker, SIGNAL(yAxisTitleDblClicked()), this,
          SIGNAL(yAxisTitleDblClicked()));
  connect(scalePicker, SIGNAL(rightAxisTitleDblClicked()), this,
          SIGNAL(rightAxisTitleDblClicked()));
  connect(scalePicker, SIGNAL(topAxisTitleDblClicked()), this,
          SIGNAL(topAxisTitleDblClicked()));
...
void Graph::deselectMarker() {
  selectedMarker = -1;
  if (d_markers_selector) delete d_markers_selector;
}

long Graph::selectedMarkerKey() { return selectedMarker; }
...
void Graph::setSelectedMarker(long mrk, bool add) {
  ...
  if (add) {
    if (d_markers_selector) {
      ...
      if (d_texts.contains(mrk))
        d_markers_selector =
            new SelectionMoveResizer((Legend *)d_plot->marker(mrk));
      else if (d_lines.contains(mrk))
        d_markers_selector =
            new SelectionMoveResizer((ArrowMarker *)d_plot->marker(mrk));
      else if (d_images.contains(mrk))
        d_markers_selector =
            new SelectionMoveResizer((ImageMarker *)d_plot->marker(mrk));
      else
        return;
    ...
    if (d_texts.contains(mrk)) {
      if (d_markers_selector) {
        if (d_markers_selector->contains((Legend *)d_plot->marker(mrk))) return;
...
void Graph::enableAxes(QVector<bool> axesOn) {
  for (int i = 0; i < QwtPlot::axisCnt; i++) d_plot->enableAxis(i, axesOn[i]);
...
QVector<bool> Graph::enabledAxes() {
  ...
  for (int i = 0; i < QwtPlot::axisCnt; i++) axesOn[i] = d_plot->axisEnabled(i);
  return axesOn;
}
...
void Graph::setAxesBaseline(const QList<int> &lst) {
  for (int i = 0; i < QwtPlot::axisCnt; i++) {
    QwtScaleWidget *scale = (QwtScaleWidget *)d_plot->axisWidget(i);
    if (scale) scale->setMargin(lst[i]);
  }
}
...
QList<int> Graph::axesType() { return axisType; }
...
void Graph::setLabelsNumericFormat(const QStringList &l) {
  for (int axis = 0; axis < 4; axis++) setLabelsNumericFormat(axis, l);
}
...
QString Graph::saveTicksType() {
  ...
  int i;
  for (i = 0; i < 4; i++) s += QString::number(ticksTypeList[i]) + "\t";
  s += "\n";
...
void Graph::setMajorTicksType(const QList<int> &lst) {
  if (d_plot->getMajorTicksType() == lst) return;
...
void Graph::setMinorTicksType(const QList<int> &lst) {
  if (d_plot->getMinorTicksType() == lst) return;
...
void Graph::setLabelsDayFormat(int axis, int format) {
  ...
  WeekDayScaleDraw *sd = new WeekDayScaleDraw(
      *static_cast<const ScaleDraw *>(d_plot->axisScaleDraw(axis)),
...
void Graph::setLabelsMonthFormat(int axis, int format) {
  ...
  MonthScaleDraw *sd = new MonthScaleDraw(
      *static_cast<const ScaleDraw *>(d_plot->axisScaleDraw(axis)),
...
void Graph::setLabelsTextFormat(int axis, const Column *column, int startRow,
                                int endRow) {
  ...
  for (int row = startRow; row <= endRow; row++)
    if (!column->isInvalid(row)) list.insert(row + 1, column->textAt(row));
...
void Graph::setLabelsTextFormat(int axis, Table *table,
                                const QString &columnName) {
  Column *col = 0;
  if (!table || !(col = table->column(columnName))) {
    QMessageBox::critical(this, tr("Internal Error"),
                          tr("<html>Failed to set axis labels on Graph %1. "
                             "Maybe you're trying to open a corrupted"
                             " project file; or there's some problem within "
                             "AlphaPlot. Please report this"
                             " as a bug (together with detailed instructions "
                             "how to reproduce this message or"
                             " the corrupted file).<p>"
                             "<a "
                             "href=\"https://sourceforge.net/tracker/"
                             "?group_id=199120&atid=968214>\">"
                             "bug tracker: "
                             "https://sourceforge.net/tracker/"
                             "?group_id=199120&atid=968214</a></html>")
                              .arg(objectName()));
    return;
  }
...
void Graph::setLabelsColHeaderFormat(int axis, Table *table) {
  ...
  for (int col = 0; col < table->columnCount(); col++)
    if (table->colPlotDesignation(col) == AlphaPlot::Y)
  ...
  switch (type) {
    case Time: {
      TimeScaleDraw *sd = new TimeScaleDraw(
          *static_cast<const ScaleDraw *>(d_plot->axisScaleDraw(axis)),
          list[0].isEmpty() ? QTime(12, 0, 0, 0) : QTime::fromString(list[0]),
          list[1]);
      sd->enableComponent(QwtAbstractScaleDraw::Labels, true);
    ...
    case Date: {
      DateScaleDraw *sd = new DateScaleDraw(
          *static_cast<const ScaleDraw *>(d_plot->axisScaleDraw(axis)),
...
QStringList Graph::axesColors() {
  ...
  QPalette pal;
  int i;
  for (i = 0; i < 4; i++) colors << QColor(Qt::black).name();
...
void Graph::updateSecondaryAxis(int axis) {
  for (int i = 0; i < n_curves; i++) {
    ...
    if (it->rtti() == QwtPlotItem::Rtti_PlotSpectrogram) {
      ...
      if (sp->colorScaleAxis() == axis) return;
    }
  ...
  if (axis == QwtPlot::yRight) a = QwtPlot::yLeft;

  if (!d_plot->axisEnabled(a)) return;
...
void Graph::setAutoScale() {
  for (int i = 0; i < QwtPlot::axisCnt; i++) d_plot->setAxisAutoScale(i);
...
QStringList Graph::analysableCurvesList() {
  ...
  for (int i = 0; i < (int)keys.count(); i++) {
    ...
    if (c && c_type[i] != ErrorBars) cList << c->title().text();
  }
  return cList;
}
...
void Graph::exportPainter(QPaintDevice &paintDevice, bool keepAspect,
                          QRect rect) {
...
void Graph::exportPainter(QPainter &painter, bool keepAspect, QRect rect,
                          QSize size) {
...
bool Graph::markerSelected() { return (selectedMarker >= 0); }
    ...
    if (d_markers_selector) {
      if (d_texts.contains(selectedMarker))
        ...
        d_markers_selector->removeAll(
            (ArrowMarker *)d_plot->marker(selectedMarker));
      else if (d_images.contains(selectedMarker))
        d_markers_selector->removeAll(
            (ImageMarker *)d_plot->marker(selectedMarker));
    }
    d_plot->removeMarker(selectedMarker);
    d_plot->replot();
    emit modifiedGraph();

    if (selectedMarker == legendMarkerID) legendMarkerID = -1;
...
void Graph::setCopiedTextOptions(int bkg, const QString &text,
...
void Graph::setCopiedArrowOptions(int width, Qt::PenStyle style,
                                  const QColor &color, bool start, bool end,
...
bool Graph::titleSelected() { return d_plot->titleLabel()->hasFocus(); }
...
void Graph::removeLegend() {
  if (legendMarkerID >= 0) {
    ...
    for (int i = index; i < texts - 1; i++) d_texts[i] = d_texts[i + 1];
...
void Graph::updateTextMarker(const QString &text, int angle, int bkg,
...
void Graph::updateCurvesData(Table *w, const QString &colName) {
  ...
  for (int i = 0; i < (int)keys.count(); i++) {
    QwtPlotItem *it = d_plot->plotItem(keys[i]);
    if (!it) continue;
    if (it->rtti() == QwtPlotItem::Rtti_PlotSpectrogram) continue;
    if (static_cast<PlotCurve *>(it)->type() == Function) continue;
    DataCurve *c = static_cast<DataCurve *>(it);
    if (c->table() == w &&
        (c->xColumnName() == colName || c->yColumnName() == colName)) {
...
bool Graph::framed() {
  ...
  if (canvas->lineWidth() > 0) frameOn = true;
...
void Graph::drawAxesBackbones(bool yes) {
  if (drawAxesBackbone == yes) return;
...
void Graph::setAxesLinewidth(int width) {
  if (d_plot->axesLinewidth() == width) return;
...
QString Graph::saveScale() {
  ...
  for (int i = 0; i < QwtPlot::axisCnt; i++) {
    ...
    s += QString::number(qMin(scDiv->lowerBound(), scDiv->upperBound()), 'g',
                         15) +
         "\t";
    s += QString::number(qMax(scDiv->lowerBound(), scDiv->upperBound()), 'g',
                         15) +
         "\t";
#else
    s +=
        QString::number(qMin(scDiv->lBound(), scDiv->hBound()), 'g', 15) + "\t";
    s +=
        QString::number(qMax(scDiv->lBound(), scDiv->hBound()), 'g', 15) + "\t";
#endif
    ...
    s +=
        QString::number(sc_eng->testAttribute(QwtScaleEngine::Inverted)) + "\n";
  }
  return s;
}
...
void Graph::setXAxisTitleColor(const QColor &c) {
  QwtScaleWidget *scale =
      (QwtScaleWidget *)d_plot->axisWidget(QwtPlot::xBottom);
  if (scale) {
...
QString Graph::saveAxesTitleAlignement() {
  ...
  QStringList axes;
  int i;
  for (i = 0; i < 4; i++) axes << QString::number(Qt::AlignHCenter);
...
QString Graph::savePieCurveLayout() {
  ...
  s += QString::number(pieCurve->startRow()) + "\t" +
       QString::number(pieCurve->endRow()) + "\t";
    ...
    for (int i = 0; i < n_curves; i++) {
      ...
      if (c->type() != ErrorBars) {
        if (c->type() == Function) {
          s += "FunctionCurve\t";
          s +=
              QString::number(static_cast<FunctionCurve *>(c)->functionType()) +
              ",";
          s += c->title().text() + ",";
          s += static_cast<FunctionCurve *>(c)->variable() + ",";
          s += QString::number(static_cast<FunctionCurve *>(c)->startRange(),
                               'g', 15) +
               ",";
          s += QString::number(static_cast<FunctionCurve *>(c)->endRange(), 'g',
                               15) +
               "\t";
          s += QString::number(static_cast<FunctionCurve *>(c)->dataSize()) +
               "\t\t\t";
          // the 2 last tabs are legacy code, kept for compatibility with old
          // project files
        } else if (c->type() == Box)
...
Legend *Graph::textMarker(long id) { return (Legend *)d_plot->marker(id); }
...
QwtPlotItem *Graph::plotItem(int index) {
  if (!n_curves || index >= n_curves || index < 0) return 0;
...
int Graph::plotItemIndex(QwtPlotItem *it) const {
  for (int i = 0; i < n_curves; i++) {
    if (d_plot->plotItem(c_keys[i]) == it) return i;
  }
  return -1;
}
...
void Graph::setCurveType(int curve_index, CurveType type, bool update) {
  ...
  if (type != Line && type != Scatter && type != LineSymbols &&
      type != VerticalBars && type != Area && type != VerticalDropLines &&
      type != Spline && type != HorizontalSteps && type != HorizontalBars &&
      type != VerticalSteps)
    return;
  if (old_type != Line && old_type != Scatter && old_type != LineSymbols &&
      old_type != VerticalBars && old_type != Area &&
      old_type != VerticalDropLines && old_type != Spline &&
      old_type != HorizontalSteps && old_type != HorizontalBars &&
      old_type != VerticalSteps)
    return;

  if ((type == VerticalBars || type == HorizontalBars) !=
      (old_type == VerticalBars || old_type == HorizontalBars)) {
    // bar curves are represented by a different class than the other curve
    // types, which makes
...
void Graph::updateCurveLayout(int index, const CurveLayout *cL) {
  ...
  if (!c) return;
  if (c_type.isEmpty() || (int)c_type.size() < index) return;
...
void Graph::updateErrorBars(QwtErrorPlotCurve *er, bool xErr, int width,
...
bool Graph::addErrorBars(const QString &yColName, Table *errTable,
                         const QString &errColName, int type, int width,
                         int cap, const QColor &color, bool through, bool minus,
                         bool plus) {
  QList<int> keys = d_plot->curveKeys();
  for (int i = 0; i < n_curves; i++) {
...
bool Graph::addErrorBars(const QString &xColName, const QString &yColName,
                         Table *errTable, const QString &errColName, int type,
                         int width, int cap, const QColor &color, bool through,
...
bool Graph::insertCurvesList(Table *w, const QStringList &names, int style,
                             int lWidth, int sSize, int startRow, int endRow) {
  if (style == Graph::Pie)
    ...
    for (int i = 0; i < lst.size(); i++) {
      ...
      if (i >= otherCurves.size()) {
        ...
        for (int k = otherCurves.size() - 1; k >= 0; k--) {
          ...
          if (w->colPlotDesignation(index) == AlphaPlot::Y) ycol = index;
        }
        if (ycol < 0) ycol = w->colY(w->colIndex(lst[i]));
        if (ycol < 0) return false;

        if (w->colPlotDesignation(j) == AlphaPlot::xErr)
...
bool Graph::insertCurve(Table *w, const QString &name, int style, int startRow,
...
void Graph::updateScale() {
  ...
  if (!m_autoscale)
#if QWT_VERSION >= 0x050200
    d_plot->setAxisScale(QwtPlot::xBottom, scDiv->lowerBound(),
                         scDiv->upperBound(), step);
#else
    d_plot->setAxisScale(QwtPlot::xBottom, scDiv->lBound(), scDiv->hBound(),
                         step);
#endif
  ...
  if (!m_autoscale)
#if QWT_VERSION >= 0x050200
    d_plot->setAxisScale(QwtPlot::yLeft, scDiv->lowerBound(),
                         scDiv->upperBound(), step);
#else
    d_plot->setAxisScale(QwtPlot::yLeft, scDiv->lBound(), scDiv->hBound(),
                         step);
#endif
...
void Graph::setBarsGap(int curve, int gapPercent, int offset) {
  ...
  if (bars->gap() == gapPercent && bars->offset() == offset) return;
...
void Graph::removePie() {
  if (legendMarkerID >= 0) {
    ...
    if (mrk) mrk->setText(QString::null);
  }
...
void Graph::removeCurves(const QString &s) {
  ...
  for (int i = 0; i < (int)keys.count(); i++) {
    ...
    if (((PlotCurve *)it)->type() == Function) continue;

    if (((DataCurve *)it)->plotAssociation().contains(s)) removeCurve(i);
  }
  d_plot->replot();
}
...
void Graph::removeCurve(int index) {
  if (index < 0 || index >= n_curves) return;
  ...
  if (it->rtti() != QwtPlotItem::Rtti_PlotSpectrogram) {
    ...
    if (d_fit_curves.contains((QwtPlotCurve *)it)) {
      ...
      if (i >= 0 && i < d_fit_curves.size()) d_fit_curves.removeAt(i);
    }
  }
...
ImageMarker *Graph::addImage(const QString &fileName) {
  ...
  if (picSize.width() > w) picSize.setWidth(w);

  int h = d_plot->canvas()->height();
  if (picSize.height() > h) picSize.setHeight(h);
...
bool Graph::modifyFunctionCurve(ApplicationWindow *parent, int curve, int type,
  ...
  if (c->functionType() == type && c->variable() == var &&
      c->formulas() == formulas && c->startRange() == ranges[0] &&
      c->endRange() == ranges[1] && c->dataSize() == points)
    return true;
...
QString Graph::generateFunctionName(const QString &name) {
  ...
  for (int i = 0; i < n_curves; i++) {
    ...
    if (c->type() == Function) lst << c->title().text();
  }
...
bool Graph::addFunctionCurve(ApplicationWindow *parent, int type,
                             const QStringList &formulas, const QString &var,
                             QList<double> &ranges, int points,
                             const QString &title) {
  QString name;
  ...
  FunctionCurve *c =
      new FunctionCurve(parent, (const FunctionCurve::FunctionType)type, name);
...
void Graph::createTable(const QwtPlotCurve *curve) {
  ...
  Column *xCol =
      new Column(tr("1", "curve data table x column name"), AlphaPlot::Numeric);
  Column *yCol =
      new Column(tr("2", "curve data table y column name"), AlphaPlot::Numeric);
...
QString Graph::saveToString(bool saveAsTemplate) {
  ...
  s += "Border\t" + QString::number(d_plot->lineWidth()) + "\t" +
       d_plot->frameColor().name() + "\n";
  s += grid()->saveToString();
  s += saveEnabledAxes();
  ...
  if (!saveAsTemplate) s += saveCurves();
...
void Graph::setFrame(int width, const QColor &color) {
  if (d_plot->frameColor() == color && width == d_plot->lineWidth()) return;
...
void Graph::showTitleContextMenu() {
  ...
  titleMenu.addAction(QPixmap(":/copy.xpm"), tr("&Copy"), this,
                      SLOT(copyTitle()));
...
void Graph::showAxisTitleMenu(int axis) {
  ...
  QMenu titleMenu(this);
  titleMenu.addAction(QPixmap(":/cut.xpm"), tr("&Cut"), this,
                      SLOT(cutAxisTitle()));
  titleMenu.addAction(QPixmap(":/copy.xpm"), tr("&Copy"), this,
                      SLOT(copyAxisTitle()));
...
void Graph::showAxisContextMenu(int axis) {
  ...
  if (axis == QwtScaleDraw::LeftScale || axis == QwtScaleDraw::RightScale) {
    if (d_plot->grid()->yEnabled()) gridsID->setChecked(true);
  } else {
    if (d_plot->grid()->xEnabled()) gridsID->setChecked(true);
  }
...
void Graph::showAxisDialog() { emit showAxisDialog(selectedAxis); }

void Graph::showScaleDialog() { emit axisDblClicked(selectedAxis); }
...
void Graph::showGrids() { showGrid(selectedAxis); }
...
void Graph::copy(ApplicationWindow *parent, Graph *g) {
  ...
  for (i = 0; i < g->curves(); i++) {
    ...
    if (it->rtti() == QwtPlotItem::Rtti_PlotCurve) {
      ...
      if (lst.contains((QwtPlotCurve *)it)) d_fit_curves << c;
    ...
    if (sd->hasComponent(QwtAbstractScaleDraw::Labels)) {
      if (axisType[i] == Graph::Numeric)
        setLabelsNumericFormat(i, plot->axisLabelFormat(i),
                               plot->axisLabelPrecision(i), axesFormulas[i]);
...
#if QWT_VERSION >= 0x050200
    QwtScaleDiv div =
        sc_engine->divideScale(qMin(sd->lowerBound(), sd->upperBound()),
                               qMax(sd->lowerBound(), sd->upperBound()),
                               majorTicks, minorTicks, d_user_step[i]);
#else
    QwtScaleDiv div = sc_engine->divideScale(
        qMin(sd->lBound(), sd->hBound()), qMax(sd->lBound(), sd->hBound()),
        majorTicks, minorTicks, d_user_step[i]);
#endif
...
void Graph::plotBoxDiagram(Table *w, const QStringList &names, int startRow,
  ...
  d_plot->setAxisScaleDraw(QwtPlot::xTop,
                           new QwtTextScaleDraw(w->selectedYLabels()));
...
void Graph::disableTools() {
  if (zoomOn()) zoom(false);
  if (drawLineActive()) drawLine(false);
  setActiveTool(NULL);
  if (d_range_selector) delete d_range_selector;
}

bool Graph::enableRangeSelectors(const QObject *status_target,
                                 const char *status_slot) {
  // disable other tools, otherwise it's undefined what tool should handle mouse
  // clicks
  disableTools();
...
void Graph::guessUniqueCurveLayout(int &colorIndex, int &symbolIndex) {
  ...
  if (curve_index >= 0 &&
    ...
    QwtErrorPlotCurve *er =
        (QwtErrorPlotCurve *)d_plot->curve(c_keys[curve_index]);
  ...
  for (int i = 0; i < n_curves; i++) {
    ...
    if (c && c->rtti() == QwtPlotItem::Rtti_PlotCurve) {
      ...
      if (index > colorIndex) colorIndex = index;
  ...
  if (n_curves > 1) colorIndex = (colorIndex + 1) % 16;
...
void Graph::plotSpectrogram(Matrix *m, CurveType type) {
  if (type != GrayMap && type != ColorMap && type != ContourMap) return;
...
void Graph::restoreSpectrogram(ApplicationWindow *app, const QStringList &lst) {
  ...
  for (line++; line != lst.end(); line++) {
    ...
    if (s.contains("<ColorPolicy>")) {
      int color_policy =
          s.remove("<ColorPolicy>").remove("</ColorPolicy>").trimmed().toInt();
      ...
      int contours = s.remove("<ContourLines>")
                         .remove("</ContourLines>")
                         .trimmed()
                         .toInt();
      ...
      if (contours) {
        ...
        s = (*(++line)).trimmed();
        int defaultPen =
            s.remove("<DefaultPen>").remove("</DefaultPen>").toInt();
        if (!defaultPen)
...
void Graph::setAntialiasing(bool on, bool update) {
  ...
  if (update) {
    ...
    for (int i = 0; i < (int)curve_keys.count(); i++) {
      ...
      if (c) c->setRenderHint(QwtPlotItem::RenderAntialiased, d_antialiasing);
    }
    QList<int> marker_keys = d_plot->markerKeys();
    for (int i = 0; i < (int)marker_keys.count(); i++) {
      QwtPlotMarker *m = d_plot->marker(marker_keys[i]);
      if (m) m->setRenderHint(QwtPlotItem::RenderAntialiased, d_antialiasing);
    }
    d_plot->replot();
  }
}
...
bool Graph::focusNextPrevChild(bool) {
  ...
  for (int i = 0; i < n; i++) {
    if (mrkKeys[i] >= max_key) max_key = mrkKeys[i];
    if (mrkKeys[i] <= min_key) min_key = mrkKeys[i];
  }
...
DataCurve *Graph::masterCurve(QwtErrorPlotCurve *er) {
  ...
  for (int i = 0; i < (int)keys.count(); i++) {
    ...
    if (((DataCurve *)it)->plotAssociation() ==
