...
void QwtErrorPlotCurve::drawErrorBars(QPainter *painter,
  ...
  for (int i = from; i <= to; i++) {
    ...
    if (type == Vertical) {
      ...
      // draw caps
      if (plus)
        QwtPainter::drawLine(painter, xi - cap / 2, y_plus, xi + cap / 2,
                             y_plus);
      if (minus && y_minus_is_finite)
        QwtPainter::drawLine(painter, xi - cap / 2, y_minus, xi + cap / 2,
                             y_minus);
      ...
      // draw caps
      if (plus)
        QwtPainter::drawLine(painter, x_plus, yi - cap / 2, x_plus,
                             yi + cap / 2);
      if (minus && x_minus_is_finite)
        QwtPainter::drawLine(painter, x_minus, yi - cap / 2, x_minus,
                             yi + cap / 2);
...
QwtDoubleRect QwtErrorPlotCurve::boundingRect() const {
  ...
  rect.setTop(std::min(minrect.top(), maxrect.top()));
  rect.setBottom(std::max(minrect.bottom(), maxrect.bottom()));
  rect.setLeft(std::min(minrect.left(), maxrect.left()));
  rect.setRight(std::max(minrect.right(), maxrect.right()));
...
bool QwtErrorPlotCurve::loadData() {
  ...
  QList<QVector<double> > data =
      convertData(d_master_curve->type() == Graph::HorizontalBars
                      ? (QList<Column *>() << y << x << err)
                      : (QList<Column *>() << x << y << err),
                  QList<int>() << xAxis() << yAxis()
                               << (type == Horizontal ? xAxis() : yAxis()));
...
QString QwtErrorPlotCurve::plotAssociation() {
  if (!d_master_curve) return QString();
...
bool QwtErrorPlotCurve::updateData(Table *t, const QString &colName) {
  if (d_table != t || colName != title().text()) return false;
