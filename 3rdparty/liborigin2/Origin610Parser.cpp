...
bool Origin610Parser::parse()
{
	...
	bool binData = false;
	vector<string> binSpreadSheets, statGraphNames;
	while(size > 0 && size <= 0x8C){// should be 0x72, 0x73 or 0x83 ?
		...
		if (name.substr(0, 4) == "SBin"){
			binData = true;
			binSpreadSheets.push_back(name.substr(1));
			LOG_PRINT(logfile, "	FOLLOWS BIN DATA TABLE: %s\n", name.substr(1).c_str());
		}
		...
		if(columnname.empty()){
			...
			if (signature == 0xAC8 && size != 1)
				signature = 0xA00;//unknown data to be skipped
			...
			case 0xACA:
				if (size){
					//matrices.push_back(Matrix(name, dataIndex));
	...
	if (binSpreadSheets.size() && statGraphNames.size()){
		for (unsigned int i = 0; i < speadSheets.size(); i++){
			for (unsigned int j = 0; j < binSpreadSheets.size() && j < statGraphNames.size(); j++){
				if (speadSheets[i].name == statGraphNames[j])
					speadSheets[i].name = binSpreadSheets[j];
			}
		}
	}
...
void Origin610Parser::readNotes()
{
	...
	while(!file.eof()){
		...
		LOG_PRINT(logfile, "NOTE %d NAME: %s\n", (int)notes.size(), notes.back().name.c_str());
		LOG_PRINT(logfile, "NOTE %d LABEL: %s\n", (int)notes.size(), notes.back().label.c_str());
		LOG_PRINT(logfile, "NOTE %d TEXT: %s\n", (int)notes.size(), notes.back().text.c_str());
...
void Origin610Parser::readMatrixInfo()
{
	...
	while(!file.eof()){
		...
		string sec_name(LAYER + sectionSize - sectionNamePos + 1, 0);
...
bool Origin610Parser::readGraphInfo()
{
	...
	while(!file.eof()){// multilayer loop
		...
		while(size && !file.eof()){
			...
			file.seekg(LAYER + 0x09, ios_base::beg);
			file >> systemFont;
			LOG_PRINT(logfile, "				systemFont = %d (@ 0x%X)\n", systemFont, (unsigned int)file.tellg());
			*/
			...
			file.seekg(LAYER + 0x0E, ios_base::beg);
			...
			else if (osize == 24 || osize == 96) // line
			{//type == 2 ->straight line, type == 4 ->curved line
		...
		if(size)//check layer is not empty
		{
			while(!file.eof()){
				...
				file >> curve.type;
				if (!curve.type){
					LOG_PRINT(logfile, "			Found unknown curve type (%d)! NOT A GRAPH!\n", (int)curve.type);
				} else
