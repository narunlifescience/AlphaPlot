...
bool Origin610Parser::parse() {
  ...
  LOG_PRINT(logfile, "	[column found = %d/0x%X @ 0x%X]", size, size,
    ...
    if (columnname.empty()) {
      ...
      switch (signature) {
        ...
        default:
          ...
          if (valuesize != 8 && valuesize <= 16) file.seekg(2, ios_base::cur);
      }
    } else {  // worksheet
  ...
  while (POS < d_file_size) {
    ...
    if (findSpreadByName(name) != -1) {
      ...
      LOG_PRINT(logfile,
                "		%s is NOT A GRAPH, trying to read next "
                "SPREADSHEET...\n",
                name.c_str())
...
void Origin610Parser::readNotes() {
  ...
  while (pos < d_file_size) {
    ...
    if (state == 0x04) notes.back().state = Window::Minimized;
...
bool Origin610Parser::readGraphInfo() {
  ...
  while (LAYER < d_file_size) {  // multilayer loop
    ...
    file >> col;
    layer.backgroundColor.type =
        (col & 0x01) ? Origin::Color::None : Origin::Color::Regular;
    file >> col;
    ...
    while (LAYER < d_file_size) {
      ...
      if (!sec_name.empty())
        ...
        if (!size || size != sectionSize) break;
