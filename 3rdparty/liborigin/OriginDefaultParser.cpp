...
bool OriginDefaultParser::parse() {
  ...
  while (col_found > 0 && col_found < 0x84) {  // should be 0x72, 0x73 or 0x83
    ...
    while (char* tmpstr =
               strtok(NULL, "_")) {  // get multiple-"_" title correct
      strcat(sname, "_");
      strcat(sname, cname);
      strcpy(cname, tmpstr);
    }
    ...
    if (cname == 0) {
      ...
      for (i = 0; i < size; i++) {  // read data
        ...
        speadSheets[speadSheets.size() - 1].columns[i].data.push_back(
            /*Data(value)*/ value);
  ...
  for (unsigned int i = 0; i < speadSheets.size(); i++) {
    ...
    if (jump == MAX_LEVEL) {
      fprintf(debug,
              "		Spreadsheet SECTION not found ! 	(@ 0x%X)\n",
              POS - 10 * 0x1F2 + 0x55);
      return -5;
    }
    ...
    for (unsigned int j = 0; j < speadSheets[spread].columns.size(); j++) {
      ...
      fprintf(debug,
              "				COLUMN \"%s\" type = %d (@ 0x%X)\n",
              speadSheets[spread].columns[j].name.c_str(), type,
      ...
      if (speadSheets[spread].columns[j].name.substr(0, length) == name) {
        fprintf(
            debug,
            "				TEST : column name = \"%s\". OK!\n",
            speadSheets[spread].columns[j].name.c_str());
      } else {
        fprintf(debug,
                "				TEST : COLUMN %d name mismatch "
                "(\"%s\" != \"%s\")\n",
