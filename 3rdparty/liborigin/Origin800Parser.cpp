...
bool Origin800Parser::parse() {
  if (fileVersion >= 2882) d_colormap_offset = 0x25F;
      ...
      switch (signature) {
        ...
        case 0x70E2:
          ...
          if (pos != string::npos) {
            ...
            LOG_PRINT(
                logfile,
                "MATRIX %s is multisheet, only first sheet will be imported!\n",
                name.c_str())
...
void Origin800Parser::readNotes() {
  ...
  while (pos < d_file_size) {
    ...
    if (!rect.bottom || !rect.right) break;
...
void Origin800Parser::readSpreadInfo() {
  ...
  while (LAYER < d_file_size) {
    ...
    if (col_index != -1) {
      if (speadSheets[spread].columns[col_index].sheet)  // read only column
  ...
  if (speadSheets[spread].sheets > 1) {
    ...
    for (unsigned int i = 0; i < 8; i++) skipLine();
    ...
    for (unsigned int sheet = 1; sheet < speadSheets[spread].sheets; sheet++) {
      for (unsigned int i = 0; i < columns; i++) {
        if (speadSheets[spread].columns[i].sheet != sheet) continue;
...
void Origin800Parser::readColumnInfo(int spread, int i) {
  ...
  if (sheetpos != string::npos) colName.resize(sheetpos);

  if (colName.size() >= 11) colName.resize(11);
  ...
  switch (c1) {
    ...
    case 0x30:  // Numeric	   - Dec1,000
    case 0x39:  // Text&Numeric - Dec1,000
      speadSheets[spread].columns[i].valueType =
          (c1 % 0x10 == 0x9) ? TextNumeric : Numeric;
...
void Origin800Parser::readMatrixInfo() {
  ...
  unsigned short view;
  file >> view;
  if (view == 3) matrixes[idx].view = Matrix::ImageView;
  ...
  for (int i = 0; i < 4; i++) {
    if (findSection(sectionNames[i], stringSize, maxSearchPos)) {
      ...
      LOG_PRINT(logfile, "		%s: %g cursor pos: 0x%X\n",
                sectionNames[i], matrixes[idx].coordinates[i],
                (unsigned int)file.tellg())
    }
  }
...
void Origin800Parser::readGraphInfo() {
  ...
  if (templateName == "LAYOUT") graphs.back().isLayout = true;
...
bool Origin800Parser::findSection(const string& name, unsigned int length,
                                  unsigned int maxLength) {
  if (!maxLength) maxLength = d_file_size - 16;
