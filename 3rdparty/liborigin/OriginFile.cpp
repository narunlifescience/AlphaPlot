...
OriginFile::OriginFile(const string& fileName) : fileVersion(0) {
...
double OriginFile::version() const { return fileVersion / 100.0; }
...
Origin::SpreadSheet& OriginFile::spread(
    vector<Origin::SpreadSheet>::size_type s) const {
...
vector<Origin::Function>::size_type OriginFile::functionIndex(
    const string& name) const {
  return parser->findFunctionByName(name);
}

Origin::Function& OriginFile::function(
    vector<Origin::Function>::size_type f) const {
...
string OriginFile::resultsLogString() const { return parser->resultsLog; }
