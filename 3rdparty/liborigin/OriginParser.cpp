...
vector<Origin::SpreadSheet>::difference_type OriginParser::findSpreadByName(
    const string& name) const {
  for (vector<SpreadSheet>::const_iterator it = speadSheets.begin();
       it != speadSheets.end(); ++it) {
    if (iequals(it->name, name, locale())) return it - speadSheets.begin();
  }
  return -1;
}

vector<Origin::Excel>::difference_type OriginParser::findExcelByName(
    const string& name) const {
  for (vector<Excel>::const_iterator it = excels.begin(); it != excels.end();
       ++it) {
...
OriginParser::findSpreadColumnByName(
    vector<Origin::SpreadSheet>::size_type spread, const string& name) const {
  for (vector<SpreadColumn>::const_iterator it =
           speadSheets[spread].columns.begin();
       it != speadSheets[spread].columns.end(); ++it) {
...
OriginParser::findExcelColumnByName(
  ...
  for (vector<SpreadColumn>::const_iterator it =
           excels[excel].sheets[sheet].columns.begin();
...
vector<Origin::Matrix>::difference_type OriginParser::findMatrixByName(
    const string& name) const {
  for (vector<Matrix>::const_iterator it = matrixes.begin();
       it != matrixes.end(); ++it) {
...
vector<Origin::Function>::difference_type OriginParser::findFunctionByName(
    const string& name) const {
  for (vector<Function>::const_iterator it = functions.begin();
       it != functions.end(); ++it) {
...
pair<string, string> OriginParser::findDataByIndex(unsigned int index) const {
  for (vector<SpreadSheet>::const_iterator it = speadSheets.begin();
    ...
    for (vector<SpreadColumn>::const_iterator it1 = it->columns.begin();
      ...
      if (it1->index == index) return make_pair("T_" + it->name, it1->name);
    }
  }

  for (vector<Matrix>::const_iterator it = matrixes.begin();
       it != matrixes.end(); ++it) {
    if (it->index == index) return make_pair("M_" + it->name, it->name);
  }

  for (vector<Excel>::const_iterator it = excels.begin(); it != excels.end();
       ++it) {
    for (vector<SpreadSheet>::const_iterator it1 = it->sheets.begin();
         it1 != it->sheets.end(); ++it1) {
      for (vector<SpreadColumn>::const_iterator it2 = it1->columns.begin();
           it2 != it1->columns.end(); ++it2) {
        if (it2->index == index) return make_pair("E_" + it->name, it2->name);
      }
    }
  }
  ...
  for (vector<Function>::const_iterator it = functions.begin();
       it != functions.end(); ++it) {
    if (it->index == index) return make_pair("F_" + it->name, it->name);
  }

  return pair<string, string>();
}

pair<ProjectNode::NodeType, string> OriginParser::findObjectByIndex(
  ...
  for (vector<Graph>::const_iterator it = graphs.begin(); it != graphs.end();
       ++it) {
    if (it->objectID == (int)index) {
      if (it->is3D)
...
void OriginParser::convertSpreadToExcel(
    vector<Origin::SpreadSheet>::size_type spread) {
  // add new Excel sheet
  excels.push_back(Excel(speadSheets[spread].name, speadSheets[spread].label,
                         speadSheets[spread].maxRows,
                         speadSheets[spread].hidden,
                         speadSheets[spread].loose));

  for (vector<SpreadColumn>::iterator it = speadSheets[spread].columns.begin();
       it != speadSheets[spread].columns.end(); ++it) {
