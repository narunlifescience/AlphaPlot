...
/// Base class for iterators, only pointers stored, no traversal logic.
  ...
  /// Return fixed-depth iterator to end of the nodes at given depth from the
  /// given iterator.
...
//    sibling_iterator subfrom, sibling_iterator subto, iterator from, iterator
//    to,
//    BinaryPredicate fun) const
//    {
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator
tree<T, tree_node_allocator>::child(const iterator_base& it,
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator&
    tree<T, tree_node_allocator>::pre_order_iterator::
    operator++() {
  assert(this->node != 0);
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::post_order_iterator&
    tree<T, tree_node_allocator>::post_order_iterator::
    operator++() {
  assert(this->node != 0);
...
void tree<T, tree_node_allocator>::fixed_depth_iterator::set_first_parent_() {
  return;  // FIXME: we do not use first_parent_ yet, and it actually needs some
           // serious reworking if
           // it is ever to work at the 'head' level.
  first_parent_ = 0;
  if (this->node == 0) return;
  if (this->node->parent != 0) first_parent_ = this->node->parent;
  if (first_parent_) find_leftmost_parent_();
}
...
void tree<T,
  ...
  while (tmppar->prev_sibling) {
    ...
    if (tmppar->first_child) first_parent_ = tmppar;
  }
}

template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::fixed_depth_iterator&
    tree<T, tree_node_allocator>::fixed_depth_iterator::
    operator++() {
  assert(this->node != 0);
...
typename tree<T, tree_node_allocator>::fixed_depth_iterator&
  ...
  if (this->node->prev_sibling != 0) {
    ...
    if (this->node->parent == 0 &&
        this->node->prev_sibling == 0)  // head element
      this->node = 0;
  } else {
    tree_node* par = this->node->parent;
    do {
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator&
    tree<T, tree_node_allocator>::leaf_iterator::
    operator++() {
  assert(this->node != 0);
  while (this->node->next_sibling == 0) {
