...
bool Origin750Parser::parse() {
  ...
  while (size > 0 && size < 0x84) {  // should be 0x72, 0x73 or 0x83
    ...
    if (columnname.empty()) {
      LOG_PRINT(logfile,
                "NO COLUMN NAME FOUND! Must be a Matrix or Function.\n")
      ...
      switch (signature) {
        ...
        case 0x70E2:
          ...
          if (size >= 100) LOG_PRINT(logfile, " matrix too big...")
          ...
          switch (data_type) {
            case 0x6001:  // double
              for (unsigned int i = 0; i < size; ++i) {
                ...
                if (size < 100) LOG_PRINT(logfile, "%g ", value)
              }
              break;
            case 0x6003:  // float
              for (unsigned int i = 0; i < size; ++i) {
            ...
            case 0x6801:  // int
              if (data_type_u == 8)  // unsigned
              {
                for (unsigned int i = 0; i < size; ++i) {
                  ...
                  file >> value;
                  matrixes.back().data.push_back((double)value);
                  if (size < 100) LOG_PRINT(logfile, "%u ", value)
                }
              } else {
                for (unsigned int i = 0; i < size; ++i) {
                  int value;
                  file >> value;
                  matrixes.back().data.push_back((double)value);
                  if (size < 100) LOG_PRINT(logfile, "%d ", value)
                }
              }
              break;
            case 0x6803:  // short
        ...
        case 0x10C8:
          ...
          if (t == 0x1194) functions.back().type = Function::Polar;
          ...
          file >> d;
          functions.back().end =
              functions.back().begin + d * (functions.back().totalPoints - 1);

          LOG_PRINT(logfile, "FUNCTION %s : %s\n",
                    functions.back().name.c_str(),
                    functions.back().formula.c_str())
          LOG_PRINT(logfile, " interval %g : %g, number of points %d\n",
                    functions.back().begin, functions.back().end,
                    functions.back().totalPoints)
      ...
      LOG_PRINT(logfile, "SPREADSHEET = %s COLUMN NAME = %s (%d) (@0x%X)\n",
                name.c_str(), columnname.c_str(), current_col,
                (unsigned int)file.tellg())
      speadSheets[spread].columns.push_back(
          SpreadColumn(columnname, dataIndex));
      string::size_type sheetpos =
          speadSheets[spread].columns.back().name.find_last_of("@");
      ...
      for (unsigned int i = 0; i < nr; ++i) {
        ...
        } else if ((data_type & 0x100) == 0x100)  // Text&Numeric
        {
          ...
          if (c == 0)  // value
          {
            file >> value;
            LOG_PRINT(logfile, "%g ", value)
            speadSheets[spread].columns[(current_col - 1)].data.push_back(
                value);
    ...
    file.seekg(1, ios_base::cur);
    LOG_PRINT(logfile, "	[column found = %d/0x%X (@ 0x%X)]\n", size,
              size, ((unsigned int)file.tellg() - 5))
    colpos = file.tellg();
  }
  ...
  unsigned int LAYER = POS;
  {
    ...
    while (1) {
      ...
      if (col_index != -1) {
        ...
        LOG_PRINT(logfile,
                  "				Column: %s has formula: %s\n",
                  sec_name.c_str(),
                  speadSheets[spread].columns[col_index].command.c_str());
      }
      ...
      if (sec_name == "__LayerInfoStorage") break;
    }
    LAYER += 0x5;
  }
    ...
    if (col_index != -1) {
      ...
      switch (c1) {
        ...
        case 0x39:  // Text&Numeric - Dec1,000
          speadSheets[spread].columns[col_index].valueType =
              (c1 % 0x10 == 0x9) ? TextNumeric : Numeric;
          speadSheets[spread].columns[col_index].valueTypeSpecification =
              c1 / 0x10;
          if (c2 >= 0x80) {
            speadSheets[spread].columns[col_index].significantDigits =
                c2 - 0x80;
            speadSheets[spread].columns[col_index].numericDisplayType =
                SignificantDigits;
          } else if (c2 > 0) {
      ...
      LOG_PRINT(
          logfile,
          "				COLUMN \"%s\" type = %s(%d) (@ 0x%X)\n",
          speadSheets[spread].columns[col_index].name.c_str(),
          colTypeNames[type], (int)c, (LAYER + 0x11))
    }
    LAYER += 0x1E7 + 0x1;
...
void Origin750Parser::readExcelInfo() {
  ...
  while (1)  // multisheet loop
  {
    ...
    while (1) {
      ...
      if (col_index != -1) {
        file >> excels[iexcel].sheets[isheet].columns[col_index].command.assign(
                    size, 0);
      }
    ...
    /////////////// COLUMN Types ///////////////////////////////////////////
    LOG_PRINT(logfile,
              "			Excel sheet %d has %zu columns\n", isheet,
              excels[iexcel].sheets[isheet].columns.size())
    ...
    while (1) {
      ...
      if (col_index != -1) {
        ...
        switch (c1) {
          ...
          case 0x39:  // Text&Numeric - Dec1,000
            ...
            excels[iexcel]
                .sheets[isheet]
                .columns[col_index]
                .valueTypeSpecification = c1 / 0x10;
            if (c2 >= 0x80) {
              excels[iexcel]
                  .sheets[isheet]
                  .columns[col_index]
                  .significantDigits = c2 - 0x80;
              excels[iexcel]
                  .sheets[isheet]
                  .columns[col_index]
                  .numericDisplayType = SignificantDigits;
            } else if (c2 > 0) {
              excels[iexcel].sheets[isheet].columns[col_index].decimalPlaces =
                  c2 - 0x03;
              excels[iexcel]
                  .sheets[isheet]
                  .columns[col_index]
                  .numericDisplayType = DecimalPlaces;
            }
            break;
          case 0x02:  // Time
            ...
            excels[iexcel]
                .sheets[isheet]
                .columns[col_index]
                .valueTypeSpecification = c2 - 0x80;
            break;
          case 0x03:  // Date
        ...
        LOG_PRINT(
            logfile,
            "				COLUMN \"%s\" type = %d(%d) (@ 0x%X)\n",
            excels[iexcel].sheets[isheet].columns[col_index].name.c_str(), type,
            c, (LAYER + 0x11))
      }
      LAYER += 0x1E7 + 0x1;
      ...
      if (size > 0) {
        if (col_index != -1) {
          file.seekg(LAYER, ios_base::beg);
          file >>
              excels[iexcel].sheets[isheet].columns[col_index].comment.assign(
                  size, 0);
        }
        LAYER += size + 0x1;
      }
  ...
  while (1)  // multilayer loop
  {
    ...
    while (1) {
      ...
      if (sec_name == "XB") {
        ...
        layer.xAxis.formatAxis[0].label =
        ...
        layer.xAxis.formatAxis[1].label =
            TextBox(text, r, color, fontSize, rotation / 10, tab,
                    (BorderType)(border >= 0x80 ? border - 0x80 : None),
                    (Attach)attach);
      } else if (sec_name == "YL") {
        ...
        layer.yAxis.formatAxis[0].label =
            TextBox(text, r, color, fontSize, rotation / 10, tab,
                    (BorderType)(border >= 0x80 ? border - 0x80 : None),
                    (Attach)attach);
      } else if (sec_name == "YR") {
        ...
        layer.yAxis.formatAxis[1].label =
            TextBox(text, r, color, fontSize, rotation / 10, tab,
                    (BorderType)(border >= 0x80 ? border - 0x80 : None),
                    (Attach)attach);
      } else if (sec_name == "ZF") {
        ...
        layer.zAxis.formatAxis[0].label =
            TextBox(text, r, color, fontSize, rotation / 10, tab,
                    (BorderType)(border >= 0x80 ? border - 0x80 : None),
                    (Attach)attach);
      } else if (sec_name == "ZB") {
        ...
        layer.zAxis.formatAxis[1].label =
            TextBox(text, r, color, fontSize, rotation / 10, tab,
                    (BorderType)(border >= 0x80 ? border - 0x80 : None),
                    (Attach)attach);
      } else if (sec_name == "3D") {
    ...
    if (size == 0x1E7)  // check layer is not empty
    {
      while (1) {
        ...
        LOG_PRINT(
            logfile,
            "			graph %zu layer %zu curve %zu type : %d\n",
            graphs.size(), graphs.back().layers.size(), layer.curves.size(),
            (int)curve.type)
        ...
        if (column.first.size() > 0) {
          ...
          if (layer.is3D()) {
            LOG_PRINT(logfile,
                      "			graph %zu layer %zu curve %zu Z "
                      ": %s.%s\n",
                      graphs.size(), graphs.back().layers.size(),
                      layer.curves.size(), column.first.c_str(),
                      column.second.c_str())
            curve.zColumnName = column.second;
          } else {
            LOG_PRINT(logfile,
                      "			graph %zu layer %zu curve %zu Y "
                      ": %s.%s\n",
                      graphs.size(), graphs.back().layers.size(),
                      layer.curves.size(), column.first.c_str(),
                      column.second.c_str())
        ...
        if (column.first.size() > 0 && layer.is3D()) {
          LOG_PRINT(
              logfile,
              "			graph %zu layer %zu curve %zu X : %s.%s\n",
              graphs.size(), graphs.back().layers.size(), layer.curves.size(),
        ...
        if (layer.is3D() || layer.isXYY3D) graphs.back().is3D = true;
        ...
        // vector
        if (curve.type == GraphCurve::FlowVector ||
            curve.type == GraphCurve::Vector) {
          file.seekg(LAYER + 0x56, ios_base::beg);
        ...
        if (curve.type == GraphCurve::Mesh3D ||
            curve.type == GraphCurve::Contour) {
          ColorMap& colorMap =
              (curve.type == GraphCurve::Mesh3D ? curve.surface.colorMap
                                                : curve.colorMap);
          file.seekg(LAYER + 0x13, ios_base::beg);
          file >> h;
...
void Origin750Parser::readProjectTree() {
  ...
  for (unsigned int i = 0; i < matrixes.size(); ++i) {
    Matrix m = matrixes[i];
    if (m.objectID >= 0) validMatrices.push_back(m);
  }
  matrixes.clear();
  matrixes = validMatrices;
}

void Origin750Parser::readProjectTreeFolder(
  ...
  file >> creationDate;
  if (creationDate >= 1e10) return;

  file >> modificationDate;
  if (modificationDate >= 1e10) return;
  ...
  tree<ProjectNode>::iterator current_folder = projectTree.append_child(
      parent,
      ProjectNode(name, ProjectNode::Folder, doubleToPosixTime(creationDate),
  ...
  for (unsigned int i = 0; i < objectcount; ++i) {
    ...
    if (c == 0x10)
      projectTree.append_child(
          current_folder, ProjectNode(notes[objectID].name, ProjectNode::Note));
    else {
      pair<ProjectNode::NodeType, string> object = findObjectByIndex(objectID);
      projectTree.append_child(current_folder,
                               ProjectNode(object.second, object.first));
    }
...
void Origin750Parser::readWindowProperties(Window& window, unsigned int size) {
  ...
  file.read(reinterpret_cast<char*>(&window.frameRect),
            sizeof(window.frameRect));
