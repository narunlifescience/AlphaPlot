...
struct Window {
  ...
  Window(const string& _name = "", const string& _label = "",
         bool _hidden = false)
      : name(_name),
        label(_label),
        objectID(-1),
...
struct SpreadSheet : public Window {
  ...
  SpreadSheet(const string& _name = "")
      : Window(_name), loose(true), multisheet(false), sheets(1) {}
};
...
struct Excel : public Window {
  ...
  Excel(const string& _name = "", const string& _label = "", int _maxRows = 0,
...
struct TextBox {
  ...
  TextBox(const string& _text = "") : text(_text) {}

  TextBox(const string& _text, const Rect& _clientRect, const Color& _color,
          unsigned short _fontSize, int _rotation, int _tab,
          BorderType _borderType, Attach _attach)
      : text(_text),
...
struct Figure {
  ...
  Figure(FigureType _type = Rectangle) : type(_type) {}
};
...
struct LineVertex {
  ...
  LineVertex()
      : shapeType(0), shapeWidth(0.0), shapeLength(0.0), x(0.0), y(0.0) {}
};
...
struct Bitmap {
  ...
  Bitmap(const string& _name = "")
      : size(0), windowName(_name), borderType(Origin::None), data(0) {}
...
struct GraphLayer {
  ...
  GraphLayer() : imageProfileTool(false), isXYY3D(false) {}

  // bool threeDimensional;
  bool is3D() const {
    for (vector<GraphCurve>::const_iterator it = curves.begin();
         it != curves.end(); ++it) {
...
struct Graph : public Window {
  ...
  Graph(const string& _name = "")
      : Window(_name), is3D(false), isLayout(false) {}
};

struct Note : public Window {
  string text;
  Note(const string& _name = "") : Window(_name) {}
};
