    ...
    : ExtensibleFileDialog(parent, extended, flags)
{
    ...
    else if (app->d_ASCII_import_locale.name()
             == QLocale(QLocale::German).name())
        boxDecimalSeparator->setCurrentIndex(2);
    else if (app->d_ASCII_import_locale.name()
             == QLocale(QLocale::French).name())
...
void ImportASCIIDialog::initAdvancedOptions()
{
    ...
    QString help_strip_spaces =
            tr("By checking this option all white spaces will be \nremoved "
               ...
               "when translating this check the what's this functions and tool "
               "tips "
               "to place the '\\n's correctly");
    help_strip_spaces += "\n\n"
    ...
    // context-sensitive help
    QString help_simplify_spaces = tr("By checking this option all white "
                                      "spaces will be \nremoved from the "
                                      "beginning and the end of the \nlines "
                                      "and each sequence of internal "
                                      "\nwhitespaces (including the TAB "
                                      "character) will \nbe replaced with "
                                      "a single space.",
                                      "when translating this check the what's "
                                      "this functions and tool tips "
                                      "to place the '\\n's correctly");
    ...
    boxDecimalSeparator->addItem(tr("default") + " ("
...
{
    QString s = tr("The column separator can be customized. The following "
                   "special codes "
                   "can be used:\n\\t for a TAB character \n\\s for a SPACE");
    s += "\n"
            + tr("The separator must not contain the following characters: "
    ...
    s += tr("By checking this option each sequence of internal whitespaces "
            "(including the TAB character) will be replaced with a single "
            "space.");
    s += tr("By checking this option all white spaces will be removed from the "
    ...
    s += "\n\n"
            + tr("Warning: using these two last options leads to column "
                 "overlaping if "
                 "the columns in the ASCII file don't have the same number of "
                 "rows.");
    s += "\n"
