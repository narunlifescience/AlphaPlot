      ...
      d_targets(targets)
{
    ...
    if (d_type == TableStatistics::StatRow) {
        ...
        for (int i = 0; i < 10; i++)
            setColumnType(i, AlphaPlot::Numeric);
        ...
        pFilter =
                qobject_cast<Double2StringFilter *>(column(1)->outputFilter());
        Q_ASSERT(pFilter != NULL);
        pFilter->setNumDigits(0);
        pFilter->setNumericFormat('f');
        pFilter =
                qobject_cast<Double2StringFilter *>(column(9)->outputFilter());
        Q_ASSERT(pFilter != NULL);
        pFilter->setNumDigits(0);
        pFilter->setNumericFormat('f');
        ...
        for (int i = 0; i < 2; i++)
            setColumnType(i, AlphaPlot::Text);
        ...
        pFilter->setNumericFormat('f');
        pFilter =
                qobject_cast<Double2StringFilter *>(column(11)->outputFilter());
        Q_ASSERT(pFilter != NULL);
        pFilter->setNumDigits(0);
        pFilter->setNumericFormat('f');
    ...
    if (d_type == TableStatistics::StatRow) {
        ...
        for (int destRow = 0; destRow < d_targets.size(); destRow++) {
            if (colName
                ...
                if (col->columnMode() != AlphaPlot::Numeric)
                    return;
                ...
                column(1)->setTextAt(destRow,
                                     "[1:" + QString::number(rows) + "]");
                column(2)->setValueAt(destRow, mean);
                column(3)->setValueAt(destRow, sd);
