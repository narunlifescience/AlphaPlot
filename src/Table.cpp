...
void Table::init()
{
    ...
    connect(ui.set_formula_button, SIGNAL(pressed()), this,
            SLOT(applyFormula()));
    connect(ui.add_function_button, SIGNAL(pressed()), this,
            SLOT(addFunction()));
...
void Table::print(const QString &fileName)
{
    ...
    for (i = 0; i < cols; i++) {
        ...
        header_label = d_view_widget->model()
...
bool Table::recalculate(int col, bool only_selected_rows)
{
    ...
    if (only_selected_rows) {
        ...
        QList<Interval<int>> deselected =
                Interval<int>(0, col_ptr->rowCount() - 1)
    ...
    foreach (Interval<int> interval, formula_intervals) {
        ...
        Script *colscript = scriptEnv->newScript(
                formula, this, QString("<%1>").arg(colName(col)));
...
{
    for (int j = 0; j < numCols(); j++) {
        if (column(j)->plotDesignation() == AlphaPlot::X)
            return j;
    }
    return -1;
}
...
void Table::importV0x0001XXHeader(QStringList header)
{
    ...
    foreach (Column *col, quarantine) {
        ...
        // setting column name while col is still part of table triggers
        // renaming to prevent name clashes
...
{
    QList<Column *> quarantine;
    for (int i = 0; i < header.count() && i < d_future_table->columnCount();
         i++)
        quarantine << column(i);
    int i = 0;
    foreach (Column *col, quarantine) {
...
void Table::applyFormula()
{
    ...
    for (int col = firstSelectedColumn(); col <= lastSelectedColumn(); col++) {
        Column *col_ptr = column(col);
        col_ptr->insertRows(col_ptr->rowCount(),
                            rowCnt() - col_ptr->rowCount());
...
{
    ui.add_function_combobox->setToolTip(scriptEnv->mathFunctionDoc(
            ui.add_function_combobox->currentText()));
}
...
void Table::handleAspectDescriptionChange(const AbstractAspect *aspect)
{
    ...
    if (col && d_future_table->columnIndex(col) != -1
        ...
        for (int i = 0; i < d_future_table->columnCount(); i++) {
            QList<Interval<int>> formula_intervals =
                    column(i)->formulaIntervals();
            foreach (Interval<int> interval, formula_intervals) {
                QString formula = column(i)->formula(interval.start());
                if (formula.contains("\"" + old_name + "\"")) {
                    formula.replace("\"" + old_name + "\"",
                                    "\"" + new_name + "\"");
                        ...
                        bool simplifySpaces, bool newTable)
{
    ...
    if (file.open(QIODevice::ReadOnly)) {
        ...
        for (int i = overwritten_cols; i < temp->columnCount(); i++) {
            ...
            Column *new_col =
                    new Column(temp->column(i)->name(), AlphaPlot::Numeric);
...
void Table::setCell()
{
    if (context()->argumentCount() != 3 || !context()->argument(0).isNumber()
        ...
        context()->throwError(tr("setCell(intiger, intiger, double) take three "
                                 "arguments!"));
    } else if (context()->argument(0).toNumber() <= 0
               || context()->argument(1).toNumber() <= 0) {
        context()->throwError(tr("row/col index cannot be 0 or -ve!"));
