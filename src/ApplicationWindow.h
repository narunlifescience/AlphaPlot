...
public:
    ...
    even if 'name' is not used in the project, by setting /param increment =
    true (the default)*/
...
public slots:
     ...
     * set by the user) and inserted as a table into a new, empty project.
    ...
    QList<QMdiSubWindow *>
    subWindowsListFromTreeRecursive(QList<QMdiSubWindow *> list,
    ...
    //! \name Surface Plots
    //@{
    Layout3D *newPlot3D(const QString &formula, const double xl,
    ...
    generateFunctiondata(const int type, const QStringList &formulas,
                         const QString &var, const QList<double> &ranges,
                         const int points);
    QList<QPair<QPair<double, double>, double>> *
    generateFunction3ddata(const Graph3DCommon::Function3DData &funcdata);
    ...
    /**
     * \brief Create a Table which is initially hidden; used to return the
    ...
    void exportASCII(const QString &tableName, const QString &sep,
    ...
    columnList(AlphaPlot::PlotDesignation plotType);
    QList<QPair<Table *, Column *>>
    columnList(Folder *folder, AlphaPlot::PlotDesignation plotType);
    ...
    QList<QPair<QPair<double, double>, double>> *
    generateFunctiondata(const QString &formula, double xl, double xr,
     ...
     * \param fromFolders: true means that the user clicked right mouse buttom
     *on an item from QListView "folders" false means that the user clicked
     *right
     *mouse
     *buttom
    ...
    // void dragFolderItems(QList<Q3ListViewItem*> items) { draggedItems =
    // items;
    // }
...
private:
    ...
    // Manages connection between 2dplot actions (not used by all 2dplot
    // actions).
