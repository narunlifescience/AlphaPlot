                                             ...
                                             double columnIndex)
{
    ...
    if (!targetTable)
        throw mu::Parser::exception_type(qPrintable(
                tr("Couldn't find a table named %1.").arg(tableName)));
    Column *column =
            targetTable->d_future_table->column(qRound(columnIndex) - 1);
    if (!column)
        throw mu::Parser::exception_type(
                qPrintable(tr("There's no column %1 in table %2!")
                                   .arg(qRound(columnIndex))
...
Column *MuParserScript::resolveColumnPath(const QString &path)
{
    ...
    // would require
    // lookbehind assertions, which are currently not supported by QRegExp.
    // Thus, we can't simply use QString::split() and have to explicitly loop
    // over the characters in path.
    ...
    if (pathComponents.isEmpty()) {
        ...
        if (!table)
            throw mu::Parser::exception_type(
                    qPrintable(tr("Accessing table values is not (yet) "
        ...
        if (pathComponents.isEmpty())
            // needed for backwards compatibility, but will be problematic once
            // we drop the requirement of project-wide unique object names
        ...
        if (!table)
            throw mu::Parser::exception_type(qPrintable(
                    tr("Couldn't find a table named %1.")
...
bool MuParserScript::translateLegacyFunctions(QString &input)
{
    ...
    while (functionStart != -1) {
        ...
        if (legacyFunction.cap(2) == "col") {
            ...
            if (arguments.at(0).startsWith("\"")) {
                ...
                columnArgument.replace("/", "\\/");
                       ...
                       && table->d_future_table->column(arguments.at(0),
                                                        false)) {
                // hack for ambiguous legacy syntax:
                // col(name) -> column("name"), if name is a column of the
                // current table
                columnArgument = "\"" + arguments.at(0) + "\"";
                // do escaping of path argument
                columnArgument.replace("\\", "\\\\");
            ...
            if (arguments.size() != 2) {
                emit_error(tr("tablecol: wrong number of arguments (need 2, "
                              "got %1)")
                                   .arg(arguments.size()),
                           0);
            ...
            if (arguments.at(1).startsWith("\"")) {
                ...
                replacement = QString("column(") + tableArgument + "/"
                        + columnArgument + ")";
            } else {
                ...
                if (!translateLegacyFunctions(rowArgument))
                    return false;
                replacement = QString("column__(") + arguments.at(0) + ","
                        + rowArgument + ")";
            }
            ...
            if (arguments.size() != 2) {
                emit_error(
                        tr("cell: wrong number of arguments (need 2, got %1)")
                                .arg(arguments.size()),
                        0);
                compiled = Script::compileErr;
                return false;
            ...
            if (arguments.at(0).startsWith("\"")) {
                ...
                if (!translateLegacyFunctions(rowArgument))
                    return false;
                replacement = QString("cell(") + arguments.at(0) + ","
                        + rowArgument + ")";
            } else {
                ...
                if (!translateLegacyFunctions(rowArgument))
                    return false;
                replacement = QString("cell_(") + columnArgument + ","
                        + rowArgument + ")";
            }
        }
        ...
        if (legacyFunction.cap(1).isEmpty())
            // matched with ^, not \W (lookbehind assertion would be darn
            // handy...)
        ...
        functionStart = legacyFunction.indexIn(
                input, functionStart + replacement.length());
    } // while (functionStart != -1)
    return true;
}
