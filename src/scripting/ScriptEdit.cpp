...
void ScriptEdit::contextMenuEvent(QContextMenuEvent *e)
{
    ...
    if (parent()->inherits("Note")) {
        ...
        connect(actionAutoexec, SIGNAL(toggled(bool)), sp,
                SLOT(setAutoexec(bool)));
    ...
    for (int i = 0; i < flist.size(); i++) {
        QAction *newAction;
        QString menupart;
        // group by text before "_" (would make sense if we renamed several
        // functions...)
        /*if (flist[i].contains("_") || (i<flist.size()-1 &&
        flist[i+1].split("_")[0]==flist[i]))
...
{
    if (lineNumber(printCursor.position())
        == lineNumber(textCursor().selectionEnd()))
...
int ScriptEdit::lineNumber(int pos) const
{
    ...
    for (QTextBlock i = document()->begin();
         !i.contains(pos) && i != document()->end(); i = i.next())
        n++;
    return n;
}
...
void ScriptEdit::execute()
{
    ...
    if (codeCursor.selectedText().isEmpty()) {
        codeCursor.movePosition(QTextCursor::StartOfLine,
                                QTextCursor::MoveAnchor);
        codeCursor.movePosition(QTextCursor::EndOfLine,
    ...
    myScript->setCode(
            codeCursor.selectedText().replace(QChar::ParagraphSeparator, "\n"));
