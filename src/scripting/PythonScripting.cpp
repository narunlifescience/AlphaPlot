...
PyObject *PythonScripting::eval(const QString &code, PyObject *argDict,
                                const char *name)
{
    PyObject *args;
    if (argDict)
    ...
    PyObject *ret = NULL;
    PyObject *co =
            Py_CompileString(code.toAscii().constData(), name, Py_eval_input);
    if (co) {
...
bool PythonScripting::exec(const QString &code, PyObject *argDict,
                           const char *name)
{
    PyObject *args;
    if (argDict)
    ...
    PyObject *tmp = NULL;
    PyObject *co =
            Py_CompileString(code.toAscii().constData(), name, Py_file_input);
    if (co) {
...
QString PythonScripting::errorMsg()
{
    ...
    if (PyErr_GivenExceptionMatches(exception, PyExc_SyntaxError)) {
        ...
        msg.append("at ").append(
                toString(PyObject_GetAttrString(value, "filename"), true));
        msg.append(":").append(
                toString(PyObject_GetAttrString(value, "lineno"), true));
        msg.append("\n");
        Py_DECREF(exception);
        Py_DECREF(value);
    } else {
        msg.append(toString(exception, true))
                .remove("exceptions.")
                .append(": ");
    ...
    if (scidavismod) {
        ...
        if (!setQObject(d_parent, "app", scidavisDict))
            QMessageBox::warning(
                    ...
                    tr("Accessing SciDAVis functions or objects from Python "
                       "code won't work."
                       "Probably your version of SIP differs from the one "
                       "SciDAVis was compiled "
                       "against;"
...
#endif
    if (!initialized)
        initialized = loadInitFile(QDir::rootDirPath() + "etc/scidavisrc");
    if (!initialized)
        initialized =
                loadInitFile(QCoreApplication::instance()->applicationDirPath()
                             + "/scidavisrc");
    if (!initialized)
...
bool PythonScripting::loadInitFile(const QString &path)
{
    ...
    bool success = false;
    if (pycFile.isReadable()
        && (pycFile.lastModified() >= pyFile.lastModified())) {
        ...
        if (compileModule) {
            PyObject *compile = PyDict_GetItemString(
                    PyModule_GetDict(compileModule), "compile");
            if (compile) {
                PyObject *tmp = PyObject_CallFunctionObjArgs(
                        compile, PyString_FromString(pyFile.filePath()),
                        PyString_FromString(pycFile.filePath()), NULL);
                if (tmp)
                    Py_DECREF(tmp);
                else
                    PyErr_Print();
