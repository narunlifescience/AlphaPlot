...
PythonScript::PythonScript(PythonScripting *env, const QString &code,
                           QObject *context, const QString &name)
    : Script(env, code, context, name)
{
    PyCode = NULL;
    // Old: All scripts share a global namespace, and module top-level has its
    // own nonstandard local namespace
    modLocalDict = PyDict_New();
    // A bit of a hack, but we need either IndexError or len() from
    // __builtins__.
    PyDict_SetItemString(
            modLocalDict, "__builtins__",
            PyDict_GetItemString(this->env()->globalDict(), "__builtins__"));
    // New: Each script gets its own copy of the global namespace.
    // it is passed as both globals and locals, so top-level assignments are
    ...
    if (ret)
        ...
        PyErr_Print();
    // "self" is unique to each script, so they can't all run in the __main__
    // namespace
...
bool PythonScript::compile(bool for_eval)
{
    ...
    if (Context->inherits("Table")) {
        PyObject *ret = PyRun_String(
                "def col(c,*arg):\n"
                "\ttry: return self.column(type(c)==str and c or "
                "c-1).valueAt(arg[0]-1)\n"
                "\texcept(IndexError): return self.column(type(c)==str and c "
                "or c-1).valueAt(i-1)\n"
                "def cell(c,r):\n"
                "\treturn self.cell(c,r)\n"
                "def tablecol(t,c):\n"
                "\treturn "
                "self.folder().rootFolder().table(t,True).column(type(c)==str "
                "and c or "
                "c-1).valuetAt(i-1)\n"
    ...
    if (PyCode) {
        ...
        // PyCode. See
        // http://mail.python.org/pipermail/python-bugs-list/2001-June/005534.html
        // and
        // http://mail.python.org/pipermail/python-list/2001-June/087605.html
        ...
        if (PyCode) {
            ...
            Py_XDECREF(PyEval_EvalCode((PyCodeObject *)PyCode, topLevelLocal,
                                       tmp));
            Py_DECREF(PyCode);
        ...
        // a return value.
        PyErr_Clear(); // silently ignore errors
        PyCode = Py_CompileString(Code.toAscii().constData(), Name,
                                  Py_file_input);
        success = PyCode != NULL;
    }
    if (!success) {
...
QVariant PythonScript::eval()
{
    ...
    if (compiled != isCompiled && !compile(true))
        return QVariant();
    PyObject *topLevelGlobal =
            hasOldGlobals ? env()->globalDict() : modGlobalDict;
    PyObject *topLevelLocal = hasOldGlobals ? modLocalDict : modGlobalDict;
    PyObject *pyret;
    beginStdoutRedirect();
    if (PyCallable_Check(PyCode)) {
        ...
        pyret = PyEval_EvalCode((PyCodeObject *)PyCode, topLevelGlobal,
                                topLevelLocal);
    endStdoutRedirect();
    if (!pyret) {
        emit_error(env()->errorMsg(), 0);
...
bool PythonScript::setInt(int val, const char *name)
{
    if (!PyDict_Contains(modLocalDict, PyString_FromString(name)))
        compiled = notCompiled;
    return (env()->setInt(val, name, modLocalDict)
            && env()->setInt(val, name, modGlobalDict));
}
