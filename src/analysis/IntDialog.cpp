...
void IntDialog::accept()
{
    ...
    if (!curvesList.contains(curve)) {
        QMessageBox::critical(this, tr("Warning"),
                              tr("The curve <b> %1 </b> doesn't exist anymore! "
                                 "Operation aborted!")
                                      .arg(curve));
        boxName->clear();
    ...
    if (from == "min") {
        ...
        try {
            ...
            if (start < minx) {
                QMessageBox::warning(app_, tr("Input error"),
                                     tr("Please give a number larger or equal "
                                        "to the minimum value of "
                                        "X, for the lower limit.\n If you do "
                                        "not know that value, type "
                                        "min in the box."));
                boxStart->clear();
                boxStart->setFocus();
                return;
            ...
            if (start > maxx) {
                QMessageBox::warning(app_, tr("Input error"),
                                     tr("Please give a number smaller or equal "
    ...
    if (end == "min") {
        ...
        try {
            ...
            if (stop > maxx) {
                ...
                // FunctionCurves!!(Ion)
                /*QMessageBox::warning((ApplicationWindow *)parent(), tr("Input
                error"), tr("Please give a number smaller or equal to the
                maximum value of X, for the upper limit.\n If you do not know
                that value, type max in the box.")); boxEnd->clear();
                boxEnd->setFocus();
                return;
                */
                boxEnd->setText(QString::number(maxx));
    ...
    Integration *i = new Integration(app_, axisrect_, associateddata,
                                     boxStart->text().toDouble(),
                                     boxEnd->text().toDouble());
    i->setMethod(static_cast<Integration::InterpolationMethod>(
...
void IntDialog::activateCurve(const QString &curveName)
{
    ...
    boxStart->setText(QString::number(std::min(xmin_, xmax_), 'g',
                                      app_->d_decimal_digits));
    boxEnd->setText(QString::number(std::max(xmin_, xmax_), 'g',
                                    app_->d_decimal_digits));
}
