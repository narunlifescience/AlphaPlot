...
void MultiPeakFit::setNumPeaks(int n)
{
    ...
    if (d_peaks > 1)
        d_explanation +=
                "(" + QString::number(d_peaks) + ") " + tr("multi-peak");
    ...
    covar = gsl_matrix_alloc(static_cast<size_t>(d_p),
...
QString MultiPeakFit::peakFormula(int peakIndex, PeakProfile profile)
{
    ...
    case Gauss:
        ...
        break;
    case Lorentz:
        formula += "2*A" + index + "/PI*w" + index + "/(4*(x-xc" + index
                + ")^2+w" + index + "^2)";
        break;
    }
    return formula;
...
void MultiPeakFit::generateFitCurve(double *par)
{
    ...
    if (d_gen_function) {
        ...
        for (i = 0; i < d_points; i++) {
            ...
            for (j = 0; j < d_peaks; j++) {
                ...
                yi += y_aux;
                y_aux += par[d_p - 1];
                gsl_matrix_set(m, static_cast<size_t>(i),
                               static_cast<size_t>(j), y_aux);
            }
        ...
        if (generate_peak_curves) {
            for (i = 0; i < peaks_aux; i++) { // add the peak curves
                for (j = 0; j < d_points; j++)
                    Y[j] = gsl_matrix_get(m, static_cast<size_t>(j),
  ...
  label = tableName + "_2";
  Curve2D *curve =
  axisrect_->addCurve2DPlot(AxisRect2D::LineScatterType::Line2D, t, ) DataCurve
  *c = new DataCurve(t, tableName + "_" + columns.at(0)->name(), label); if
  (d_peaks > 1) c->setPen(QPen(ColorBox::color(d_curveColorIndex), 2)); else
                                 ...
                                 const QString &plotName)
{
    ...
    for (int j = 0; j < d_peaks; j++) {
        ...
        if (d_profile == Lorentz)
            info += QLocale().toString(M_2_PI * par[3 * j] / par[3 * j + 2],
                                       'g', d_prec)
                    + "\n";
        else
            info += QLocale().toString(sqrt(M_2_PI) * par[3 * j]
                                               / par[3 * j + 2],
                                       'g', d_prec)
                    + "\n";
    }
