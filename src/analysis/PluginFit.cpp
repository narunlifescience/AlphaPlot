...
bool PluginFit::load(const QString &pluginName)
{
    if (!QFile::exists(pluginName)) {
        QMessageBox::critical(app_, tr("File not found"),
                              tr("Plugin file: <p><b> %1 </b> <p>not found. "
                                 "Operation aborted!")
                                      .arg(pluginName));
        return false;
    }
    ...
    if (fitFunction) {
        d_param_names =
                QString(fitFunction()).split(",", QString::SkipEmptyParts);
    ...
    if (fitExplain)
        d_param_explain =
                QString(fitExplain()).split(",", QString::SkipEmptyParts);
    else
        for (int i = 0; i < d_p; i++)
            d_param_explain << "";
