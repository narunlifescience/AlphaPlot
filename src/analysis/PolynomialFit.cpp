...
void PolynomialFit::fit()
{
    ...
    gsl_matrix *X = gsl_matrix_alloc(static_cast<size_t>(d_n),
                                     static_cast<size_t>(d_p));
...
void LinearFit::fit()
{
    ...
    if (d_y_error_source == UnknownErrors)
        gsl_fit_linear(d_x, 1, d_y, 1, static_cast<size_t>(d_n), &c0, &c1,
                       &cov00, &cov01, &cov11, &chi_2);
    else
        gsl_fit_wlinear(d_x, 1, weights, 1, d_y, 1, static_cast<size_t>(d_n),
                        &c0, &c1, &cov00, &cov01, &cov11, &chi_2);
