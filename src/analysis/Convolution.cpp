                                   ...
                                   const QString &responseColName)
{
    ...
    if (signal_col < 0) {
        QMessageBox::warning(qobject_cast<ApplicationWindow *>(parent()),
                             ...
                             tr("The signal data set %1 does not exist!")
                                     .arg(signalColName));
        d_init_err = true;
        return;
        ...
        QMessageBox::warning(qobject_cast<ApplicationWindow *>(parent()),
                             tr("AlphaPlot") + " - " + tr("Error"),
                             tr("The response data set %1 does not exist!")
                                     .arg(responseColName));
        d_init_err = true;
        return;
    }
    ...
    if (d_n_response >= rows / 2) {
        ...
        QMessageBox::warning(qobject_cast<ApplicationWindow *>(parent()),
                             tr("AlphaPlot") + " - " + tr("Error"),
                             tr("The response dataset '%1' must contain an odd "
                                "number of points!")
                                     .arg(responseColName));
        d_init_err = true;
        return;
    ...
    if (d_y && d_x) {
        memset(d_x, 0,
               d_n_signal * sizeof(double)); // zero-pad signal data array
        for (int i = 0; i < d_n; i++)
