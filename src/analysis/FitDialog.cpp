...
void FitDialog::initFitPage()
{
    ...
    boxParams->horizontalHeader()->setSectionResizeMode(1,
                                                        QHeaderView::Stretch);
    boxParams->horizontalHeader()->setSectionResizeMode(
    ...
    connect(btnDeleteFitCurves, SIGNAL(clicked()), this,
            SLOT(deleteFitCurves()));
...
void FitDialog::initEditPage()
{
    ...
    connect(buttonPlugins, SIGNAL(clicked()), this,
            SLOT(choosePluginsFolder()));
    ...
    connect(funcBox, SIGNAL(currentRowChanged(int)), this,
            SLOT(showExpression(int)));
    connect(boxUseBuiltIn, SIGNAL(toggled(bool)), this,
            SLOT(setFunction(bool)));
...
void FitDialog::initAdvancedPage()
{
    ...
    connect(btnParamTable, SIGNAL(clicked()), this,
            SLOT(showParametersTable()));
    connect(btnCovMatrix, SIGNAL(clicked()), this,
        ...
        QMessageBox::critical(this, tr("Error: function name"),
                              "<p><b>" + boxName->text() + "</b>"
                                      + tr(" is a built-in function name"
                                           "<p>You must choose another name "
                                           "for your function!"));
        editBox->setFocus();
        return;
    }
...
void FitDialog::showFitPage()
{
    ...
    for (int i = param_table_rows; i < paramList.count(); i++) {
        ...
        /* It was !Qt::ItemIsEditable, Did the original author want to enable
           all flags except ItemIsEditable which is
    ...
    if (ok) {
        ...
        if (categoryBox->currentRow() == 0 && d_user_function_params.size() > 0)
            ...
            case 5:
                ...
                break;
            case 6:
                lst = MultiPeakFit::generateParameterList(
            ...
            case 7:
                ...
                break;
            case 8:
                lst = PolynomialFit::generateParameterList(
                        polynomOrderBox->value());
                break;
            }
...
void FitDialog::accept()
{
    ...
    if (!boxParams->isColumnHidden(2)) {
        for (i = 0; i < rows; i++) { // count the non-constant parameters
            QCheckBox *cb =
                    qobject_cast<QCheckBox *>(boxParams->cellWidget(i, 2));
            if (!cb->isChecked())
                n++;
        }
    } else
    ...
    if (!boxParams->isColumnHidden(2)) {
        ...
        for (i = 0; i < rows; i++) {
            ...
            if (!cb->isChecked()) {
                paramsInit[j] =
                        QLocale().toDouble(boxParams->item(i, 1)->text());
    ...
    if (!boxParams->isColumnHidden(2)) {
        ...
        for (i = 0; i < rows; i++) {
            QCheckBox *cb =
                    qobject_cast<QCheckBox *>(boxParams->cellWidget(i, 2));
            if (!cb->isChecked())
                boxParams->item(i, 1)->setText(QLocale().toString(
                        res[j++], 'g', boxPrecision->value()));
        }
    } else {
        for (i = 0; i < rows; i++)
...
bool FitDialog::validInitialValues()
{
    for (int i = 0; i < boxParams->rowCount(); i++) {
        ...
        try {
            MyParser parser;
            parser.SetExpr(
                    CONFS(boxParams->item(i, 1)->text()).toUtf8().constData());
            parser.Eval();
        } catch (mu::ParserError &e) {
