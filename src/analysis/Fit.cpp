...
double *Fit::fitGslMultifit(int &iterations, int &status)
{
    ...
    if (d_y_error_source == UnknownErrors) {
        // multiply covar by variance of residuals, which is used as an estimate
        // for the statistical errors (this relies on the Y errors being set
        // to 1.0, so that s->f is properly normalized)
...
double *Fit::fitGslMultimin(int &iterations, int &status)
{
    ...
    chi_2 = s_min->fval;
    gsl_matrix *J = gsl_matrix_alloc(static_cast<size_t>(d_n),
                                     static_cast<size_t>(d_p));
    ...
    for (int i = 0; i < d_p; i++) {
        ...
        if (d_scale_errors)
            info += QLocale().toString(
                            sqrt(chi_2_dof
                                 * gsl_matrix_get(covar, static_cast<size_t>(i),
          ...
          if (t->numRows() < d_n) {
              if (!fail_silently)
                  QMessageBox::critical(app_, tr("Error"),
                                        tr("The column %1 has less points than "
                                           "the fitted data set. "
...
Table *Fit::parametersTable(const QString &tableName)
{
    ...
    t->setHeader(QStringList()
                 << tr("Parameter") << tr("Value") << tr("Error"));
    ...
    if (!d_result_errors) {
        ...
        for (int i = 0; i < d_p; i++) {
            if (d_scale_errors)
                ...
                d_result_errors[i] = sqrt(gsl_matrix_get(
                        covar, static_cast<size_t>(i), static_cast<size_t>(i)));
        }
    }
...
void Fit::fit()
{
    ...
    if (d_formula.isEmpty()) {
        QMessageBox::critical(app_, tr("Fit Error"),
                              tr("You must specify a valid fit function first. "
                                 "Operation aborted!"));
        return;
    }
...
int Fit::evaluate_f(const gsl_vector *x, gsl_vector *f)
{
    ...
    for (int j = 0; j < d_n; j++) {
        ...
        bool success;
        gsl_vector_set(f, static_cast<size_t>(j),
                       (d_script->eval().toDouble(&success) - d_y[j])
                               / d_y_errors[j]);
        if (!success)
            return GSL_EINVAL;
    }
...
double Fit::evaluate_d(const gsl_vector *x)
{
    ...
    for (int j = 0; j < d_n; j++) {
        d_script->setDouble(d_x[j], "x");
        bool success;
        result += pow((d_script->eval().toDouble(&success) - d_y[j])
                              / d_y_errors[j],
                      2);
        if (!success)
            return GSL_EINVAL;
...
int Fit::evaluate_df(const gsl_vector *x, gsl_matrix *J)
{
    ...
    for (int i = 0; i < d_n; i++) {
        ...
        for (int j = 0; j < d_p; j++) {
            ...
            gsl_deriv_central(&F, gsl_vector_get(x, static_cast<size_t>(j)),
                              1e-8, &result, &abserr);
            if (!data.success)
                return GSL_EINVAL;
