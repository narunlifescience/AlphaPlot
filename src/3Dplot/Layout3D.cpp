      ...
      custominter_(new Custom3DInteractions)
{
    ...
    // set animations
    m_animationCameraX_ = new QPropertyAnimation(
            graph_->scene()->activeCamera(), "xRotation");
    ...
    upAnimation_ = new QPropertyAnimation(graph_->scene()->activeCamera(),
                                          "yRotation");
    ...
    downAnimation_ = new QPropertyAnimation(graph_->scene()->activeCamera(),
                                            "yRotation");
...
QAbstract3DGraph *Layout3D::getAbstractGraph() const
{
    return graph_;
}
...
QSize Layout3D::getContainerSize() const
{
    return main_widget_->size();
}

Graph3DCommon::Plot3DType Layout3D::getPlotType() const
{
    return plottype_;
}
...
void Layout3D::save(XmlStreamWriter *xmlwriter)
{
    ...
    xmlwriter->writeAttribute("color",
                              theme->lightColor().name(QColor::HexArgb));
    ...
    xmlwriter->writeAttribute(
            "backgroundcolor",
            theme->labelBackgroundColor().name(QColor::HexArgb));
