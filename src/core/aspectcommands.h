 ...
 *                                                                         *
 ***************************************************************************/
#ifndef ASPECT_COMMANDS
#define ASPECT_COMMANDS
...
{
public:
    AspectNameChangeCmd(AbstractAspect::Private *target,
                        const QString &new_name)
        : d_target(target), d_other_name(new_name)
    {
        setText(QObject::tr("%1: rename to %2")
                        .arg(d_target->name())
                        .arg(new_name));
    }
...
{
public:
    AspectCommentChangeCmd(AbstractAspect::Private *target,
...
{
public:
    AspectCaptionSpecChangeCmd(AbstractAspect::Private *target,
...
{
public:
    AspectCreationTimeChangeCmd(AbstractAspect::Private *target,
...
{
public:
    AspectChildAddCmd(AbstractAspect::Private *target, AbstractAspect *child,
                      int index)
        : AspectChildRemoveCmd(target, child, false)
    {
        setText(QObject::tr("%1: add %2")
                        .arg(d_target->name())
                        .arg(d_child->name()));
        d_index = index;
    }
...
public:
    ...
    virtual void redo()
    {
        // Moving in one go confuses QTreeView, so we would need another two
        // signals to be mapped to QAbstractItemModel::layoutAboutToBeChanged()
        // and
        // ::layoutChanged().
...
{
public:
    AspectChildReparentCmd(AbstractAspect::Private *target,
    ...
    // calling undo transfers ownership of d_child back to its previous parent
    // aspect
    virtual void undo()
    {
        Q_ASSERT(d_index != -1);
