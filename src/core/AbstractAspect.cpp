...
bool AbstractAspect::readBasicAttributes(XmlStreamReader *reader)
{
    ...
    if (str.isEmpty()) {
        reader->raiseWarning(prefix + tr("aspect name missing or empty")
                             + postfix);
    }
    setName(str);
    // read creation time
                                   ...
                                   AbstractAspect *child, int new_index)
{
    ...
    Q_ASSERT(new_parent != NULL);
    QString new_name =
            new_parent->d_aspect_private->uniqueNameFor(child->name());
...
void AbstractAspect::setName(const QString &value)
{
    ...
    // FIXME: once the project file format is fully XML-based (i.e. able to
    // escape special characters), this can be removed
    QString sanitized_value = value;
    sanitized_value.remove(QChar('\n'));
    sanitized_value.remove(QChar('\r'));
    sanitized_value.remove(QChar('\t'));
    if (sanitized_value != value)
        info(tr("Tabs and line breaks in object names are currently not "
                "supported. "
    ...
    if (d_aspect_private->parent()) {
        ...
        if (new_name != sanitized_value)
            info(tr("Intended name \"%1\" diverted to \"%2\" in order to avoid "
                    "name "
                    "collision.")
                         .arg(sanitized_value)
                         .arg(new_name));
...
QList<AbstractAspect *>
AbstractAspect::descendantsThatInherit(const char *class_name)
{
