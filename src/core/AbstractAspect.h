 ...
 *
 * Aspects organize themselves into trees, where a parent takes ownership of its
 * children. Usually,
 * though not necessarily, a Project instance will sit at the root of the tree
 * (without a Project
 * ancestor, project() will return 0 and undo does not work). Children are
 * organized using
 * addChild(), removeChild(), child(), indexOfChild() and childCount() on the
 * parent's side as well
 * as the equivalent convenience methods index() and remove() on the child's
 * side.
 *
 * AbstractAspect manages for every Aspect the properties #name, #comment,
 * #caption_spec and
 * #creation_time. All of these translate into the caption() as described in the
 * documentation
 * of setCaptionSpec().
 *
 * If an undoStack() can be found (usually it is managed by Project), changes to
 * the properties
 * as well as adding/removing children support multi-level undo/redo. In order
 * to support undo/redo
 * for problem-specific data in derived classes, make sure that all changes to
 * your data are done
 * by handing appropriate commands to exec().
 *
 * Optionally,
 * you can supply an icon() to be used by different views (including the
 * ProjectExplorer)
 * and/or reimplement createContextMenu() for a custom context menu of views.
 *
 * The private data of AbstractAspect is contained in a separate class
 * AbstractAspect::Private.
 * The write access to AbstractAspect::Private should always be done using
 * aspect commands
 * to allow undo/redo.
 */
...
public:
    ...
    int index() const
    {
        return parentAspect() ? parentAspect()->indexOfChild(this) : 0;
    }
    ...
    //! \name undo related
    //@{
    //! Return the undo stack of the Project, or 0 if this Aspect is not part of a
    //! Project.
    /**
     * It's also possible to construct undo-enabled Aspect trees without
     * Project. The only requirement is that the root Aspect reimplements
     * undoStack() to get the undo stack from somewhere (the default
     * implementation just delegates to parentAspect()).
     */
...
public slots:
    ...
    /**
     * The caption is constructed by substituting some magic tokens in the
     * specification:
     *
     * - \%n - name()
     * - \%c - comment()
     * - \%t - creationTime()
     *
     * Additionally, you can use the construct %C{<text>}. <text> is only shown
     * in the caption if comment() is not empty (name and creation time should
     * never be empty).
     *
    ...
    //! Make the specified name unique among my children by incrementing a
    //! trailing number.
...
protected:
    ...
    /**
     * Unlike the aspectAdded() signals, this method does not get called inside
     * undo/redo actions;
    ...
    /**
     * Unlike the aspectAboutToBeRemoved() signals, this method does not get
     * called inside undo/redo actions;
     * allowing subclasses to execute undo commands of their own.
     */
    ...
    //! Implementations should call this whenever status information should be
    //! given to the user.
    /**
     * This will cause statusInfo() to be emitted. Typically, this will cause
     * the specified string to be displayed in a status bar, a log window or
     * some similar non-blocking way so as not to disturb the workflow.
     */
