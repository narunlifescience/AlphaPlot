...
private:
    ...
    /**
     * Depending on whether an Aspect is removed programatically or by closing
     * its default view (held by me), either aspectAboutToBeRemoved() generates
     * a close event for me or closeEvent() removes #d_part from its parent.
     * Before one causes the other to be called, #d_closing is set to true so as
