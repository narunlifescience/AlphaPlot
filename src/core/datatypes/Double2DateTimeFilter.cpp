...
QDateTime Double2DateTimeFilter::dateTimeAt(int row) const
{
    ...
    QDateTime result;
    // This gets a little messy, since QDate can't represent the reference date
    // of JDN. Thus, we manually interpret invalid dates as JDN reference.
    ...
    case Day:
        ...
        return result.addMSecs(
                qRound64((input_value - int(floor(input_value))) * 86400000.0));
    case Hour:
        if (m_date_time_0.isValid())
            ...
            return result.addMSecs(qRound64(
                    (input_value - int(floor(input_value))) * 3600000.0));
        }
    case Minute:
        if (m_date_time_0.isValid())
            ...
            result.setDate(
                    QDate::fromJulianDay(int(floor(input_value)) / (24 * 60)));
            result.setTime(QTime(12, 0, 0, 0));
            result = result.addSecs((int(floor(input_value)) % (24 * 60)) * 60);
            return result.addMSecs(qRound64(
                    (input_value - int(floor(input_value))) * 60000.0));
        }
    case Second:
        if (m_date_time_0.isValid())
            ...
            result = result.addSecs(int(floor(input_value)) % (24 * 60 * 60));
            return result.addMSecs(
                    qRound64((input_value - int(floor(input_value))) * 1000.0));
        }
    case Millisecond:
        if (m_date_time_0.isValid())
