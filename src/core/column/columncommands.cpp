...
void ColumnFullCopyCmd::redo()
{
    if (d_backup == 0) {
        ...
        d_col->replaceData(d_backup->dataPointer(),
                           d_backup->validityAttribute());
...
void ColumnPartialCopyCmd::redo()
{
    if (d_src_backup == 0) {
        ...
        d_src_backup =
                new Column::Private(d_src_backup_owner, d_col->columnMode());
        d_src_backup->copy(d_src, d_src_start, 0, d_num_rows);
        d_col_backup_owner = new Column("temp", d_col->columnMode());
        d_col_backup =
                new Column::Private(d_col_backup_owner, d_col->columnMode());
...
ColumnSetPlotDesignationCmd::ColumnSetPlotDesignationCmd(
        Column::Private *col, AlphaPlot::PlotDesignation pd,
        QUndoCommand *parent)
