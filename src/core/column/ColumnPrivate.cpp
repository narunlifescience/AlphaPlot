    ...
    : d_owner(owner)
{
    ...
    case AlphaPlot::Numeric:
                         ...
                         // global setting method
        {
            ...
            static_cast<Double2StringFilter *>(d_output_filter)
                    ->setNumericFormat(
                            settings.value("DefaultNumericFormat", 'f')
                                    .toChar()
                                    .toLatin1());
        }
#endif
    ...
    case AlphaPlot::Month:
        ...
        d_output_filter = new DateTime2StringFilter();
        static_cast<DateTime2StringFilter *>(d_output_filter)
                ->setFormat("MMMM");
        connect(static_cast<DateTime2StringFilter *>(d_output_filter),
                SIGNAL(formatChanged()), d_owner, SLOT(notifyDisplayChange()));
        d_data_type = AlphaPlot::TypeDateTime;
                                    ...
                                    AbstractFilter *filter)
{
    ...
    // remark: the deletion of the old data will be done in the dtor of a
    // command
    ...
    case AlphaPlot::Numeric:
        disconnect(static_cast<Double2StringFilter *>(d_output_filter),
                   SIGNAL(formatChanged()), d_owner,
                   SLOT(notifyDisplayChange()));
        switch (mode) {
        case AlphaPlot::Numeric:
            break;
        case AlphaPlot::Text:
            ...
            temp_col = new Column("temp_col",
                                  *(static_cast<QVector<qreal> *>(old_data)),
                                  d_validity);
            d_data = new QStringList();
            d_data_type = AlphaPlot::TypeString;
            break;
    ...
    case AlphaPlot::Text:
        ...
        case AlphaPlot::Numeric:
            ...
            temp_col = new Column("temp_col",
                                  *(static_cast<QStringList *>(old_data)),
                                  d_validity);
            d_data = new QVector<double>();
            d_data_type = AlphaPlot::TypeDouble;
            break;
                           ...
                           int dest_start, int num_rows)
{
    ...
    // copy the validity information
    for (int i = 0; i < num_rows; i++)
        d_validity.setValue(dest_start + i,
                            source->isInvalid(source_start + i));
    ...
    if (before <= rowCount()) {
        ...
        case AlphaPlot::TypeMonth:
            for (int i = 0; i < count; i++)
                static_cast<QList<QDateTime> *>(d_data)->insert(before,
                                                                QDateTime());
            break;
        case AlphaPlot::TypeString:
            for (int i = 0; i < count; i++)
...
void Column::Private::removeRows(int first, int count)
{
    ...
    if (first < rowCount()) {
        ...
        switch (d_data_type) {
        case AlphaPlot::TypeDouble:
            static_cast<QVector<double> *>(d_data)->remove(first,
                                                           corrected_count);
            break;
        case AlphaPlot::TypeDateTime:
        case AlphaPlot::TypeDay:
...
{
    if (d_data_type != AlphaPlot::TypeString)
        return QString();
...
{
    if (d_data_type != AlphaPlot::TypeDateTime)
        return QDateTime();
...
{
    if (d_data_type != AlphaPlot::TypeDouble)
        return 0.0;
...
void Column::Private::replaceTexts(int first, const QStringList &new_values)
{
    ...
    for (int i = 0; i < num_rows; i++)
        static_cast<QStringList *>(d_data)->replace(first + i,
                                                    new_values.at(i));
