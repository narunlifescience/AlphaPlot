...
/**
 The writing interface defined here is only to be used by column commands and
 Column contructors.
*/
...
public:
    //! Ctor
    Private(Column *owner, AlphaPlot::ColumnMode mode);
    //! Dtor
    ~Private();
    //! Special ctor (to be called from Column only!)
    Private(Column *owner, AlphaPlot::ColumnDataType type,
            AlphaPlot::ColumnMode mode, void *data,
            IntervalAttribute<bool> validity);

    //! Return the data type of the column
    AlphaPlot::ColumnDataType dataType() const { return d_data_type; }
     ...
     * \param num_rows the number of rows to copy
     */
    bool copy(const Private *source, int source_start, int dest_start,
              int num_rows);
    ...
    /**
     * Replace column mode, data type, data pointer, validity and filters
     * directly
     */
...
private:
     ...
     */
    AlphaPlot::ColumnDataType d_data_type;
    //! The column mode
    /**
