...
void Column::initPrivate(std::unique_ptr<QStringList> d,
                         IntervalAttribute<bool> v)
{
    d_column_private = new Private(this, AlphaPlot::TypeString, AlphaPlot::Text,
                                   d.release(), v);
}
...
{
    if (!new_values.isEmpty())
        exec(new ColumnReplaceDateTimesCmd(d_column_private, first,
                                           new_values));
}
...
void Column::save(QXmlStreamWriter *writer) const
{
    ...
    writer->writeAttribute(
            "mode", AlphaPlot::enumValueToString(columnMode(), "ColumnMode"));
    writer->writeAttribute(
            "plot_designation",
            AlphaPlot::enumValueToString(plotDesignation(), "PlotDesignation"));
    ...
    case AlphaPlot::TypeDouble:
        for (i = 0; i < rowCount(); i++) {
            ...
            writer->writeAttribute(
                    "type",
                    AlphaPlot::enumValueToString(dataType(), "ColumnDataType"));
...
bool Column::load(XmlStreamReader *reader)
{
    if (reader->isStartElement() && reader->name() == "column") {
        ...
        // read type
        str = attribs.value(reader->namespaceUri().toString(), "type")
                      .toString();
        if (str.isEmpty()) {
            reader->raiseError(tr("column type missing"));
            return false;
        }
        int type_code = AlphaPlot::enumStringToValue(str, "ColumnDataType");
        if (type_code == -1) {
            reader->raiseError(tr("column type invalid"));
            return false;
        }
        // read mode
        str = attribs.value(reader->namespaceUri().toString(), "mode")
                      .toString();
        if (str.isEmpty()) {
            reader->raiseError(tr("column mode missing"));
            return false;
        }
        int mode_code = AlphaPlot::enumStringToValue(str, "ColumnMode");
        if (mode_code == -1) {
        ...
        str = attribs.value(reader->namespaceUri().toString(),
                            "plot_designation")
                      .toString();
        int pd_code = AlphaPlot::enumStringToValue(str, "PlotDesignation");
        if (str.isEmpty())
...
bool Column::XmlReadRow(XmlStreamReader *reader)
{
    ...
    type_code = AlphaPlot::enumStringToValue(str, "ColumnDataType");
    ...
    str = attribs.value(reader->namespaceUri().toString(), "invalid")
                  .toString();
    if (str == "yes")
        setInvalid(index);
...
QString ColumnStringIO::textAt(int row) const
{
    if (d_setting)
        ...
        return d_owner->d_column_private->outputFilter()->output(0)->textAt(
                row);
}
...
void ColumnStringIO::setTextAt(int row, const QString &value)
{
    d_setting = true;
    d_to_set = value;
    d_owner->copy(d_owner->d_column_private->inputFilter()->output(0), 0, row,
                  1);
    d_setting = false;
    d_to_set.clear();
}
...
{
    if (other->columnMode() != AlphaPlot::Text)
        return false;
...
bool ColumnStringIO::copy(const AbstractColumn *source, int source_start,
                          int dest_start, int num_rows)
{
    if (source->columnMode() != AlphaPlot::Text)
        return false;
