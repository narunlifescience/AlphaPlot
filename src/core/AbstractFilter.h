 ...
 *
 * AbstractFilter provides an abstraction for analysis operations. It is
 * modelled on an
 * electronic filtering circuit: From the outside, a filter appears as a black
 * box with
 * a number of input and output ports (obviously, those numbers do not
 * necessarily agree).
 *
 ...
 * again by an AbstractColumn, which you can connect to as many other filters,
 * tables
 * or plots as you like.
 * Ownership of the data sources always stays with the class which is providing
 * the data,
 * that is, neither input() nor output() transfer ownership.
 *
 * Furthermore, you can use inputCount() and outputCount() to query the number
 * of
 * input and output ports, respectively and you can obtain label strings for
 * inputs (via
 * inputLabel()) and outputs (via AbstractColumn::label()). This allows generic
 * filter
 * handling routines to be written, which is important for using filters
 * provided by plugins.
 *
 * Its simplicity of use notwithstanding, AbstractFilter provides a powerful and
 * versatile
 * basis also for analysis operations that would not commonly be referred to as
 * "filter".
 ...
 * The main design goal was to make implementing new filters as easy as
 * possible.
 * Filters with only one output port can subclass AbstractSimpleFilter, which is
 * even easier
 ...
 * addition to data transfer between these classes the signals defined by
 * AbstractColumn
 * have to be handled on both inputs and outputs. Signals from data sources
 * connected to the input
 * ports are automatically connected to a matching set of virtual methods, which
 * can be
 * reimplemented by subclasses to handle these events.
 *
 * While AbstractFilter handles the tedious part of connecting a data source to
 * an input port,
 * its subclasses are given a chance to reject such connections (e.g., based on
 * the data type
