    ...
    : QDialog(parent), ui_(new Ui_RandomDistributionDialog)
{
    ...
    ui_->distComboBox->addItem(tr("Hypergeometric Distribution"),
    Hypergeometric);
    ...
    connect(ui_->randomDistributionButtonBox, SIGNAL(rejected()),
            SLOT(cancel()));
}

RandomDistributionDialog::~RandomDistributionDialog()
{
    delete ui_;
}
...
void RandomDistributionDialog::cancel()
{
    QDialog::reject();
}
