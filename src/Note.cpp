...
bool Note::load(XmlStreamReader *xmlreader)
{
    if (xmlreader->isStartElement() && xmlreader->name() == "note") {
        ...
        QString postfix(
                tr(" (non-critical)", "postfix for XML error messages"));

        QXmlStreamAttributes attribs = xmlreader->attributes();
        QString basicattr;

        // read name
        basicattr = attribs.value(xmlreader->namespaceUri().toString(), "name")
                            .toString();
        ...
        setName(basicattr);
        // read creation time
        basicattr = attribs.value(xmlreader->namespaceUri().toString(),
                                  "creation_time")
                            .toString();
        ...
        basicattr = attribs.value(xmlreader->namespaceUri().toString(),
                            ...
                            .toString();
        setCaptionPolicy(
                static_cast<MyWidget::CaptionPolicy>(basicattr.toInt()));
        basicattr =
                attribs.value(xmlreader->namespaceUri().toString(), "status")
                        .toString();
        ...
        while (!xmlreader->atEnd()) {
            ...
            if (xmlreader->isEndElement() && xmlreader->name() == "note")
                break;
            ...
            if (xmlreader->isStartElement()) {
                if (xmlreader->name() == "content") {
                    QXmlStreamAttributes attribs = xmlreader->attributes();
                    QString value =
                            attribs.value(xmlreader->namespaceUri().toString(),
                                          "value")
                                    .toString();
                    setText(value);
                } else {
                ...
                if (xmlreader->name() == "geometry") {
                    ...
                    QString valuey2 =
                            attribs.value(xmlreader->namespaceUri().toString(),
                                          "y2")
                                    .toString();
                    setGeometry(valuex1.toInt(), valuey1.toInt(),
                                valuex2.toInt(), valuey2.toInt());
                } else {
                    // unknown element
                    xmlreader->raiseWarning(
                            tr("unknown element '%1'")
