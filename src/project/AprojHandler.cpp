...
ApplicationWindow *AprojHandler::openproject(const QString &filename)
{
    ...
    /*if (!checkXmlSchema(filename)) {
      qDebug() << QString("unable to validate %1 using xml
    schema").arg(filename); delete file;
    ...
    " "untouched, in case you want to try " "rescuing data manually. If you want
    to " "continue working with the automatically " "restored backup copy, you
    have to " "explicitly overwrite the original file.")); return
    ...
    while (!xmlreader->atEnd()) {
        ...
        if (token == QXmlStreamReader::StartElement
            ...
            if (attributes.hasAttribute("scripting")) {
                if (!app->setScriptingLang(
                            attributes.value("scripting").toString(), true))
                    QMessageBox::warning(
                            app, tr("File opening error"),
                            tr("The file \"%1\" was created using \"%2\" as "
                               "scripting "
                               "language.\n\n Initializing support for this "
                               "language FAILED; "
                               "I'm using default instead.\nVarious parts of "
                               "this file may "
                               "not be displayed as expected.")
                                    .arg(filename)
                                    .arg(attributes.value("scripting")
                                                 .toString()));
            }
            ...
            QString lastmodified_time = xmlreader->attributes()
                                                .value("lastmodified_time")
                                                .toString();
            QString current =
                    xmlreader->attributes().value("current").toString();
            ...
            // (hack) for some unknown reason this connection need to be
            // manually set here
            ...
            if (ok) {
              if (ptype == "surface")
                ...
                xmlreader->raiseError(tr("Layout3D PlotType unknown
            %1").arg(ptype)); } else xmlreader->raiseError(tr("Layout3D PlotType
            missing or empty")); plot->load(xmlreader.get(), tables(app),
            matrixs(app));*/
        } else if (token == QXmlStreamReader::StartElement
                   && xmlreader->name() == "log") {
            QXmlStreamAttributes attributes = xmlreader->attributes();
            if (attributes.hasAttribute("value")) {
                ...
                xmlreader->raiseWarning(
                        tr("Invalid attribute '%1' log element. "
            ...
            while (recursivecount_ > 0) {
                Folder *fl =
                        static_cast<Folder *>(app->current_folder->parent());
                if (fl) {
...
QFile *AprojHandler::openCompressedFile(const QString &filename)
{
    ...
    if (!file || !file->open()) {
        gzclose(in);
        QMessageBox::critical(app_, tr("File opening error"),
                              tr("Can't create temporary file for writing "
                                 "uncompressed copy of %1.")
                                      .arg(filename));
        return nullptr;
    }
    ...
    forever {
        ...
        if (file->write(buf, len) != len) {
            QMessageBox::critical(app_, tr("File opening error"),
                                  tr("Error writing to temporary file: %1")
...
bool AprojHandler::saveproject(const QString &filename, Folder *folder)
{
    ...
    if (compress)
        file_compress(QFile::encodeName(fname).constData(), "wb9");
    file->close();
    return true;
}

void AprojHandler::saveTreeRecursive(Folder *folder, XmlStreamWriter *xmlwriter)
{
    ...
    foreach (MyWidget *subwindow, folder->windowsList()) {
        (qobject_cast<Layout2D *>(subwindow)
         || qobject_cast<Layout3D *>(subwindow))
                ? plots << subwindow
                : others << subwindow;
    }
    others.append(plots);
...
bool AprojHandler::checkXmlSchema(const QString &filename)
{
    ...
    if (!xsdfile->open(QIODevice::ReadOnly)) {
        qDebug() << QString("unable to open xml schema file %1")
                            .arg(xmlschemafile_);
        return false;
    }
