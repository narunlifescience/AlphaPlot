...
Matrix::Matrix(void *, int rows, int cols, const QString &name)
    : AbstractPart(name), d_plot_menu(nullptr), showonlyvalues_(false)
#endif
{
...
#else
Matrix::Matrix() : AbstractPart("temp"), showonlyvalues_(false)
#endif
{
    d_view = nullptr;
    createActions();
...
void Matrix::setDimensions(int rows, int cols)
{
    ...
    if (col_diff > 0)
        exec(new MatrixInsertColumnsCmd(d_matrix_private, columnCount(),
                                        col_diff));
    else if (col_diff < 0)
        exec(new MatrixRemoveColumnsCmd(d_matrix_private,
                                        columnCount() + col_diff, -col_diff));
    if (row_diff > 0)
    ...
    for (int r = 0; r < rows; r++) {
        for (int c = 0; c < cols; c++) {
            if (d_view->isCellSelected(first_row + r, first_col + c))
                output_str +=
                        QLocale().toString(cell(first_row + r, first_col + c),
...
void Matrix::createActions()
{
    ...
    action_clear_matrix = new QAction(
    ...
    actionManager()->addAction(action_mirror_horizontally,
                               "mirror_horizontally");
...
{
    WAIT_CURSOR;
    exec(new MatrixSetCoordinatesCmd(d_matrix_private, xStart(), xEnd(),
                                     yStart(), y));
    RESET_CURSOR;
}
...
void Matrix::showOnlyValues(const bool status)
{
    showonlyvalues_ = status;
}

bool Matrix::getShowOnlyValues() const
{
    return showonlyvalues_;
}
