...
// (maximum) initial matrix size (this is the size of the window, not the number
// of cells)
 ...
 */
#ifndef LEGACY_CODE_0_2_x
    Matrix(AbstractScriptingEngine *engine, int rows, int cols,
           const QString &name);
#else
    ...
    void setColumnCells(int col, int first_row, int last_row,
                        const QVector<qreal> &values);
    //! Return the values in the given cells as double vector
    QVector<qreal> rowCells(int row, int first_column, int last_column);
    //! Set the values in the given cells from a double vector
    void setRowCells(int row, int first_column, int last_column,
                     const QVector<qreal> &values);
...
private:
    ...
    Matrix();
    // TODO: the default sizes are to be controlled by the global Matrix
    // settings
...
private:
    ...
    QString d_formula; // TODO: should we support interval/rectangle based
                       // formulas?
