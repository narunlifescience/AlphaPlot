...
MatrixInsertColumnsCmd::MatrixInsertColumnsCmd(
        future::Matrix::Private *private_obj, int before, int count,
        QUndoCommand *parent)
    : QUndoCommand(parent),
...
MatrixInsertRowsCmd::MatrixInsertRowsCmd(future::Matrix::Private *private_obj,
    ...
    : QUndoCommand(parent),
      ...
      d_count(count)
{
    setText(QObject::tr("%1: insert %2 row(s)")
                    .arg(d_private_obj->name())
                    .arg(d_count));
}
...
MatrixRemoveColumnsCmd::MatrixRemoveColumnsCmd(
        future::Matrix::Private *private_obj, int first, int count,
        QUndoCommand *parent)
    : QUndoCommand(parent),
      d_private_obj(private_obj),
      d_first(first),
...
void MatrixRemoveColumnsCmd::redo()
{
    if (d_backups.isEmpty()) {
        ...
        for (int i = 0; i < d_count; i++)
            d_backups.append(
                    d_private_obj->columnCells(d_first + i, 0, last_row));
    }
...
void MatrixRemoveColumnsCmd::undo()
{
    ...
    int last_row = d_private_obj->rowCount() - 1;
    for (int i = 0; i < d_count; i++)
        d_private_obj->setColumnCells(d_first + i, 0, last_row,
                                      d_backups.at(i));
}
///////////////////////////////////////////////////////////////////////////
...
MatrixRemoveRowsCmd::MatrixRemoveRowsCmd(future::Matrix::Private *private_obj,
    ...
    : QUndoCommand(parent),
      ...
      d_count(count)
{
    setText(QObject::tr("%1: remove %2 row(s)")
                    .arg(d_private_obj->name())
                    .arg(d_count));
}
...
void MatrixRemoveRowsCmd::redo()
{
    if (d_backups.isEmpty()) {
        ...
        for (int col = 0; col < d_private_obj->columnCount(); col++)
            d_backups.append(
                    d_private_obj->columnCells(col, d_first, last_row));
    }
...
void MatrixRemoveRowsCmd::undo()
{
    ...
    int last_row = d_first + d_count - 1;
    for (int col = 0; col < d_private_obj->columnCount(); col++)
        d_private_obj->setColumnCells(col, d_first, last_row,
                                      d_backups.at(col));
}
///////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////
MatrixClearCmd::MatrixClearCmd(future::Matrix::Private *private_obj,
                               QUndoCommand *parent)
...
///////////////////////////////////////////////////////////////////////////
MatrixClearColumnCmd::MatrixClearColumnCmd(future::Matrix::Private *private_obj,
                                           int col, QUndoCommand *parent)
    : QUndoCommand(parent), d_private_obj(private_obj), d_col(col)
{
    setText(QObject::tr("%1: clear column %2")
                    .arg(d_private_obj->name())
                    .arg(d_col + 1));
}
...
{
    if (d_backup.isEmpty())
        d_backup = d_private_obj->columnCells(d_col, 0,
                                              d_private_obj->rowCount() - 1);
    d_private_obj->clearColumn(d_col);
}

void MatrixClearColumnCmd::undo()
{
    d_private_obj->setColumnCells(d_col, 0, d_private_obj->rowCount() - 1,
                                  d_backup);
}
///////////////////////////////////////////////////////////////////////////
...
MatrixSetCellValueCmd::MatrixSetCellValueCmd(
        future::Matrix::Private *private_obj, int row, int col, double value,
        QUndoCommand *parent)
    : QUndoCommand(parent),
      d_private_obj(private_obj),
      d_row(row),
      ...
      d_value(value)
{
    // remark: don't use many QString::arg() calls in ctors of commands that
    // might be called often, they use a lot of execution time
...
MatrixSetCoordinatesCmd::MatrixSetCoordinatesCmd(
        future::Matrix::Private *private_obj, double x1, double x2, double y1,
        double y2, QUndoCommand *parent)
    : QUndoCommand(parent),
      d_private_obj(private_obj),
      d_new_x1(x1),
      d_new_x2(x2),
      ...
      d_new_y2(y2)
{
    setText(QObject::tr("%1: set matrix coordinates")
                    .arg(d_private_obj->name()));
}
...
///////////////////////////////////////////////////////////////////////////
MatrixSetFormulaCmd::MatrixSetFormulaCmd(future::Matrix::Private *private_obj,
                                         QString formula)
...
MatrixSetColumnCellsCmd::MatrixSetColumnCellsCmd(
        future::Matrix::Private *private_obj, int col, int first_row,
...
void MatrixSetColumnCellsCmd::redo()
{
    if (d_old_values.isEmpty())
        d_old_values =
                d_private_obj->columnCells(d_col, d_first_row, d_last_row);
...
void MatrixSetRowCellsCmd::redo()
{
    if (d_old_values.isEmpty())
        d_old_values =
                d_private_obj->rowCells(d_row, d_first_column, d_last_column);
    d_private_obj->setRowCells(d_row, d_first_column, d_last_column, d_values);
}

void MatrixSetRowCellsCmd::undo()
{
    d_private_obj->setRowCells(d_row, d_first_column, d_last_column,
                               d_old_values);
}
///////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////
MatrixTransposeCmd::MatrixTransposeCmd(future::Matrix::Private *private_obj,
                                       QUndoCommand *parent)
    : QUndoCommand(parent), d_private_obj(private_obj)
{
...
MatrixMirrorHorizontallyCmd::MatrixMirrorHorizontallyCmd(
        future::Matrix::Private *private_obj, QUndoCommand *parent)
    : QUndoCommand(parent), d_private_obj(private_obj)
{
