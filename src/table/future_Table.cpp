/***************************************************************************
    File                 : Table.cpp
    Project              : AlphaPlot
    Description          : Aspect providing a spreadsheet table with column
 logic
    --------------------------------------------------------------------
...
Table::Table(AbstractScriptingEngine *engine, int rows, int columns,
             const QString &name)
...
#endif
{
    ...
    for (int i = 0; i < columns; i++) {
        Column *new_col =
                new Column(QString::number(i + 1), AlphaPlot::Numeric);
        new_col->setPlotDesignation(i == 0 ? AlphaPlot::X : AlphaPlot::Y);
        cols << new_col;
    }
    appendColumns(cols);
    setRowCount(rows);
...
void Table::insertColumns(int before, QList<Column *> new_cols)
{
    if (new_cols.size() < 1 || before < 0 || before > columnCount())
        return;
    WAIT_CURSOR;
    beginMacro(QObject::tr("%1: insert %2 column(s)")
                       .arg(name())
                       .arg(new_cols.size()));
    int pos = before;
    foreach (Column *col, new_cols)
        insertChild(col, pos++);
    // remark: the TableInsertColumnsCmd will be created in
...
void Table::removeColumns(int first, int count)
{
    ...
    // remark:  the TableRemoveColumnsCmd will be created in
...
void Table::removeRows(int first, int count)
{
    ...
    exec(new TableSetNumberOfRowsCmd(d_table_private,
                                     d_table_private->rowCount() - count));
    endMacro();
    RESET_CURSOR;
}
...
void Table::setRowCount(int new_size)
{
    if ((new_size < 0) || (new_size == d_table_private->rowCount()))
        return;
    WAIT_CURSOR;
    beginMacro(QObject::tr("%1: set the number of rows to %2")
                       .arg(name())
                       .arg(new_size));
...
int Table::columnCount(AlphaPlot::PlotDesignation pd) const
{
    int count = 0;
...
void Table::pasteIntoSelection()
{
    ...
    if (mimeData->hasText()) {
        ...
        for (int i = 0; i < input_row_count; i++) {
            cell_texts.append(
                    input_rows.at(i).trimmed().split(QRegExp("\\s+")));
        ...
        // selection will be expanded to the needed size from the current cell
        {
            ...
            if (last_col >= columnCount()) {
                ...
                for (int i = 0; i < last_col + 1 - columnCount(); i++) {
                    Column *new_col =
                            new Column(QString::number(i + 1), AlphaPlot::Text);
                    new_col->setPlotDesignation(AlphaPlot::Y);
                    cols << new_col;
                }
                appendColumns(cols);
        ...
        for (int r = 0; r < rows && r < input_row_count; r++) {
            for (int c = 0; c < cols && c < input_col_count; c++) {
                if (d_view->isCellSelected(first_row + r, first_col + c)
                    ...
                    if (d_view->formulaModeActive()) {
                        col_ptr->setFormula(first_row + r,
                                            cell_texts.at(r).at(c));
                        col_ptr->setInvalid(first_row + r, false);
                    } else
                        col_ptr->asStringColumn()->setTextAt(
                                first_row + r, cell_texts.at(r).at(c));
                }
            }
        }
...
void Table::fillSelectedCellsWithRandomNumbers()
{
    ...
    foreach (Column *col_ptr, d_view->selectedColumns()) {
        ...
        case AlphaPlot::Text: {
            ...
            for (int row = first; row <= last; row++)
                if (d_view->isCellSelected(row, col))
                    results << QString::number(double(qrand())
                                               / double(RAND_MAX));
                else
                    results << col_ptr->textAt(row);
            col_ptr->replaceTexts(first, results);
                ...
                if (d_view->isCellSelected(row, col))
                    results << QDateTime(
                            earliestDate.addDays(
                                    ...
                                    * (static_cast<double>(
                                            earliestDate.daysTo(latestDate)))
                                    / (static_cast<double>(RAND_MAX))),
                            midnight.addMSecs((static_cast<qint64>(qrand()))
                                              * 1000 * 60 * 60 * 24
                                              / RAND_MAX));
                else
...
void Table::fillSelectedCellsWithCustomRandomNumbers()
{
    ...
    foreach (Column *col_ptr, d_view->selectedColumns()) {
        if (col_ptr->columnMode() != AlphaPlot::Numeric) {
            QMessageBox::warning(
                    nullptr, tr("Random Distribution Error!"),
        ...
        const QVector<double> &params)
{
    ...
    case RandomDistributionDialog::Gaussian:
        rndDistLmd = [](gsl_rng *rnd, const QVector<double> &prm) -> double {
            return gsl_ran_gaussian(rnd, prm.at(1) /*sigma*/)
                    + prm.at(0) /*mu*/;
        };
        break;
    ...
    case RandomDistributionDialog::RayleighTail:
        rndDistLmd = [](gsl_rng *rnd, const QVector<double> &prm) -> double {
            return gsl_ran_rayleigh_tail(rnd, prm.at(1) /*a*/,
                                         prm.at(0) /*sigma*/);
        };
        break;
    ...
    case RandomDistributionDialog::Lognormal:
        rndDistLmd = [](gsl_rng *rnd, const QVector<double> &prm) -> double {
            return gsl_ran_lognormal(rnd, prm.at(1) /*zeta*/,
                                     prm.at(0) /*sigma*/);
        };
        break;
...
void Table::setSelectionAs(AlphaPlot::PlotDesignation pd)
{
    if (!d_view)
        return;
    WAIT_CURSOR;
...
{
    setSelectionAs(AlphaPlot::X);
}

void Table::setSelectedColumnsAsY()
{
    setSelectionAs(AlphaPlot::Y);
}

void Table::setSelectedColumnsAsZ()
{
    setSelectionAs(AlphaPlot::Z);
}

void Table::setSelectedColumnsAsYError()
{
    setSelectionAs(AlphaPlot::yErr);
}

void Table::setSelectedColumnsAsXError()
{
    setSelectionAs(AlphaPlot::xErr);
}

void Table::setSelectedColumnsAsNone()
{
    setSelectionAs(AlphaPlot::noDesignation);
}
...
void Table::normalizeColumns(QList<Column *> cols)
{
    ...
    foreach (Column *col, cols) {
        if (col->dataType() == AlphaPlot::TypeDouble) {
            double max = 0.0;
...
void Table::normalizeSelection()
{
    ...
    foreach (Column *col_ptr, d_view->selectedColumns()) {
        int col = columnIndex(col_ptr);
        if (col_ptr->dataType() == AlphaPlot::TypeDouble)
            for (int row = 0; row < col_ptr->rowCount(); row++)
                if (d_view->isCellSelected(row, col)
                    && col_ptr->valueAt(row) > max)
...
QMenu *Table::createContextMenu() const
{
    ...
    // ----
    // Rename --> AbstractAspect::createContextMenu(); maybe call this
    // "Properties" and include changing comment/caption spec
    // Duplicate --> AbstractPart::createContextMenu()
    // Hide/Show --> Do we need hiding of views (in addition to minimizing)? How
    // do we avoid confusion with hiding of Aspects?
    // Activate ?
...
void Table::createActions()
{
...
#ifndef LEGACY_CODE_0_2_x
    action_mask_selection = new QAction(QIcon(QPixmap(":/mask.xpm")),
                                        tr("&Mask", "mask selection"), this);
    actionManager()->addAction(action_mask_selection, "mask_selection");

    action_unmask_selection =
            new QAction(QIcon(QPixmap(":/unmask.xpm")),
                        tr("&Unmask", "unmask selection"), this);
    ...
    action_recalculate = new QAction(
    ...
    action_fill_random_distribution = new QAction(
    ...
    action_toggle_comments = new QAction(
    ...
    action_add_column =
            new QAction(IconLoader::load("edit-table-insert-column-right",
    ...
    action_clear_table = new QAction(
...
#ifndef LEGACY_CODE_0_2_x
    action_clear_masks = new QAction(QIcon(QPixmap(":/unmask.xpm")),
                                     tr("Clear Masks"), this);
    ...
    action_set_as_y =
            new QAction(QIcon(QPixmap()), tr("Y", "plot designation"), this);
    ...
    action_set_as_xerr = new QAction(
    ...
    action_set_as_yerr = new QAction(
    ...
    action_normalize_columns =
            new QAction(IconLoader::load("edit-normalize-random-number",
                                         IconLoader::LightDark),
    ...
    actionManager()->addAction(action_normalize_selection,
                               "normalize_selection");
    ...
    action_statistics_columns = new QAction(
...
bool Table::export_to_TeX(QString fileName, TeXTableSettings &tex_settings)
{
    ...
    out << QString("\\begin{tabular}{|*{")
                    + QString().setNum(columns_list.count()) + ("}{")
    ...
    QStringList str_row;
    for (int row_index = first_row_index; row_index < last_row_index + 1;
         row_index++) {
        str_row.clear();
        foreach (Column *col, columns_list)
...
void Table::goToCell()
{
    ...
    int col =
            QInputDialog::getInt(nullptr, tr("Go to Cell"), tr("Enter column"),
...
int Table::colX(int col)
{
    for (int i = col - 1; i >= 0; i--) {
        if (column(i)->plotDesignation() == AlphaPlot::X)
            return i;
    }
    int cols = columnCount();
...
int Table::colY(int col)
{
    ...
    if (column(col)->plotDesignation() == AlphaPlot::xErr
        || column(col)->plotDesignation() == AlphaPlot::yErr) {
...
void Table::sortColumns(Column *leading, QList<Column *> cols, bool ascending)
{
    ...
    {
    public:
        static bool doubleLess(const QPair<double, int> &a,
                               const QPair<double, int> &b)
        {
            return a.first < b.first;
        }
        static bool doubleGreater(const QPair<double, int> &a,
                                  const QPair<double, int> &b)
        {
            return a.first > b.first;
        }
        static bool QStringLess(const QPair<QString, int> &a,
                                const QPair<QString, int> &b)
        {
            return a < b;
        }
        static bool QStringGreater(const QPair<QString, int> &a,
                                   const QPair<QString, int> &b)
        {
            return a > b;
        }
        static bool QDateTimeLess(const QPair<QDateTime, int> &a,
                                  const QPair<QDateTime, int> &b)
        {
            return a < b;
        }
        static bool QDateTimeGreater(const QPair<QDateTime, int> &a,
                                     const QPair<QDateTime, int> &b)
        {
            return a > b;
        }
            ...
            if (col->dataType() == AlphaPlot::TypeDouble) {
                ...
                if (ascending)
                    std::stable_sort(map.begin(), map.end(),
                                     CompareFunctions::doubleLess);
                else
                    std::stable_sort(map.begin(), map.end(),
                                     CompareFunctions::doubleGreater);
                ...
                if (ascending)
                    std::stable_sort(map.begin(), map.end(),
                                     CompareFunctions::QStringLess);
                else
                    std::stable_sort(map.begin(), map.end(),
                                     CompareFunctions::QStringGreater);
                ...
                if (ascending)
                    std::stable_sort(map.begin(), map.end(),
                                     CompareFunctions::QDateTimeLess);
                else
                    std::stable_sort(map.begin(), map.end(),
                                     CompareFunctions::QDateTimeGreater);
    ...
    {
        if (leading->dataType() == AlphaPlot::TypeDouble) {
            QList<QPair<double, int>> map;
...
void Table::handleRowsAboutToBeInserted(const AbstractColumn *col, int before,
                                        int count)
{
...
void Table::handleRowsAboutToBeRemoved(const AbstractColumn *col, int first,
                                       int count)
{
    Q_UNUSED(col);
    Q_UNUSED(first);
...
void Table::connectColumn(const Column *col)
{
    ...
    connect(col,
            ...
            this,
            SLOT(handleRowsAboutToBeInserted(const AbstractColumn *, int,
                                             int)));
...
bool Table::load(XmlStreamReader *reader)
{
    if (reader->isStartElement() && reader->name() == "table") {
        ...
        while (!reader->atEnd()) {
            ...
            if (reader->isStartElement()) {
                if (reader->name() == "comment") {
                    ...
                    Column *column =
                            new Column(tr("Column %1").arg(1), AlphaPlot::Text);
        ...
        if (cols != columnCount())
            reader->raiseWarning(tr("columns attribute and number of read "
...
void Table::loadIcons()
{
    ...
    action_select_all->setIcon(
            IconLoader::load("edit-table-select-all", IconLoader::LightDark));
    action_add_column->setIcon(IconLoader::load(
            "edit-table-insert-column-right", IconLoader::LightDark));
    ...
    // column related actions
    action_insert_columns->setIcon(IconLoader::load("edit-table-insert-column",
                                                    IconLoader::LightDark));
    action_remove_columns->setIcon(IconLoader::load("edit-table-delete-column",
                                                    IconLoader::LightDark));
    action_clear_columns->setIcon(
            IconLoader::load("edit-table-clear-column", IconLoader::LightDark));
    action_add_columns->setIcon(IconLoader::load(
            "edit-table-insert-column-right", IconLoader::LightDark));
    action_set_as_xerr->setIcon(
            IconLoader::load("graph-x-error", IconLoader::LightDark));
    action_set_as_yerr->setIcon(
            IconLoader::load("graph-y-error", IconLoader::LightDark));
    action_normalize_columns->setIcon(IconLoader::load(
            "edit-normalize-random-number", IconLoader::LightDark));
    action_normalize_selection->setIcon(IconLoader::load(
            "edit-normalize-random-number", IconLoader::LightDark));
...
int Table::Private::numColsWithPD(AlphaPlot::PlotDesignation pd)
{
    int count = 0;
...
void Table::Private::updateHorizontalHeader(int start_col, int end_col)
{
    ...
    if (numColsWithPD(AlphaPlot::X) > 1) {
        int x_cols = 0;
        for (int i = 0; i < d_column_count; i++) {
            if (d_columns.at(i)->plotDesignation() == AlphaPlot::X)
                composeColumnHeader(i,
                                    d_columns.at(i)->name() + "[X"
                                            + QString::number(++x_cols) + "]");
            else if (d_columns.at(i)->plotDesignation() == AlphaPlot::Y) {
                if (x_cols > 0)
                    composeColumnHeader(i,
                                        d_columns.at(i)->name() + "[Y"
                                                + QString::number(x_cols)
                                                + "]");
                else
                    composeColumnHeader(i, d_columns.at(i)->name() + "[Y]");
            } else if (d_columns.at(i)->plotDesignation() == AlphaPlot::Z) {
                if (x_cols > 0)
                    composeColumnHeader(i,
                                        d_columns.at(i)->name() + "[Z"
                                                + QString::number(x_cols)
                                                + "]");
                else
                    composeColumnHeader(i, d_columns.at(i)->name() + "[Z]");
            } else if (d_columns.at(i)->plotDesignation() == AlphaPlot::xErr) {
                if (x_cols > 0)
                    composeColumnHeader(i,
                                        d_columns.at(i)->name() + "[xEr"
                                                + QString::number(x_cols)
                                                + "]");
                else
                    composeColumnHeader(i, d_columns.at(i)->name() + "[xEr]");
            } else if (d_columns.at(i)->plotDesignation() == AlphaPlot::yErr) {
                if (x_cols > 0)
                    composeColumnHeader(i,
                                        d_columns.at(i)->name() + "[yEr"
                                                + QString::number(x_cols)
                                                + "]");
                else
...
QVariant Table::Private::headerData(int section, Qt::Orientation orientation,
                                    int role) const
{
    switch (orientation) {
    case Qt::Horizontal:
