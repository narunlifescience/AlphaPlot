...
void TableView::init()
{
    ...
    connect(d_hide_button, SIGNAL(pressed()), this,
    ...
    connect(sel_model,
            SIGNAL(currentColumnChanged(const QModelIndex &,
                                        const QModelIndex &)),
            this,
            SLOT(currentColumnChanged(const QModelIndex &,
                                      const QModelIndex &)));
    connect(sel_model,
...
void TableView::retranslateStrings()
{
    ...
    ui.type_box->addItem(
            IconLoader::load("view-calendar", IconLoader::LightDark),
            tr("Date and time"), QVariant(int(AlphaPlot::DateTime)));
    ui.type_box->setCurrentIndex(0);

    ui.date_time_interval->clear();
    ui.date_time_interval->addItem(tr("years"),
                                   int(Double2DateTimeFilter::Year));
    ...
    ui.date_time_interval->addItem(tr("hours"),
                                   int(Double2DateTimeFilter::Hour));
...
void TableView::setColumnForControlTabs(int col)
{
    ...
    case AlphaPlot::DateTime: {
        ...
        ui.format_box->setCurrentIndex(
                ui.format_box->findData(filter->format()));
        break;
    }
    default:
        ui.date_time_interval->setVisible(false);
...
void TableView::updateFormatBox()
{
    ...
    case AlphaPlot::Text:
        ...
        break;
    case AlphaPlot::Month:
        ui.format_box->addItem(tr("Number without leading zero"),
                               QVariant("M"));
    ...
    case AlphaPlot::DateTime: {
        ...
        QStringList date_stringlist = {
            "yyyy-MM-dd", "yyyy/MM/dd", "dd/MM/yyyy", "dd/MM/yy", "dd.MM.yyyy",
            "dd.MM.yy",   "MM/yyyy",    "dd.MM.",     "yyyyMMdd"
        };
    ...
    default:
        ui.format_box->addItem(QString()); // just for savety to have at least
                                           // one item in any case
    }
...
void TableView::applyType()
{
    ...
    case AlphaPlot::Numeric:
        foreach (Column *col, list) {
            col->beginMacro(
                    QObject::tr("%1: change column type").arg(col->name()));
                    ...
                    Double2DateTimeFilter::UnitInterval unit =
                            static_cast<Double2DateTimeFilter::UnitInterval>(
                                    ui.date_time_interval
                                            ->itemData(ui.date_time_interval
                                                               ->currentIndex())
                                            .toInt());
...
{
    if (full)
        return d_view_widget->selectionModel()->isRowSelected(row,
                                                              QModelIndex());
    else
...
bool TableView::isCellSelected(int row, int col)
{
    ...
    return d_view_widget->selectionModel()->isSelected(
...
void TableView::moveFloatingButton() const
{
    ...
    if (!d_control_tabs->isHidden()) {
        d_hide_button->move(widget()->width()
                                    - (d_control_tabs->width()
                                       + d_hide_button->width()
                                       + verticalScrollWidth),
