 ...
 *
This class (incl. Table::Private and its commands) is one aspect in the projet
hierarchy
that represents a spreadsheet table with column logic. Table provides the public
API while
Table::Private completely encapsulates the data. The table commands (derived
from QUndoCommand)
encapsulate all write operations which can be undone and redone, if the table
has an undo stack
associated with it (usually by the project root aspect).

The underlying private data object is not visible to any classes other then
those meantioned
above with one exeption:
Pointers to columns can be passed around an manipulated directly. The owner
Table (parent aspect
of the Column objects) will be notified by emission of signals and react
accordingly.
All public methods of Table and Column are undo aware.

Table also manages its main view of class TableView. Table and TableView can
call each others
API in both directions. User interaction ist party handled in TableView and
translated into
Table API calls (e.g., when a user edits a cell this will be handled by the
delegate of
TableView and Table will not know whether a script or a user changed the data.).
Other parts
of the user interaction are handled by actions provides by Table, e.g., via a
context menu.

Selections are handled by TableView and can be queried by Table. All selection
based functions
do nothing unless the view exists. The view is created by the first call to
view();
*/
#ifndef LEGACY_CODE_0_2_x
...
public:
    class Private; // This could also be private, but then all commands need to
                   // be friend classes
    friend class Private;

#ifndef LEGACY_CODE_0_2_x
    Table(AbstractScriptingEngine *engine, int rows, int columns,
          const QString &name);
#else
    ...
    /**
     * This method may be called multiple times during the life time of an
     * Aspect, or it might not get called at all. Aspects must not depend on the
     * existence of a view for their operation.
     */
    ...
    {
        return d_default_comment_visibility;
    }
    //! Return the text displayed in the given cell
    QString text(int row, int col);
    void setSelectionAs(AlphaPlot::PlotDesignation pd);
...
private slots:
    ...
    void handleRowsAboutToBeInserted(const AbstractColumn *col, int before,
                                     int count);
    void handleRowsInserted(const AbstractColumn *col, int before, int count);
    void handleRowsAboutToBeRemoved(const AbstractColumn *col, int first,
                                    int count);
