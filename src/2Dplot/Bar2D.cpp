...
Axis2D *Bar2D::getxaxis() const
{
    return xaxis_;
}

Axis2D *Bar2D::getyaxis() const
{
    return yaxis_;
}

Qt::PenStyle Bar2D::getstrokestyle_barplot() const
{
    return pen().style();
}

QColor Bar2D::getstrokecolor_barplot() const
{
    return pen().color();
}

double Bar2D::getstrokethickness_barplot() const
{
    return pen().widthF();
}

QColor Bar2D::getfillcolor_barplot() const
{
    return brush().color();
}

DataBlockBar *Bar2D::getdatablock_barplot() const
{
    return bardata_;
}
...
DataBlockHist *Bar2D::getdatablock_histplot() const
{
    return histdata_;
}
...
{
    if (histdata_->getautobin() == status)
        return;
...
{
    if (histdata_->getbinsize() == binsize)
        return;
...
bool Bar2D::load(XmlStreamReader *xmlreader)
{
    ...
    while (!xmlreader->atEnd()) {
        if (xmlreader->isEndElement() && xmlreader->name() == "bar")
            break;
        ...
        if (xmlreader->isStartElement() && xmlreader->name() == "box") {
            ...
            (ok) ? setWidth(w)
                 : xmlreader->raiseWarning(
                         tr("Bar2D width property setting error"));
            ...
            while (!xmlreader->atEnd()) {
                ...
                if (xmlreader->isStartElement() && xmlreader->name() == "pen") {
                    ...
                    if (ok) {
                        ...
                        xmlreader->raiseWarning(
                                tr("Bar2D pen property setting error"));
                }
            }
            ...
            while (!xmlreader->atEnd()) {
                ...
                if (xmlreader->isStartElement()
                    ...
                    if (ok) {
                        ...
                        xmlreader->raiseWarning(
                                tr("Bar2D brush property setting error"));
                }
            }
        }
