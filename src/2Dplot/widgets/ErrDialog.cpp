      ...
      app_(qobject_cast<ApplicationWindow *>(parent->parent()))
{
    ...
    connect(percentBox, SIGNAL(toggled(bool)), valueBox,
...
void ErrDialog::plotNames()
{
    ...
    foreach (Curve2D *curve, curvelist) {
        if (curve->getplottype_cplot() == Graph2DCommon::PlotType::Associated) {
            ...
            plotted_columns_ << plotdata;
            nameLabel->addItem(plotdata.table->name() + "_"
                               + plotdata.ycol->name() + "["
            ...
            if (selectplotdata.table == graphdata->gettable()
                ...
                if (xErrBox->isChecked()) {
                    if (selectplotdata.xcol->dataType()
                        ...
                        QMessageBox::critical(app_, tr("Error"),
                                              tr("You can only add error bars "
                                                 "to numeric column(s)!"));
                        return;
                    }
                } else {
                ...
                if (!errors)
                    return;
                (xErrBox->isChecked())
                        ? ls->setXerrorBar(selectplotdata.table, errors, 0,
                                           errors->rowCount() - 1)
                        : ls->setYerrorBar(selectplotdata.table, errors, 0,
                                           errors->rowCount() - 1);
            }
        }
        foreach (Curve2D *curve, curvelist) {
        ...
        foreach (Bar2D *bar, barlist) {
            ...
            if (selectplotdata.table == bardata->gettable()
                ...
                if (!errors)
                    return;
                (xErrBox->isChecked())
                        ? bar->setXerrorBar(selectplotdata.table, errors, 0,
                                            errors->rowCount() - 1)
                        : bar->setYerrorBar(selectplotdata.table, errors, 0,
                                            errors->rowCount() - 1);
            }
        }
