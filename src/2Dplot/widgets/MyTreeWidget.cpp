      ...
      movedownimageitem_(new QAction("Layer Down", this))
{
    ...
    moveupcolormap_->setIcon(
            IconLoader::load("edit-up", IconLoader::LightDark));
    moveuptextitem_->setIcon(
            IconLoader::load("edit-up", IconLoader::LightDark));
    moveuplineitem_->setIcon(
            IconLoader::load("edit-up", IconLoader::LightDark));
    moveupimageitem_->setIcon(
            IconLoader::load("edit-up", IconLoader::LightDark));
    ...
    connect(adderrorbar_, &QAction::triggered, this,
            &MyTreeWidget::adderrorbar);
    ...
    connect(leftdatetimeaxis_, SIGNAL(triggered(bool)), this,
            SLOT(addAxis2D()));
    ...
    connect(rightdatetimeaxis_, SIGNAL(triggered(bool)), this,
            SLOT(addAxis2D()));
    ...
    connect(moveupls_, &QAction::triggered, this, [=]() {
        moveplottablelayer<LineSpecial2D>(
...
void MyTreeWidget::CurrentItemChanged(QTreeWidgetItem *current)
{
    ...
    if (currentaxisrect)
        currentaxisrect->selectAxisRect();
}
...
void MyTreeWidget::showContextMenu(const QPoint &pos)
{
    ...
    switch (static_cast<PropertyItemType>(
...
void MyTreeWidget::removeLineSpecialChannel2D()
{
    ...
    if (position != -1)
        axisrect->removeChannel2D(channellist.at(position));
}
...
void MyTreeWidget::removeVector2D()
{
    ...
    bool result =
            vector->getxaxis()->getaxisrect_axis()->removeVector2D(vector);
    if (!result) {
        qDebug() << "unable to remove line scatter 2d plot";
        return;
    }
...
template<class T>
void MyTreeWidget::moveplottablelayer(QAction *action,
