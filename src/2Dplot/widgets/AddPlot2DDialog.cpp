...
QSize AddPlot2DDialog::sizeHint() const
{
...
void AddPlot2DDialog::contextMenuEvent(QContextMenuEvent *e)
{
    ...
    if (rect.contains(pos)) {
        ...
        if (lst.size() > 1)
            contextMenu.addAction(tr("&Plot Selection"), this,
                                  SLOT(addCurves()));
        else if (lst.size() == 1)
    ...
    if (rect.contains(pos)) {
        ...
        if (lst.size() > 1)
            ...
            contextMenu.addAction(tr("&Delete Curve"), this,
                                  SLOT(removeCurves()));
        contextMenu.exec(QCursor::pos());
    }
...
void AddPlot2DDialog::loadplotcontents()
{
    ...
    foreach (Bar2D *bar, barlist) {
        if (!bar->ishistogram_barplot()) {
            ...
            QPair<Table *, Column *> columnpair = QPair<Table *, Column *>(
                    bardata->gettable(), bardata->getycolumn());
            plotted_columns_ << columnpair;
            contents->addItem(columnpair.first->name() + "_"
                              + columnpair.second->name() + "["
    ...
    foreach (QListWidgetItem *item, lst) {
        ...
        if (pair.first->firstXCol() < 0) {
            QMessageBox::warning(this, tr("Error"),
                                 tr("No X column available in table: ")
                                         + pair.first->name() + "!");
            return;
        }
