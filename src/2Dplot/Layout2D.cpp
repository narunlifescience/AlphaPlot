      ...
      ypickerline_(nullptr)
{
    ...
    layoutManagebuttonsBox_->setContentsMargins(
            buttonboxmargin_, buttonboxmargin_, buttonboxmargin_,
            buttonboxmargin_);
    ...
    connect(plot2dCanvas_, &Plot2D::mouseMove, this,
            &Layout2D::mouseMoveSignal);
    ...
    connect(plot2dCanvas_, &Plot2D::beforeReplot, this,
            &Layout2D::beforeReplot);
                                                           ...
                                                           int key)
{
    ...
    // data bounds
    statBoxData.boxWhiskerDataBounds.sd_lower =
            statBoxData.mean - statBoxData.sd;
    statBoxData.boxWhiskerDataBounds.sd_upper =
            statBoxData.mean + statBoxData.sd;
    statBoxData.boxWhiskerDataBounds.se_lower =
            statBoxData.mean - statBoxData.se;
    statBoxData.boxWhiskerDataBounds.se_upper =
            statBoxData.mean + statBoxData.se;
                                      ...
                                      const QString yLabel)
{
    ...
    Curve2D *curve = element->addFunction2DPlot(xdata, ydata, xAxis.at(0),
                                                yAxis.at(0), name);
    curve->rescaleAxes();
}
                                                   ...
                                                   int from, int to, int key)
{
    ...
    AxisRect2D *element =
            addAxisRectItem(AlphaPlot::ColumnDataType::TypeString,
                            AlphaPlot::ColumnDataType::TypeDouble);
    QList<Axis2D *> xAxis =
            element->getAxesOrientedTo(Axis2D::AxisOreantation::Bottom);
    xAxis << element->getAxesOrientedTo(Axis2D::AxisOreantation::Top);
    ...
    if (multilayout) {
        foreach (Column *col, collist) {
            ...
            QList<Axis2D *> yAxis =
                    element->getAxesOrientedTo(Axis2D::AxisOreantation::Left);
            yAxis << element->getAxesOrientedTo(Axis2D::AxisOreantation::Right);
            element->addHistogram2DPlot(barType, table, col, from, to,
                                 ...
                                 QList<Column *> ycollist, int from, int to)
{
    ...
    case AxisRect2D::BarType::HorizontalBars: {
        element =
                addAxisRectItem(ycollist.at(0)->dataType(), xData->dataType());
                                       ...
                                       int to)
{
    ...
    for (int i = 0; i < ycollist.size(); i++) {
        Bar2D *bar =
                element->addBox2DPlot(barType, table, xData, ycollist.at(i),
                                    ...
                                    Column *y2Data, int from, int to)
{
    ...
    Vector2D *vex =
            element->addVectorPlot(vectorplot, table, x1Data, y1Data, x2Data,
                                   y2Data, from, to, xAxis.at(0), yAxis.at(0));
    vex->rescaleAxes();
}
...
QList<AxisRect2D *> Layout2D::getAxisRectList()
{
    ...
    for (int i = 0; i < layout_->elementCount(); i++) {
        AxisRect2D *axisrect =
                dynamic_cast<AxisRect2D *>(layout_->elementAt(i));
        if (axisrect)
        ...
        Column *xData, QList<Column *> ycollist, int from, int to)
{
    ...
    foreach (Column *col, ycollist) {
        LineSpecial2D *linescatter =
                element->addLineSpecial2DPlot(plotType, table, xData, col, from,
                                   ...
                                   QList<Column *> ycollist, int from, int to)
{
    ...
    addTextToAxisTicker(ycollist.at(0), yAxis.at(0), from, to);
    foreach (Column *col, ycollist) {
        Curve2D *curve = element->addCurve2DPlot(
                plotType, table, xcol, col, from, to, xAxis.at(0), yAxis.at(0));
        curve->rescaleAxes();
    }
}
...
AxisRect2D *Layout2D::getCurrentAxisRect()
{
    return currentAxisRect_;
}

Plot2D *Layout2D::getPlotCanwas() const
{
    return plot2dCanvas_;
}

AxisRect2D *
Layout2D::addAxisRectItem(const AlphaPlot::ColumnDataType &xcoldatatype,
...
void Layout2D::removeAxisRectItem()
{
    ...
    for (int i = 0; i < currentAxisRect_->getChannelVec().count(); i++) {
        currentAxisRect_->removeChannel2D(
                currentAxisRect_->getChannelVec().at(i));
    }
...
void Layout2D::addTextToAxisTicker(Column *col, Axis2D *axis, int from, int to)
{
    if (col->dataType() == AlphaPlot::ColumnDataType::TypeString) {
        ...
        ticker.staticCast<QCPAxisTickerDateTime>()->setDateTimeFormat(
                Utilities::splitstring(static_cast<DateTime2StringFilter *>(
                                               col->outputFilter())
                                               ->format()));
    }
}
...
void Layout2D::mouseReleaseSignal(QMouseEvent *event)
{
    if (event->button() == Qt::RightButton) {
        ...
        menu->popup(plot2dCanvas_->mapToGlobal(
                QPoint(static_cast<int>(startPos.x()),
...
bool Layout2D::exportGraph()
{
    ...
    if (selected_filter.contains(".pdf")) {
        ...
        success = plot2dCanvas_->saveRastered(
                file_name, raster_width, raster_height, raster_scale, c_char,
    ...
    if (!success) {
        QMessageBox::critical(
                ...
                tr("Unknown error exporting: <br><h4> %1 </h4><p>May be "
                   "the advanced image export parameters are not rightfully "
                   "set!")
                        .arg(file_name));
    }
    return success;
...
void Layout2D::updateData(Table *table, const QString &name)
{
    ...
    foreach (AxisRect2D *axisrect, getAxisRectList()) {
        ...
        foreach (LineSpecial2D *ls, lslist) {
            ...
            if (data->table == table) {
                if (data->xcol == col || data->ycol == col) {
                    ls->setGraphData(data->table, data->xcol, data->ycol,
                                     data->from, data->to);
                    modified = true;
                }
            }
        ...
        for (int i = 0; i < channellist.count(); i++) {
            ...
            if (data1->table == table) {
                if (data1->xcol == col || data1->ycol == col) {
                    channel.first->setGraphData(data1->table, data1->xcol,
                                                data1->ycol, data1->from,
                                                data1->to);
                    modified = true;
                }
            }
            if (data2->table == table) {
                if (data2->xcol == col || data2->ycol == col) {
                    channel.second->setGraphData(data2->table, data2->xcol,
                                                 data2->ycol, data2->from,
                                                 data2->to);
                    modified = true;
                }
            }
        ...
        foreach (Curve2D *curve, curvelist) {
            if (curve->getplottype_cplot()
                == Graph2DCommon::PlotType::Associated) {
                PlotData::AssociatedData *data =
                        curve->getdatablock_cplot()->getassociateddata();
                if (curve->getxerrorbar_curveplot()) {
                    DataBlockError *xerror = curve->getxerrorbar_curveplot()
                                                     ->getdatablock_error();
                    if (xerror->gettable() == table) {
                        if (xerror->geterrorcolumn() == col) {
                            curve->getxerrorbar_curveplot()->setErrorData(
                                    xerror->gettable(),
                                    xerror->geterrorcolumn(), xerror->getfrom(),
                                    xerror->getto());
                            modified = true;
                        }
                    }
                ...
                if (curve->getyerrorbar_curveplot()) {
                    DataBlockError *yerror = curve->getyerrorbar_curveplot()
                                                     ->getdatablock_error();
                    if (yerror->gettable() == table) {
                        if (yerror->geterrorcolumn() == col) {
                            curve->getyerrorbar_curveplot()->setErrorData(
                                    yerror->gettable(),
                                    yerror->geterrorcolumn(), yerror->getfrom(),
                                    yerror->getto());
                            modified = true;
                        }
                    }
                ...
                if (data->table == table) {
                    if (data->xcol == col || data->ycol == col) {
                        curve->setCurveData(data->table, data->xcol, data->ycol,
                                            data->from, data->to);
                        modified = true;
                    }
                }
        ...
        foreach (Bar2D *bar, barlist) {
            if (!bar->ishistogram_barplot()) {
                ...
                if (data->table == table) {
                    if (data->xcol == col || data->ycol == col) {
                        bar->setBarData(data->table, data->xcol, data->ycol,
                                        data->from, data->to);
                        modified = true;
                    }
                }
        ...
        foreach (Vector2D *vector, vectorlist) {
            if (vector->gettable_vecplot() == table) {
                if (vector->getfirstcol_vecplot() == col
                    ...
                    vector->setGraphData(vector->gettable_vecplot(),
                                         vector->getfirstcol_vecplot(),
                                         vector->getsecondcol_vecplot(),
                                         vector->getthirdcol_vecplot(),
                                         vector->getfourthcol_vecplot(),
                                         vector->getfrom_vecplot(),
        ...
        foreach (Pie2D *pie, pieveclist) {
            if (pie->gettable_pieplot() == table) {
                if (pie->getxcolumn_pieplot() == col) {
                    pie->setGraphData(
                            pie->gettable_pieplot(), pie->getxcolumn_pieplot(),
...
void Layout2D::print()
{
    ...
    connect(previewDialog.get(), &QPrintPreviewDialog::paintRequested,
            [=](QPrinter *printer) {
                ...
                double scale =
                        pageRect.width() / static_cast<double>(plotWidth);

                painter->setMode(QCPPainter::pmVectorized);
                painter->setMode(QCPPainter::pmNoCaching);
                // comment this out if you want cosmetic thin lines (always 1
                // pixel thick independent of pdf zoom level)
                    ...
                    QList<Matrix *> mats)
{
    if (xmlreader->isStartElement() && xmlreader->name() == "plot2d") {
        ...
        if (ok)
            ...
            xmlreader->raiseWarning(
                    tr("Invalid caption policy or read error."));
        // read name
        ...
        while (!xmlreader->atEnd()) {
            xmlreader->readNext();
            if (xmlreader->isEndElement() && xmlreader->name() == "canvas")
                break;
            if (xmlreader->isStartElement() && xmlreader->name() == "canvas") {
                double dpr =
                        xmlreader->readAttributeDouble("devicepixelratio", &ok);
                if (ok)
                ...
                if (ok)
                    ...
                    xmlreader->raiseWarning(
                            tr("Layout2D opengl missing or empty"));
                //
                QString bkcol =
        ...
        while (!xmlreader->atEnd()) {
            xmlreader->readNext();
            if (xmlreader->isEndElement() && xmlreader->name() == "plot2d")
                break;
