...
bool Curve2D::getlineantialiased_cplot() const
{
    return antialiased();
}
...
QBrush Curve2D::getscatterbrush_cplot() const
{
    return scatterStyle().brush();
}

double Curve2D::getscattersize_cplot() const
{
...
QPen Curve2D::getscatterpen_cplot() const
{
    return scatterStyle().pen();
}
...
QString Curve2D::getlegendtext_cplot() const
{
    return name();
}

Axis2D *Curve2D::getxaxis() const
{
    return xAxis_;
}

Axis2D *Curve2D::getyaxis() const
{
    return yAxis_;
}
...
void Curve2D::setlegendtext_cplot(const QString &text)
{
    setName(text);
}
...
void Curve2D::save(XmlStreamWriter *xmlwriter, int xaxis, int yaxis)
{
    ...
    if (curvedata_) {
        ...
        xmlwriter->writeAttribute("from",
                                  QString::number(curvedata_->getfrom()));
        ...
        for (int i = 0; i < functionData_.data()->size(); ++i) {
            ...
            xmlwriter->writeAttribute(
                    "ydata",
                    QString::number(functionData_.data()->at(i)->value));
    ...
    // error bar
    if (xerroravailable_)
        xerrorbar_->save(xmlwriter);
    if (yerroravailable_)
        yerrorbar_->save(xmlwriter);
    // line
...
bool Curve2D::load(XmlStreamReader *xmlreader)
{
    ...
    while (!xmlreader->atEnd()) {
        if (xmlreader->isEndElement() && xmlreader->name() == "curve")
            break;
            ...
            while (!xmlreader->atEnd()) {
                ...
                if (xmlreader->isStartElement() && xmlreader->name() == "pen") {
                    ...
                    if (ok) {
                        ...
                        xmlreader->raiseWarning(tr(
                                "Curve2D scatter pen property setting error"));
                }
            }
            ...
            while (!xmlreader->atEnd()) {
                ...
                if (xmlreader->isStartElement()
                    ...
                    if (ok) {
                        ...
                        xmlreader->raiseWarning(tr(
                                "Curve2D scatterbrush property setting error"));
                }
            }
        }
        xmlreader->readNext();
...
{
    if (splinepoints_->size() < 2 && splinecontrolpoints_->size() < 2)
        return;
    ...
    for (int i = 0, j = 0; i < splinepoints_->size() - 1; i++, j++) {
        QPointF ctrlpoint1 = QPointF(
                xAxis_->coordToPixel(splinecontrolpoints_->at(i + j).x()),
                yAxis_->coordToPixel(splinecontrolpoints_->at(i + j).y()));
        QPointF ctrlpoint2 = QPointF(
                xAxis_->coordToPixel(splinecontrolpoints_->at(i + 1 + j).x()),
...
QVector<QPointF> Curve2D::calculateControlPoints(const QVector<QPointF> &points)
{
...
void Curve2D::removepicker(QMouseEvent *event, const QVariant &details)
{
    ...
    if (dataPoints.dataPointCount() > 0) {
        ...
        if (point.x() > event->localPos().x() - 10
            ...
            && point.y() > event->localPos().y() - 10
            && point.y() < event->localPos().y() + 10) {
            if (curvedata_->removedatafromtable(it->mainKey(),
                                                it->mainValue())) {
                if (curve2dtype_ == Curve2D::Curve2DType::Spline)
                    loadSplineData();
            }
