...
Axis2D *ColorMap2D::getxaxis() const
{
    return xaxis_;
}

Axis2D *ColorMap2D::getyaxis() const
{
    return yaxis_;
}
...
QString ColorMap2D::getname_colormap() const
{
    return colorScale_->label();
}
...
Axis2D::AxisLabelFormat ColorMap2D::getcolormapscaleticklabelformat_axis() const
{
...
void ColorMap2D::save(XmlStreamWriter *xmlwriter)
{
    ...
    xmlwriter->writeAttribute(
            "width", QString::number(getcolormapscalewidth_colormap()));
    (getcolormapscale_colormap()->axis()->visible())
            ? xmlwriter->writeAttribute("visible", "true")
            : xmlwriter->writeAttribute("visible", "false");
    xmlwriter->writeAttribute(
            "offset",
            QString::number(getcolormapscale_colormap()->axis()->offset()));
    xmlwriter->writeAttribute(
            "from",
            QString::number(
                    getcolormapscale_colormap()->axis()->range().lower));
    xmlwriter->writeAttribute(
            "to",
            QString::number(
                    getcolormapscale_colormap()->axis()->range().upper));
    ...
    xmlwriter->writeAttribute(
            "padding",
            QString::number(
                    getcolormapscale_colormap()->axis()->labelPadding()));
    ...
    xmlwriter->writeAttribute(
            "in",
            QString::number(
                    getcolormapscale_colormap()->axis()->tickLengthIn()));
    xmlwriter->writeAttribute(
            "out",
            QString::number(
                    getcolormapscale_colormap()->axis()->tickLengthOut()));
    ...
    xmlwriter->writeAttribute(
            "in",
            QString::number(
                    getcolormapscale_colormap()->axis()->subTickLengthIn()));
    xmlwriter->writeAttribute(
            "out",
            QString::number(
                    getcolormapscale_colormap()->axis()->subTickLengthOut()));
    ...
    xmlwriter->writeAttribute(
            "padding",
            QString::number(
                    getcolormapscale_colormap()->axis()->tickLabelPadding()));
    xmlwriter->writeAttribute(
            "rotation",
            QString::number(
    ...
    xmlwriter->writeAttribute(
            "precision",
            QString::number(
                    getcolormapscale_colormap()->axis()->numberPrecision()));
...
bool ColorMap2D::load(XmlStreamReader *xmlreader)
{
    ...
    if (ok) {
        ...
        xmlreader->raiseWarning(
                tr("ColorMap2D gradient property setting error"));
    ...
    while (!xmlreader->atEnd()) {
        if (xmlreader->isEndElement() && xmlreader->name() == "colormap")
            break;
        ...
        if (xmlreader->isStartElement() && xmlreader->name() == "scale") {
            ...
            (ok) ? getcolormapscale_colormap()->setVisible(mapv)
                 : xmlreader->raiseWarning(tr(
                         "ColorMap2D mapscale visible property setting error"));
            // levelcount
            ...
            // scale type
            QString scaletype =
                    xmlreader->readAttributeString("scaletype", &ok);
            if (ok) {
                if (scaletype == "linear") {
            ...
            while (!xmlreader->atEnd()) {
                ...
                if (xmlreader->isStartElement() && xmlreader->name() == "pen") {
                    ...
                    if (ok) {
                        getcolormapscale_colormap()->axis()->setBasePen(
                                strokep);
                    } else
                        xmlreader->raiseWarning(tr(
                                "ColorMap2D scale pen property setting error"));
                }
            }
            // label property
            while (!xmlreader->atEnd()) {
                xmlreader->readNext();
                if (xmlreader->isEndElement() && xmlreader->name() == "label")
                    break;
                // label text
                QString lbltxt = xmlreader->readAttributeString("text", &ok);
                (ok) ? setname_colormap(lbltxt)
                     : xmlreader->raiseWarning(tr(
                             "ColorMap2D label text property setting error"));
                // label padding
                int lblpadding = xmlreader->readAttributeInt("padding", &ok);
                (ok) ? getcolormapscale_colormap()->axis()->setLabelPadding(
                        lblpadding)
                     : xmlreader->raiseWarning(tr("ColorMap2D label padding "
                ...
                while (!xmlreader->atEnd()) {
                    ...
                    if (xmlreader->isStartElement()
                        ...
                        if (ok) {
                            getcolormapscale_colormap()->axis()->setLabelFont(
                                    fontpair.first);
                            getcolormapscale_colormap()->axis()->setLabelColor(
                                    fontpair.second);
                        } else
                            xmlreader->raiseWarning(
                                    tr("ColorMap2D font & color property "
                                       "setting error"));
                    }
                }
            }
            while (!xmlreader->atEnd()) {
                xmlreader->readNext();
                if (xmlreader->isEndElement() && xmlreader->name() == "ticks")
                    break;
                ...
                if (ok)
                    ...
                    xmlreader->raiseWarning(tr(
                            "ColorMap2D tick visible property setting error"));
                // Ticks in
                int in = xmlreader->readAttributeInt("in", &ok);
                if (ok)
                    getcolormapscale_colormap()->axis()->setTickLengthIn(in);
                else
                    xmlreader->raiseWarning(tr("ColorMap2D Tick length in "
                                               "property setting error"));
                // Tick out
                ...
                if (ok)
                    ...
                    xmlreader->raiseWarning(tr("ColorMap2D Tick Length out "
                                               "property setting error"));
                while (!xmlreader->atEnd()) {
                    xmlreader->readNext();
                    if (xmlreader->isEndElement() && xmlreader->name() == "pen")
            ...
            while (!xmlreader->atEnd()) {
                ...
                (ok) ? getcolormapscale_colormap()->axis()->setSubTicks(
                        subtickvisible)
                     : xmlreader->raiseWarning(tr("ColorMap2D subtick visible "
                                                  "property setting error"));
                // Ticks in
                int in = xmlreader->readAttributeInt("in", &ok);
                (ok) ? getcolormapscale_colormap()->axis()->setSubTickLengthIn(
                        in)
                     : xmlreader->raiseWarning(tr("ColorMap2D subTick length "
                                                  "in property setting error"));
                // out length
                int out = xmlreader->readAttributeInt("out", &ok);
                (ok) ? getcolormapscale_colormap()->axis()->setSubTickLengthOut(
                        out)
                     : xmlreader->raiseWarning(
                             tr("ColorMap2D subTick Length out property "
                                "setting error"));
                while (!xmlreader->atEnd()) {
                    xmlreader->readNext();
                    if (xmlreader->isEndElement() && xmlreader->name() == "pen")
                    ...
                    if (xmlreader->isStartElement()
                        && xmlreader->name() == "pen") {
                        QPen p = xmlreader->readPen(&ok);
                        (ok) ? getcolormapscale_colormap()
                                        ->axis()
                                        ->setSubTickPen(p)
                             : xmlreader->raiseWarning(
                                     tr("ColorMap2D subtick pen property "
                                        "setting error"));
                    }
                }
            }
            ...
            while (!xmlreader->atEnd()) {
                ...
                (ok) ? getcolormapscale_colormap()->axis()->setTickLabels(
                        ticklabelvisible)
                     : xmlreader->raiseWarning(
                             tr("ColorMap2D tick label visible property "
                                "setting error"));
                // Tick label padding
                ...
                (ok) ? getcolormapscale_colormap()->axis()->setTickLabelPadding(
                        ticklabelpadding)
                     : xmlreader->raiseWarning(
                             tr("ColorMap2D tick label padding in property "
                                "setting error"));
                // Tick Label Rotation
                ...
                (ok) ? getcolormapscale_colormap()
                     ...
                     : xmlreader->raiseWarning(
                             tr("ColorMap2D tick label rotation property "
                                "setting error"));
                // Tick label side
                ...
                if (ok)
                    if (ticklabelside == "up")
                        ...
                        xmlreader->raiseWarning(tr("ColorMap2D tick label side "
                                                   "property setting error"));
                else
                    xmlreader->raiseWarning(tr("ColorMap2D tick label side "
                                               "property setting error"));
                ...
                if (ok)
                    if (ticklabelformat == "E")
                        setcolormapscaleticklabelformat_axis(
                                Axis2D::AxisLabelFormat::E);
                    else if (ticklabelformat == "G")
                        setcolormapscaleticklabelformat_axis(
                                Axis2D::AxisLabelFormat::G);
                    else if (ticklabelformat == "e")
                        setcolormapscaleticklabelformat_axis(
                                Axis2D::AxisLabelFormat::e);
                    else if (ticklabelformat == "eb")
                        setcolormapscaleticklabelformat_axis(
                                Axis2D::AxisLabelFormat::eb);
                    else if (ticklabelformat == "ebc")
                        setcolormapscaleticklabelformat_axis(
                                Axis2D::AxisLabelFormat::ebc);
                    else if (ticklabelformat == "f")
                        setcolormapscaleticklabelformat_axis(
                                Axis2D::AxisLabelFormat::f);
                    else if (ticklabelformat == "g")
                        setcolormapscaleticklabelformat_axis(
                                Axis2D::AxisLabelFormat::g);
                    else if (ticklabelformat == "gb")
                        setcolormapscaleticklabelformat_axis(
                                Axis2D::AxisLabelFormat::gb);
                    else if (ticklabelformat == "gbc")
                        setcolormapscaleticklabelformat_axis(
                                Axis2D::AxisLabelFormat::gbc);
                    else
                        xmlreader->raiseWarning(
                                tr("ColorMap2D tick label format property "
                                   "setting error"));
                else
                    xmlreader->raiseWarning(tr("ColorMap2D tick label format "
                                               "property setting error"));
                ...
                (ok) ? getcolormapscale_colormap()->axis()->setNumberPrecision(
                        ticklabelprecision)
                     : xmlreader->raiseWarning(
                             tr("ColorMap2D tick label precision property "
                                "setting error"));
                while (!xmlreader->atEnd()) {
                    xmlreader->readNext();
                    if (xmlreader->isEndElement()
                    ...
                    if (xmlreader->isStartElement()
                        ...
                        QPair<QFont, QColor> fontpair =
                                xmlreader->readFont(&ok);
                        if (ok) {
                            getcolormapscale_colormap()
                                    ->axis()
                                    ->setTickLabelFont(fontpair.first);
                            getcolormapscale_colormap()
                                    ->axis()
                                    ->setTickLabelColor(fontpair.second);
                        } else
                            xmlreader->raiseWarning(
                                    tr("ColorMap2D font & color property "
                                       "setting error"));
