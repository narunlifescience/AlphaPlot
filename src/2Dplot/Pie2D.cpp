                         ...
                         int to)
{
    ...
    for (int i = from; i <= to; i++) {
        ...
        PieLegendItem2D *pielegenditem = new PieLegendItem2D(
                axisrect_->getLegend(), color, xData->textAt(i));
...
AxisRect2D *Pie2D::getaxisrect() const
{
    return axisrect_;
}

Qt::PenStyle Pie2D::getstrokestyle_pieplot() const
{
    return mPen.style();
}

QColor Pie2D::getstrokecolor_pieplot() const
{
    return mPen.color();
}

double Pie2D::getstrokethickness_pieplot() const
{
    return mPen.widthF();
}

int Pie2D::getmarginpercent_pieplot() const
{
    return marginpercent_;
}

Graph2DCommon::PieStyle Pie2D::getStyle_pieplot() const
{
    return style_;
}
...
void Pie2D::draw(QCPPainter *painter)
{
    ...
    if (ellipseRect.width() > ellipseRect.height()) {
        ...
        double margin =
                (ellipseRect.height() * static_cast<double>(marginpercent_) / 2)
                / 100;
        ellipseRect.adjust(dif / 2 + margin, margin, -(dif / 2) - margin,
                           -margin);
    } else {
        double dif = ellipseRect.height() - ellipseRect.width();
        double margin =
                (ellipseRect.width() * static_cast<double>(marginpercent_) / 2)
                / 100;
                              ...
                              QBrush fillbrush)
{
    ...
    path.moveTo(x, y);
    path.arcTo(op1 + offset, op1 + offset, op2 + offset, op2 + offset,
               startangle, stopangle - startangle);
    path.arcTo(ip1 + offset, ip1 + offset, ip2 + offset, ip2 + offset,
...
bool Pie2D::load(XmlStreamReader *xmlreader)
{
    if (xmlreader->isStartElement() && xmlreader->name() == "pie") {
        ...
        if (ok) {
            if (style == "pie")
                ...
                xmlreader->raiseWarning(
                        tr("Pie2D style property setting error"));
        } else
        ...
        if (ok)
            ...
            xmlreader->raiseWarning(
                    tr("Pie2D marginpercent property setting error"));
        ...
        while (!xmlreader->atEnd()) {
            ...
            if (xmlreader->isStartElement() && xmlreader->name() == "pen") {
                ...
                if (ok)
                    ...
                    xmlreader->raiseWarning(
                            tr("Pie2D strokepen property setting error"));
            }
        }
