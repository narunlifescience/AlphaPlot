...
bool Legend2D::gethidden_legend() const
{
    return visible();
}
...
AxisRect2D *Legend2D::getaxisrect_legend() const
{
    return axisrect_;
}
...
void Legend2D::save(XmlStreamWriter *xmlwriter)
{
    ...
    xmlwriter->writeAttribute(
            "x",
            QString::number(
                    axisrect_->insetLayout()->insetRect(0).topLeft().x()));
    xmlwriter->writeAttribute(
            "y",
            QString::number(
                    axisrect_->insetLayout()->insetRect(0).topLeft().y()));
    xmlwriter->writeAttribute(
            "width",
            QString::number(axisrect_->insetLayout()->insetRect(0).width()));
    xmlwriter->writeAttribute(
            "height",
    ...
    xmlwriter->writeAttribute("iconheight",
    ...
    if (xmlreader->isStartElement() && xmlreader->name() == "legend") {
        ...
        if (ok) {
            ...
            if (ok) {
                ...
                if (ok) {
                    double height =
                            xmlreader->readAttributeDouble("height", &ok);
                    if (ok) {
                        QRectF rect;
        ...
        int icontextpadding =
                xmlreader->readAttributeInt("icontextpadding", &ok);
        (ok) ? setIconTextPadding(icontextpadding)
             : xmlreader->raiseError(tr(
                     "no Legend2D icon text padding property element found"));
        // font
            ...
            if (xmlreader->isStartElement() && xmlreader->name() == "font") {
                ...
                if (ok) {
                    ...
                    xmlreader->raiseWarning(
                            tr("Legend2D font property setting error"));
            }
        }
        // pen
        while (!xmlreader->atEnd()) {
        ...
        while (!xmlreader->atEnd()) {
            ...
            if (xmlreader->isStartElement() && xmlreader->name() == "brush") {
                ...
                if (ok)
                    ...
                    xmlreader->raiseWarning(
                            tr("Legend2D brush property setting error"));
            }
        }
    } else // no element
...
void Legend2D::mousePressEvent(QMouseEvent *event, const QVariant &details)
{
    ...
    if (event->button() == Qt::LeftButton) {
        if (selectTest(event->pos(), false) > 0) {
            ...
            // since insetRect is in axisRect coordinates (0..1), we transform
            // the mouse position:
            QPointF mousePoint(
                    (event->pos().x() - axisrect_->left())
                            / static_cast<double>(axisrect_->width()),
                    (event->pos().y() - axisrect_->top())
                            / static_cast<double>(axisrect_->height()));
            dragLegendOrigin_ = mousePoint
                    - axisrect_->insetLayout()->insetRect(0).topLeft();
...
void LegendItem2D::mousePressEvent(QMouseEvent *event, const QVariant &details)
{
...
{
    isspline_ =
            (plottable->getcurvetype_cplot() == Curve2D::Curve2DType::Spline)
            ? true
            : false;
}
