...
Axis2D *AxisRect2D::addAxis2DifNeeded(Column *col)
{
    ...
    if (col->plotDesignation() == AlphaPlot::X) {
        QList<Axis2D *> haxes =
                getAxes2D(Axis2D::Axis2D::AxisOreantation::Bottom);
        ...
        if (!axis) {
            int l = 0, r = 0;
            foreach (Axis2D *axis2d, vaxes) {
                (axis2d->getorientation_axis() == Axis2D::AxisOreantation::Left)
                        ? l++
                        : r++;
            }
            (r < l) ? axis = addAxis2D(Axis2D::AxisOreantation::Right,
                                       tickertype)
                    : axis = addAxis2D(Axis2D::AxisOreantation::Left,
                                       tickertype);
        }
    }
    return axis;
...
bool AxisRect2D::removeAxis2D(Axis2D *axis, bool force)
{
    ...
    foreach (LineSpecial2D *ls, lsvec_) {
        if (ls->getxaxis() == axis || ls->getyaxis() == axis)
            status = false;
    }

    foreach (Curve2D *curve, curvevec_) {
        if (curve->getxaxis() == axis || curve->getyaxis() == axis)
            status = false;
    }
    ...
    foreach (Vector2D *vec, vectorvec_) {
        if (vec->getxaxis() == axis || vec->getyaxis() == axis)
            status = false;
    }

    foreach (Bar2D *bar, barvec_) {
        if (bar->getxaxis() == axis || bar->getyaxis() == axis)
            status = false;
    }

    if (!status) {
        QMessageBox::warning(nullptr, tr("Axis associated with plot"),
                             tr("This axis is associated with a plot! eithor "
                                "remove the plot or "
...
QBrush AxisRect2D::getAxisRectBackground() const
{
    return axisRectBackGround_;
}
...
Grid2D *AxisRect2D::bindGridTo(Axis2D *axis)
{
    ...
    case Axis2D::AxisOreantation::Top:
        ...
        if (gridpair_.first.first != nullptr)
    ...
    case Axis2D::AxisOreantation::Right:
        ...
        if (gridpair_.second.first != nullptr)
...
void AxisRect2D::setstackbar()
{
    ...
    foreach (Bar2D *bar, barvec_) {
        if (bar->getstackposition_barplot() != -1)
            bvec << bar;
    }
    // sort the order
    if (!bvec.isEmpty()) {
...
QList<Axis2D *>
AxisRect2D::getAxes2D(const Axis2D::AxisOreantation &orientation) const
{
...
QList<Axis2D *> AxisRect2D::getXAxes2D() const
{
    QList<Axis2D *> axes2d = QList<Axis2D *>();
    for (int i = 0; i < axes_.size(); i++) {
        if (axes_.at(i)->getorientation_axis()
                    == Axis2D::AxisOreantation::Bottom
            || axes_.at(i)->getorientation_axis()
...
int AxisRect2D::getXAxisNo(Axis2D *axis)
{
    QList<Axis2D *> xaxes = getXAxes2D();
    if (axis
        && (axis->getorientation_axis() == Axis2D::AxisOreantation::Bottom
...
int AxisRect2D::getYAxisNo(Axis2D *axis)
{
    QList<Axis2D *> yaxes = getYAxes2D();
    if (axis
        && (axis->getorientation_axis() == Axis2D::AxisOreantation::Left
...
LineSpecial2D *
AxisRect2D::addLineSpecial2DPlot(const LineScatterSpecialType &type,
                                 ...
                                 int from, int to, Axis2D *xAxis, Axis2D *yAxis)
{
    ...
    case LineScatterSpecialType::CentralStepAndScatter2D:
        lineSpecial->setlinetype_lsplot(
                Graph2DCommon::LineStyleType::StepCenter);
        lineSpecial->setscattershape_lsplot(Graph2DCommon::ScatterStyle::Disc);
        lineSpecial->setlinefillstatus_lsplot(false);
        lineSpecial->setlineantialiased_lsplot(false);
        lineSpecial->setscatterantialiased_lsplot(true);
    ...
    case LineScatterSpecialType::HorizontalStep2D:
        lineSpecial->setlinetype_lsplot(
                Graph2DCommon::LineStyleType::StepRight);
        lineSpecial->setscattershape_lsplot(Graph2DCommon::ScatterStyle::None);
        lineSpecial->setlinefillstatus_lsplot(false);
        lineSpecial->setlineantialiased_lsplot(false);
        lineSpecial->setscatterantialiased_lsplot(true);
                                    ...
                                    Axis2D *yAxis)
{
    ...
    case LineScatterType::LineAndScatter2D:
        curve = new Curve2D(Curve2D::Curve2DType::Curve, table, xcol, ycol,
                            from, to, xAxis, yAxis);
        break;
    case LineScatterType::Spline2D:
        curve = new Curve2D(Curve2D::Curve2DType::Spline, table, xcol, ycol,
                            from, to, xAxis, yAxis);
        break;
    }
                                    ...
                                    Axis2D *xAxis, Axis2D *yAxis)
{
    ...
    VectorLegendItem2D *legendItem =
            new VectorLegendItem2D(axisRectLegend_, vec);
    axisRectLegend_->addItem(legendItem);
    connect(legendItem, SIGNAL(legendItemClicked()), SLOT(legendClick()));
...
TextItem2D *AxisRect2D::addTextItem2D(QString text)
{
    ...
    textitem->position->setAxes(gridpair_.first.second,
                                gridpair_.second.second);
...
QList<Axis2D *>
AxisRect2D::getAxesOrientedTo(const Axis2D::AxisOreantation &orientation) const
{
...
void AxisRect2D::selectAxisRect()
{
        ...
        if (layers_.at(i) == layer) {
            ...
            case QCustomPlot::LayerInsertMode::limAbove:
                if (i + 1 < layers_.size()) {
                    ...
                    qDebug() << "unable to move layer(s). this layer is "
                                "already the "
                                "top layer";
                }
                break;
    ...
    for (int i = 0; i < layers_.size(); i++) {
        foreach (auto channel, channelvec_) {
            if (channel.first->layer() == layer) {
                layermoved = parentPlot()->moveLayer(
                        channel.second->layer(), layer,
                        QCustomPlot::LayerInsertMode::limBelow);
                if (!layermoved)
                    qDebug() << "unable to move channel layer(s). error ";
            } else if (channel.first->layer() == layerswap) {
                layermoved = parentPlot()->moveLayer(
                        channel.second->layer(), layerswap,
                        QCustomPlot::LayerInsertMode::limBelow);
                if (!layermoved)
                    qDebug() << "unable to move channel layer(s). error ";
            }
...
void AxisRect2D::setGraphTool(const Graph2DCommon::Picker &picker)
{
    picker_ = picker;
    foreach (LineSpecial2D *ls, lsvec_) {
        ls->setpicker_lsplot(picker);
    }
    foreach (Curve2D *curve, curvevec_) {
        curve->setpicker_cplot(picker);
    }
    foreach (Bar2D *bar, barvec_) {
        bar->setpicker_barplot(picker);
    }
    foreach (StatBox2D *statbox, statboxvec_) {
                                          ...
                                          QList<Table *> tabs)
{
    ...
    if (xmlreader->isStartElement() && xmlreader->name() == "channel") {
        ...
        while (!xmlreader->atEnd()) {
            ...
            if (xmlreader->isStartElement()
                ...
                if (ok) {
                    (table1) ? xcolumn1 = table1->column(xcolname)
                             : xcolumn1 = nullptr;
                } else
                    xmlreader->raiseError(tr("LineSpecialChannel2D Table X "
                                             "column not found error"));
                QString ycolname =
                        xmlreader->readAttributeString("ycolumn", &ok);
                if (ok) {
                    (table1) ? ycolumn1 = table1->column(ycolname)
                             : ycolumn1 = nullptr;
                } else
                    xmlreader->raiseError(tr("LineSpecialChannel2D Table Y "
                                             "column not found error"));
                ...
                if (!ok)
                    xmlreader->raiseError(
                            tr("LineSpecialChannel2D to not found error"));
                ...
                if (xmlreader->isStartElement()
                    ...
                    if (ok) {
                        ...
                        xmlreader->raiseWarning(
                                tr("LineSpecialChannel2D line style property "
                                   "setting error"));
                    ...
                    if (!ok)
                        xmlreader->raiseWarning(
                                tr("LineSpecialChannel2D line fill status "
                                   "property setting "
                                   "error"));

                    // line antialias
                    lineantialias1 =
                            xmlreader->readAttributeBool("antialias", &ok);
                    if (!ok)
                        xmlreader->raiseWarning(
                    ...
                    while (!xmlreader->atEnd()) {
                        ...
                        if (xmlreader->isStartElement()
                            ...
                            if (!ok)
                                xmlreader->raiseWarning(
                                        tr("LineSpecialChannel2D line pen "
                                           "property setting error"));
                        }
                    }
                    ...
                    while (!xmlreader->atEnd()) {
                        ...
                        if (xmlreader->isStartElement()
                            ...
                            if (!ok)
                                xmlreader->raiseWarning(
                                        tr("LineSpecialChannel2D linebrush "
                                           "property setting error"));
                        }
                    }
                }
                ...
                // scatter
                if (xmlreader->isStartElement()
                    && xmlreader->name() == "scatter") {
                    // scatter shape
                    QString scattershape =
                            xmlreader->readAttributeString("style", &ok);
                    if (ok) {
                        if (scattershape == "dot") {
                            ...
                            scatterstyle1 =
                                    Graph2DCommon::ScatterStyle::Diamond;
                        } else if (scattershape == "triangle") {
                            scatterstyle1 =
                                    Graph2DCommon::ScatterStyle::Triangle;
                        } else if (scattershape == "pluscircle") {
                            scatterstyle1 =
                                    Graph2DCommon::ScatterStyle::PlusCircle;
                        } else if (scattershape == "plussquare") {
                            scatterstyle1 =
                                    Graph2DCommon::ScatterStyle::PlusSquare;
                        } else if (scattershape == "crosscircle") {
                            scatterstyle1 =
                                    Graph2DCommon::ScatterStyle::CrossCircle;
                        } else if (scattershape == "crosssquare") {
                            scatterstyle1 =
                                    Graph2DCommon::ScatterStyle::CrossSquare;
                        } else if (scattershape == "triangleinverted") {
                            scatterstyle1 = Graph2DCommon::ScatterStyle::
                                    TriangleInverted;
                        }
                    } else
                        xmlreader->raiseWarning(
                                tr("LineSpecialChannel2D scatter shape "
                                   "property setting error"));
                    ...
                    if (!ok)
                        xmlreader->raiseWarning(
                                tr("LineSpecialChannel2D scatter size property "
                                   "setting error"));
                    ...
                    if (!ok)
                        xmlreader->raiseWarning(
                                tr("LineSpecialChannelD scatter antialias "
                                   "property setting "
                                   "error"));
                    ...
                    while (!xmlreader->atEnd()) {
                        ...
                        if (xmlreader->isStartElement()
                            ...
                            if (!ok)
                                xmlreader->raiseWarning(
                                        tr("LineSpecialChannel2D scatter pen "
                                           "property setting error"));
                        }
                    }
                    ...
                    while (!xmlreader->atEnd()) {
                        ...
                        if (xmlreader->isStartElement()
                            ...
                            if (!ok)
                                xmlreader->raiseWarning(
                                        tr("LineSpecialChannel2D scatterbrush "
                                           "property setting "
                                           "error"));
                        }
                    }
        ...
        while (!xmlreader->atEnd()) {
            ...
            if (xmlreader->isStartElement()
                ...
                QString xcolname =
                        xmlreader->readAttributeString("xcolumn", &ok);
                if (ok) {
                    (table2) ? xcolumn2 = table2->column(xcolname)
                             : xcolumn2 = nullptr;
                } else
                    xmlreader->raiseError(tr("LineSpecialChannel2D Table X "
                                             "column not found error"));
                QString ycolname =
                        xmlreader->readAttributeString("ycolumn", &ok);
                if (ok) {
                    (table2) ? ycolumn2 = table2->column(ycolname)
                             : ycolumn2 = nullptr;
                } else
                    xmlreader->raiseError(tr("LineSpecialChannel2D Table Y "
                                             "column not found error"));
                ...
                if (xmlreader->isStartElement()
                    ...
                    if (ok) {
                        if (scattershape == "dot") {
                            ...
                            scatterstyle2 = Graph2DCommon::ScatterStyle::
                                    TriangleInverted;
                        }
                    } else
                        xmlreader->raiseWarning(
    ...
    if (table1 && table2 && xcolumn1 && xcolumn2 && ycolumn1 && ycolumn2
        ...
        QPair<LineSpecial2D *, LineSpecial2D *> lspair =
                addLineSpecialChannel2DPlot(table1, xcolumn1, ycolumn1,
                                            ycolumn2, from1, to1, xaxis1,
                                            yaxis1);
        // ls1
    ...
    foreach (QCPLayer *layer, layers_) {
        ...
        for (int i = 0; i < chvec.size(); i++) {
            if (layer == chvec.at(i).first->layer()) {
                ...
                chvec.at(i).first->save(
                        xmlwriter, getXAxisNo(chvec.at(i).first->getxaxis()),
                        getYAxisNo(chvec.at(i).first->getyaxis()));
                chvec.at(i).second->save(
                        xmlwriter, getXAxisNo(chvec.at(i).second->getxaxis()),
        ...
        while (!xmlreader->atEnd()) {
            ...
            if (xmlreader->isStartElement() && xmlreader->name() == "brush") {
                ...
                if (ok)
                    ...
                    xmlreader->raiseWarning(
                            tr("Layout brush property setting error"));
            }
        }
        while (!xmlreader->atEnd()) {
            xmlreader->readNext();
            if (xmlreader->isEndElement() && xmlreader->name() == "layout")
                break;
            if (xmlreader->isStartElement() && xmlreader->name() == "axis") {
                ...
                // Type property
                QString position =
                        xmlreader->readAttributeString("position", &ok);
                if (ok) {
                    if (position == "left")
                        ...
                        xmlreader->raiseError(tr("(critical) Axis2D Position "
                                                 "property setting error"));
                } else
                    xmlreader->raiseError(tr("(critical) Axis2D Position "
                                             "property setting error"));
                // Tickertype property
                QString ticker =
                        xmlreader->readAttributeString("tickertype", &ok);
                if (ok) {
                    if (ticker == "value")
                        tickertype = Axis2D::TickerType::Value;
                    else if (ticker == "log")
                        ...
                        xmlreader->raiseError(tr("(critical) Axis2D Tickertype "
                                                 "property setting error"));
                    }
                } else
                    xmlreader->raiseError(tr("(critical) Axis2D Tickertype "
                ...
                if (ok && associatedgrid) {
                    ...
                    xmlreader->raiseWarning(
                            tr("Axis2D unable to set Grid2D error"));
                axis->load(xmlreader);
            } else
                    // xgrid
                    if (xmlreader->isStartElement()
                        && xmlreader->name() == "xgrid") {
                gridpair_.first.first->load(xmlreader, "xgrid");
            } else
                    // ygrid
                    if (xmlreader->isStartElement()
                        && xmlreader->name() == "ygrid") {
                ...
                if (!ok)
                    xmlreader->raiseWarning(tr("Curve2D legendtext not found"));
                ...
                if (ok && datatype == "table") {
                    ...
                    while (!xmlreader->atEnd()) {
                        ...
                        if (xmlreader->isStartElement()
                            && xmlreader->name() == "data") {
                            double xval = xmlreader->readAttributeDouble(
                                    "xdata", &ok);
                            bool xok = ok;
                            double yval = xmlreader->readAttributeDouble(
                                    "ydata", &ok);
                            if (xok && ok) {
                                xdata->append(xval);
                                ydata->append(yval);
                    ...
                    if (xdata->size() > 0 && ydata->size() > 0
                        ...
                        curve = addFunction2DPlot(xdata, ydata, xaxis, yaxis,
                                                  legend);
                        curve->setlegendtext_cplot(legend);
                    } else {
                        xmlreader->raiseError(
                    ...
                    if (xmlreader->isStartElement()
                        ...
                        if (table && column && !type.isEmpty()
                            ...
                            if (type == "x") {
                                ...
                                curve->getxerrorbar_curveplot()->load(
                                        xmlreader);
                            } else if (type == "y") {
                                curve->setYerrorBar(table, column, from, to);
                                curve->getyerrorbar_curveplot()->load(
                                        xmlreader);
                            }
                        }
                    }
                ...
                if (table && xcolumn && ycolumn && xaxis && yaxis) {
                    ls = addLineSpecial2DPlot(ltype, table, xcolumn, ycolumn,
                                              from, to, xaxis, yaxis);
                    ls->setName(legend);
                    ...
                    while (!xmlreader->atEnd()) {
                        ...
                        if (xmlreader->isStartElement()
                            ...
                            QString tablename = xmlreader->readAttributeString(
                                    "table", &ok);
                            if (ok) {
                                table = getTableByName(tabs, tablename);
                            } else
                            ...
                            if (ok) {
                                ...
                                xmlreader->raiseError(
                                        tr("ErrorBar2D Table column not found "
                                           "error"));
                            int from = xmlreader->readAttributeInt("from", &ok);
                            if (!ok)
                                xmlreader->raiseError(
                ...
                QString legend = xmlreader->readAttributeString("legend", &ok);
                if (!ok)
                    xmlreader->raiseWarning(tr("Bar2D legendtext not found"));
                ...
                if (bartype == "barxy") {
                    ...
                    int from = xmlreader->readAttributeInt("from", &ok);
                    if (!ok)
                        xmlreader->raiseError(tr("Bar2D from not found error"));
                    int to = xmlreader->readAttributeInt("to", &ok);
                    if (!ok)
                        xmlreader->raiseError(tr("Bar2D to not found error"));
                    int stackorder =
                    ...
                    if (table && xcolumn && ycolumn && xaxis && yaxis) {
                        bar = addBox2DPlot(barorientation, table, xcolumn,
                                           ycolumn, from, to, xaxis, yaxis,
                                           stackorder);
                    }
                    ...
                    if (table && column && xaxis && yaxis) {
                        bar = addHistogram2DPlot(barorientation, table, column,
                                                 from, to, xaxis, yaxis);
                    }
                } else
                ...
                if (ok) {
                    ...
                    xmlreader->raiseError(
                            tr("Vector2D Table X1 column not found error"));
                QString y1colname =
                ...
                if (ok) {
                    ...
                    xmlreader->raiseError(
                            tr("Vector2D Table Y1 column not found error"));
                ...
                if (ok) {
                    ...
                    xmlreader->raiseError(
                            tr("Vector2D Table X2 column not found error"));
                QString y2colname =
                ...
                if (ok) {
                    ...
                    xmlreader->raiseError(
                            tr("Vector2D Table Y2 column not found error"));

                int from = xmlreader->readAttributeInt("from", &ok);
                if (!ok)
                    xmlreader->raiseError(tr("Vector2D from not found error"));
                int to = xmlreader->readAttributeInt("to", &ok);
                if (!ok)
                    xmlreader->raiseError(tr("Vector2D to not found error"));

                if (table && x1column && y1column && x2column && y2column
                    && xaxis && yaxis) {
                    Vector2D *curve = addVectorPlot(
                            type, table, x1column, y1column, x2column, y2column,
                            from, to, xaxis, yaxis);
                    curve->load(xmlreader);
                }
            } else
                ...
                int from = xmlreader->readAttributeInt("from", &ok);
                if (!ok)
                    xmlreader->raiseError(tr("Pie2D from not found error"));
                int to = xmlreader->readAttributeInt("to", &ok);
                if (!ok)
                    xmlreader->raiseError(tr("Pie2D to not found error"));
                if (table && xcolumn) {
                    Pie2D *pie =
                            addPie2DPlot(Graph2DCommon::PieStyle::Pie, table,
            ...
            if (xmlreader->isStartElement() && xmlreader->name() == "statbox") {
                ...
                if (ok) {
                    ...
                    xmlreader->raiseError(
                            tr("StatBox2D X axis not found error"));
                int yax = xmlreader->readAttributeInt("yaxis", &ok);
                if (ok) {
                    yaxis = getYAxis(yax);
                } else
                    xmlreader->raiseError(
                            tr("StatBox2D Y axis not found error"));

                QString legendtext =
                        xmlreader->readAttributeString("legend", &ok);
                if (!ok)
                    xmlreader->raiseWarning(
                            tr("StatBox2D legend text not found error"));
                ...
                if (ok) {
                    ...
                    xmlreader->raiseError(
                            tr("StatBox2D Table not found error"));
                ...
                if (ok) {
                    ...
                    xmlreader->raiseError(
                            tr("StatBox2D Table column not found error"));

                int from = xmlreader->readAttributeInt("from", &ok);
                if (!ok)
                    xmlreader->raiseError(tr("StatBox2D from not found error"));
                int to = xmlreader->readAttributeInt("to", &ok);
                if (!ok)
                    xmlreader->raiseError(tr("StatBox2D to not found error"));
                int key = xmlreader->readAttributeInt("key", &ok);
                if (!ok)
                    xmlreader->raiseError(tr("StatBox2D key not found error"));

                if (table && column && xaxis && yaxis) {
                    Layout2D *lout = qobject_cast<Layout2D *>(
                            plot2d_->parent()->parent());
                    if (lout) {
                        StatBox2D::BoxWhiskerData sbdata =
                                lout->generateBoxWhiskerData(table, column,
                                                             from, to, key);
                        QSharedPointer<QCPAxisTickerText> textTicker =
                                qSharedPointerCast<QCPAxisTickerText>(
                                        xaxis->getticker_axis());
                        StatBox2D *statbox =
                                addStatBox2DPlot(sbdata, xaxis, yaxis);
                ...
                if (ok) {
                    ...
                    xmlreader->raiseError(
                            tr("ColorMap2D Matrix not found error"));

                if (matrix) {
                    ColorMap2D *colmap = addColorMap2DPlot(
                            matrix, getXAxes2D().at(0), getYAxes2D().at(0));
...
void AxisRect2D::draw(QCPPainter *painter)
{
    ...
    if (printorexportjob_)
        return;
    if (isAxisRectSelected_)
        drawSelection(painter);
}

void AxisRect2D::legendClick()
{
    emit AxisRectClicked(this);
}
