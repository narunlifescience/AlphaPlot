...
ImageItem2D::ImageItem2D(AxisRect2D *axisrect, Plot2D *plot,
    ...
    : QCPItemPixmap(plot),
      axisrect_(axisrect),
      layername_(QString("<Image2DItem>")
...
AxisRect2D *ImageItem2D::getaxisrect() const
{
    return axisrect_;
}

QString ImageItem2D::getsource_imageitem() const
{
    return imagefilename_;
}

QColor ImageItem2D::getstrokecolor_imageitem() const
{
    return pen().color();
}
...
void ImageItem2D::save(XmlStreamWriter *xmlwriter)
{
    ...
    xmlwriter->writeAttribute(
            "x", QString::number(position("topLeft")->coords().x()));
    xmlwriter->writeAttribute(
            "y", QString::number(position("topLeft")->coords().y()));
        ...
        if (ok) {
            ...
            if (ok) {
                ...
                xmlreader->raiseWarning(tr(
                        "ImageItem2D set position property y setting error"));
        } else
...
void ImageItem2D::mouseReleaseEvent(QMouseEvent *event, const QPointF &startPos)
{
    Q_UNUSED(startPos);
    if (event->button() == Qt::LeftButton) {
        if (draggingimageitem_) {
