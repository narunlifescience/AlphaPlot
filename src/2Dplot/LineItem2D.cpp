...
AxisRect2D *LineItem2D::getaxisrect() const
{
    return axisrect_;
}

QColor LineItem2D::getstrokecolor_lineitem() const
{
    return pen().color();
}
...
LineItem2D::getendstyleenum_lineitem(const QString end) const
{
...
void LineItem2D::save(XmlStreamWriter *xmlwriter)
{
    ...
    xmlwriter->writeAttribute("startstyle",
                              getendstylestring_lineitem(getendstyle_lineitem(
                                      LineEndLocation::Start)));
    xmlwriter->writeAttribute("startwidth",
                              QString::number(starting_->width()));
    xmlwriter->writeAttribute("startlength",
                              QString::number(starting_->length()));
    xmlwriter->writeAttribute("endstyle",
                              getendstylestring_lineitem(getendstyle_lineitem(
                                      LineEndLocation::Stop)));
            ...
            if (ok) {
                ...
                if (ok) {
                    ...
                    if (ok) {
                        ...
                        xmlreader->raiseWarning(tr("LineItem2D set position y2 "
                                                   "property setting error"));
                } else
                    xmlreader->raiseWarning(tr("LineItem2D set position x2 "
                                               "property setting error"));
            } else
                xmlreader->raiseWarning(tr(
                        "LineItem2D set position y1 property setting error"));
        } else
...
void LineItem2D::mouseReleaseEvent(QMouseEvent *event, const QPointF &startPos)
{
    Q_UNUSED(startPos);
    if (event->button() == Qt::LeftButton) {
        lineitemclicked_ = false;
