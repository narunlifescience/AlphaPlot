                                         ...
                                         const int to)
{
    ...
    if (end_row >= xcol->rowCount())
        end_row = xcol->rowCount() - 1;
    if (end_row >= ycol->rowCount())
        end_row = ycol->rowCount() - 1;
    ...
    for (int i = associateddata_->from; i < associateddata_->to + 1; i++) {
        if (associateddata_->xcol->valueAt(i) == key) {
            if (associateddata_->ycol->valueAt(i) == value) {
                associateddata_->xcol->asStringColumn()->setTextAt(i,
                                                                   QString());
                associateddata_->ycol->asStringColumn()->setTextAt(i,
                                                                   QString());
                return true;
            }
        }
                                       ...
                                       const int to)
{
    ...
    // strip unused end rows
    if (end_row >= xcolumn->rowCount())
        end_row = xcolumn->rowCount() - 1;
    if (end_row >= ycolumn->rowCount())
        end_row = ycolumn->rowCount() - 1;
                                         ...
                                         const int from, const int to)
{
    ...
    // strip unused end rows
    if (end_row >= errorcolumn->rowCount())
        end_row = errorcolumn->rowCount() - 1;
                                        ...
                                        const int from, const int to)
{
    ...
    QScopedArrayPointer<double> X(
            new double[n]); // stores ranges (x) and bins (y)
    Y.resize(n);
