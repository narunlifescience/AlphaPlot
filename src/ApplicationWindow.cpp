...
#include "project/AprojHandler.h"
      ...
      multiPeakfittype_(0)
{
    ...
    connect(d_project->undoStack(), SIGNAL(canUndoChanged(bool)),
            ui_->actionUndo, SLOT(setEnabled(bool)));
    connect(d_project->undoStack(), SIGNAL(canRedoChanged(bool)),
...
// Scripting console window
    ...
    connect(ui_->actionImportASCII, SIGNAL(triggered()), this,
            SLOT(importASCII()));
    connect(ui_->actionQuit, SIGNAL(triggered()), qApp,
    ...
    connect(ui_->actionShowEditToolbar, SIGNAL(toggled(bool)), editToolbar,
            SLOT(setVisible(bool)));
    connect(ui_->actionShowGraphToolbar, SIGNAL(toggled(bool)),
            graphToolsToolbar, SLOT(setVisible(bool)));
...
// Scripting menu
    ...
    // Plot menu
    connect(d_plot_mapper, SIGNAL(mapped(int)), this,
            SLOT(selectPlotType(int)));
    ...
    connect(ui_->actionPlot2DHorizontalSteps, SIGNAL(triggered()),
            d_plot_mapper, SLOT(map()));
    ...
    connect(ui_->actionPlot3DRibbon, SIGNAL(triggered()), this,
            SLOT(plot3DRibbon()));
    connect(ui_->actionPlot3DBar, SIGNAL(triggered()), this,
            SLOT(plot3DBars()));
    ...
    connect(ui_->actionDuplicateWindow, SIGNAL(triggered()), this,
            SLOT(clone()));
...
#endif
    connect(ui_->actionHomepage, SIGNAL(triggered()), this,
            SLOT(showHomePage()));
#ifdef SEARCH_FOR_UPDATES
...
#endif
    connect(ui_->actionVisitForum, SIGNAL(triggered()), this,
            SLOT(showForums()));
    ...
    connect(actionInvertMatrix, SIGNAL(triggered()), this,
            SLOT(invertMatrix()));
    ...
    if (subwindow) {
        ...
        if (isActiveSubWindow(subwindow, SubWindowType::Plot2DSubWindow)) {
            ...
            connect(ui_->actionExecuteAll, SIGNAL(triggered()), subwindow,
                    SLOT(executeAll()));
            connect(ui_->actionEvaluateExpression, SIGNAL(triggered()),
                    subwindow, SLOT(evaluate()));
        } else
...
void ApplicationWindow::plotVectXYXY()
{
    ...
    if (list.count() == 4) {
        ...
        layout->generateVector2DPlot(
                Vector2D::VectorPlot::XYXY, table, xcol, ycollist.at(0),
...
void ApplicationWindow::plotVectXYAM()
{
    ...
    if (s.count() == 4) {
        ...
        Layout2D *layout = newGraph2D();
        layout->generateVector2DPlot(
                Vector2D::VectorPlot::XYAM, table, xcol, ycollist.at(0),
                ycollist.at(1), ycollist.at(2), table->firstSelectedRow(),
                table->firstSelectedRow() + table->selectedRowCount() - 1);
    } else
        QMessageBox::warning(
...
void ApplicationWindow::add3DData()
{
    ...
    if (static_cast<int>(zColumns.count()) <= 0) {
        QMessageBox::critical(this, tr("Warning"),
                              tr("There are no available columns with plot "
                                 "designation set to Z!"));
        return;
    }
...
void ApplicationWindow::change3DMatrix()
{
    ...
    // Layout3D *layout = qobject_cast<Layout3D
    // *>(d_workspace->activeSubWindow()); if (layout && layout->getMatrix())
...
void ApplicationWindow::change3DMatrix(const QString &matrix_name)
{
    if (!isActiveSubwindow(SubWindowType::Plot3DSubWindow))
        return;
    // Layout3D *layout = qobject_cast<Layout3D
    // *>(d_workspace->activeSubWindow()); Matrix *mat = matrix(matrix_name); if
    // (m && g) g->changeMatrix(m);
    emit modified();
}
...
Layout3D *ApplicationWindow::dataPlotXYZ(Table *table,
                                         const Graph3DCommon::Plot3DType &type)
{
...
Matrix *ApplicationWindow::importImage()
{
    ...
    QString fn = QFileDialog::getOpenFileName(
            this, tr("Import image from file"), imagesDirPath, filter);
    if (!fn.isEmpty()) {
        QFileInfo fi(fn);
        ...
        const AbstractAspect *aspect)
{
    ...
    foreach (QMdiSubWindow *subwindow, subwindowlist) {
        TableStatistics *table_stat =
                qobject_cast<TableStatistics *>(subwindow);
        if (!table_stat)
            continue;
    ...
    if (!axisrect) {
        QMessageBox::warning(
                ...
                tr("<h4>There are no suitable layout/plot available in this "
                   "window.</h4><p><h4>Please add a layout/plot and try "
                   "again!</h4>"));
        return;
    }
...
ApplicationWindow *ApplicationWindow::plotFile(const QString &fn)
{
    ...
    Table *t =
            app->newTable(fn, app->columnSeparator, 0, true, app->strip_spaces,
...
void ApplicationWindow::importASCII()
{
    ImportASCIIDialog *import_dialog = new ImportASCIIDialog(
            isActiveSubwindow(SubWindowType::TableSubWindow), this,
                                    ...
                                    QLocale local_numeric_locale)
{
    ...
    // this is very much a special case, and thus is handled completely in its
    // own block
    ...
    foreach (QString file, files) {
        Table *temp = new Table(scriptEnv, file, local_column_separator,
                                local_ignored_lines, local_rename_columns,
                                local_strip_spaces, local_simplify_spaces,
                                local_convert_to_numeric, local_numeric_locale,
                                "temp", 0, 0, 0);
        ...
        if (import_mode == ImportASCIIDialog::NewRows
            ...
            if (local_convert_to_numeric) {
                for (int col = 0;
                     col < qMin(temp->columnCount(), table->columnCount());
                     col++)
                    if (table->column(col)->columnMode()
                        != AlphaPlot::Numeric) {
                        QMessageBox::critical(
                                this, tr("ASCII Import Failed"),
        ...
        case ImportASCIIDialog::NewColumns:
            while (temp->d_future_table->childCount() > 0)
                temp->d_future_table->reparentChild(
                        table->d_future_table, temp->d_future_table->child(0));
            break;
        case ImportASCIIDialog::NewRows: {
            ...
            for (int col = 0; col < missing_columns; col++) {
                Column *new_col = new Column(
                        ...
                        local_convert_to_numeric ? AlphaPlot::Numeric
                                                 : AlphaPlot::Text);
            ...
            table->d_future_table->setRowCount(table->rowCnt()
                                               + temp->rowCnt());
        ...
        case ImportASCIIDialog::Overwrite: {
            ...
            for (int col = 0;
                 col < table->columnCount() && col < temp->columnCount();
                 col++) {
                Column *src_col = temp->column(col);
                Column *dst_col = table->column(col);
                Q_ASSERT(src_col->dataType() == dst_col->dataType());
        ...
        if (projectname != "untitled") {
            ...
            if (fileName == pn) {
                QMessageBox::warning(
                        ...
                        tr("The file: <b>%1</b> is the current file!")
                                .arg(fileName));
                return;
            }
        }
...
void ApplicationWindow::openRecentAproj()
{
    ...
    if (projectname != "untitled") {
        ...
        if (filename == pathname) {
            QMessageBox::warning(
                    this, tr("File opening error"),
                    tr("The file: <p><b> %1 </b><p> is the current file!")
                            .arg(filename));
            return;
        }
    }
                                    ...
                                    const QString &scriptName, int lineNumber)
{
    ...
    if (consoleWindow->hasFocus()) {
        ...
        QMessageBox::critical(
                this, tr("AlphaPlot") + " - " + tr("Script Error"), message);
    }
}
        ...
        if (fn.contains(".qmt", Qt::CaseSensitive)
            ...
            if ((fileType != "AlphaPlot") && (fileType != "QtiPlot")) {
                QMessageBox::critical(this, tr("File opening error"),
                                      tr("The file: <b> %1 </b> was not "
                                         "created using AlphaPlot!")
                                              .arg(fn));
                return;
            }
            ...
            if (templateType == "<SurfacePlot>") {
                ...
                *>(w)->setAlignement(strlst[1].toInt(), strlst[2].toInt());
                    while (!t.atEnd()) {  // open layers
                      ...
                      if (s.left(7) == "<graph>") {
                        ...
                        openGraphAproj(this, qobject_cast<MultiLayer *>(w),
                lst);
                      }
                    }
                  }
...
#endif
    appLanguage = settings.value("Language",
...
// Follows an ugly hack to fix Qt4 porting issues (only needed on Windows)
    ...
    if (applicationFont.size() == 4)
        appFont = QFont(applicationFont.at(0), applicationFont.at(1).toInt(),
                        applicationFont.at(2).toInt(),
                        applicationFont.at(3).toInt());

    settings.beginGroup("Dialogs");
    d_extended_open_dialog =
            settings.value("ExtendedOpenDialog", true).toBool();
...
#    endif
...
#ifdef Q_OS_WIN
    templatesDir = settings.value("TemplatesDir", qApp->applicationDirPath())
                           .toString();
    asciiDirPath =
    ...
    legendBackground.setAlpha(settings.value("Transparency", 0)
                                      .toInt()); // transparent by default;
    ...
    if (plot3DFonts.size() == 12) {
        ...
        plot3DNumbersFont =
                QFont(plot3DFonts[4], plot3DFonts[5].toInt(),
    ...
    pasteFitResultsToPlot =
            settings.value("PasteResultsToPlot", false).toBool();
    writeFitResultsToLog = settings.value("WriteResultsToLog", true).toBool();
    generateUniformFitPoints =
            settings.value("GenerateFunction", true).toBool();
    ...
    d_export_transparency =
            settings.value("ExportTransparency", false).toBool();
    ...
    if (isActiveSubWindow(w, SubWindowType::Plot2DSubWindow)) {
        ...
        if (!plot2D->getCurrentAxisRect()) {
            QMessageBox::critical(this, tr("Export Error"),
                                  tr("<h4>There are no plot layouts available "
                                     "in this window!</h4>"));
            return;
        } else {
            plot2D->exportGraph();
        ...
        if (isActiveSubWindow(w, SubWindowType::Plot2DSubWindow)) {
            ...
            if (!plot2D->getCurrentAxisRect()) {
                ...
                QMessageBox::warning(this, tr("Warning"),
                                     tr("There are no plot layers available in "
                                        "window <b>%1</b>.<br>"
        ...
        if (!f.open(QIODevice::WriteOnly)) {
            ...
            QMessageBox::critical(
                    ...
                    tr("Could not write to file: <br><h4>%1</h4><p>"
                       "Please verify that you have the right to write to this "
                       "location!")
                            .arg(file_name));
            return;
        }
        if (file_suffix.contains(".eps") || file_suffix.contains(".pdf")
            ...
            for (int i = 0; i < static_cast<int>(list.count()); i++) {
                if (file_suffix.contains("." + (list[i]).toLower())) {
                    if (plot3D)
                        plot3D->exportGraphwithoutdialog(
                                file_name, file_suffix,
                                plot3D->getContainerSize());
                    if (plot2D)
                        plot2D->exportGraphwithoutdialog(file_name,
                                                         file_suffix);
                    ;
                }
            }
...
void ApplicationWindow::saveProjectAs()
{
    ...
    filter += QString(tr("Compressed AlphaPlot project %1"))
                      .arg("(*.aproj.gz);;");
...
void ApplicationWindow::saveAsTemplate()
{
    if (!d_workspace->activeSubWindow())
        return;
    MyWidget *mywidget =
            qobject_cast<MyWidget *>(d_workspace->activeSubWindow());
    if (!mywidget)
        return;
    ...
    if (!fn.isEmpty()) {
        ...
        if (!f.open(QIODevice::WriteOnly)) {
            QMessageBox::critical(
                    this, tr("Export Error"),
                    tr("Could not write to file: <br><h4> %1 </h4><p>Please "
                       "verify that "
    ...
    while (alreadyUsedName(newName)) {
        QMessageBox::critical(
                ...
                tr("Name <b>%1</b> already exists!").arg(newName) + "<p>"
                        ...
                        + tr("Warning: for internal consistency reasons only "
                             "alphabets "
...
QList<QPair<Table *, Column *>>
ApplicationWindow::columnList(AlphaPlot::PlotDesignation plotType)
{
    QList<QMdiSubWindow *> subwindowlist = subWindowsList();
...
QList<QPair<Table *, Column *>>
ApplicationWindow::columnList(Folder *folder,
    ...
    if (!axisrect) {
        QMessageBox::warning(this, tr("Warning"),
                             tr("<h4>There are no plot layout elements "
                                ...
                                "<p><h4>Please add/select a layout element and "
                                "try again!</h4>"));
        return;
    }
...
void ApplicationWindow::showExportASCIIDialog()
{
    ...
    if (table) {
        ExportDialog *ed =
                new ExportDialog(this, Qt::WindowContextHelpButtonHint);
        ed->setAttribute(Qt::WA_DeleteOnClose);
        connect(ed,
                SIGNAL(exportTable(const QString &, const QString &, bool,
                                   bool)),
                this,
                SLOT(exportASCII(const QString &, const QString &, bool,
                                 bool)));
            ...
            if (isActiveSubWindow(subwindow, SubWindowType::TableSubWindow)) {
                ...
                if (f.exists(fileName) && confirmOverwrite) {
                    ...
                    switch (QMessageBox::question(
                            ...
                            tr("&Yes"), tr("&All"), tr("&Cancel"), 0, 1)) {
                    case 0:
                        success = t->exportASCII(fileName, sep, colNames,
                                                 expSelection);
                        break;
...
void ApplicationWindow::convolute()
{
    ...
    if (static_cast<int>(s.count()) != 2) {
        QMessageBox::warning(
                this, tr("Error"),
                tr("Please select two columns for this operation:\n the first "
                   "represents the signal and the second the response "
                   "function!"));
        return;
    }
    ...
    if (int(t->selectedColumns().count()) > 0) {
        ...
        for (int i = 0; i < t->numCols(); i++)
            if (t->isColumnSelected(i, false))
                targets << i;
        newTableStatistics(t, TableStatistics::StatColumn, targets)
                ->showNormal();
    } else
...
void ApplicationWindow::exportPDF()
{
    ...
    if (isActiveSubWindow(subwindow, SubWindowType::Plot2DSubWindow)
        ...
        QMessageBox::warning(this, tr("Warning"),
                             tr("<h4>There are no plot layers available in "
                                "this window.</h4>"));
        return;
    }
        ...
        if (!f.open(QIODevice::WriteOnly)) {
            QMessageBox::critical(this, tr("Export Error"),
                                  tr("Could not write to file: "
                                     "<h4>%1</h4><p>Please verify that you "
                                     "have the right to write to this location "
                                     "or that the file is not "
...
void ApplicationWindow::print(QMdiSubWindow *subwindow)
{
    if (isActiveSubWindow(subwindow, SubWindowType::Plot2DSubWindow)
        && qobject_cast<Layout2D *>(subwindow)->getCurrentAxisRect()
                == nullptr) {
        QMessageBox::warning(this, tr("Warning"),
                             tr("<h4>There are no plot layouts available in "
                                "this window.</h4>"));
        return;
    }
    ...
    connect(previewDialog.get(), &QPrintPreviewDialog::paintRequested,
            [=](QPrinter *printer) {
                ...
                foreach (QMdiSubWindow *subwindow, subWindowsList()) {
                    if (isActiveSubWindow(subwindow,
                                          SubWindowType::Plot2DSubWindow)) {
                        if (newpage) {
                            printer->newPage();
                            newpage = false;
                        ...
                        int plotWidth =
                                layout->getPlotCanwas()->viewport().width();
                        int plotHeight =
                                layout->getPlotCanwas()->viewport().height();

                        painter->setMode(QCPPainter::pmVectorized);
                        painter->setMode(QCPPainter::pmNoCaching);
                        // comment this out if you want cosmetic thin lines
                        // (always 1 pixel thick independent of pdf zoom level)
                        // painter.setMode(QCPPainter::pmNonCosmetic);
                        layout->getCurrentAxisRect()->setPrintorExportJob(true);
                        layout->getPlotCanwas()->toPainter(
                                painter.get(), plotWidth, plotHeight);
                        layout->getCurrentAxisRect()->setPrintorExportJob(
                                false);
                        newpage = true;
                    }
                }
...
void ApplicationWindow::fitExponentialGrowth()
{
    fitExponential(-1);
}

void ApplicationWindow::fitFirstOrderExponentialDecay()
{
    fitExponential(1);
}
...
void ApplicationWindow::showFFTDialog()
{
    ...
    if (isActiveSubWindow(subwindow, SubWindowType::Plot2DSubWindow)) {
        AxisRect2D *axisrect =
                qobject_cast<Layout2D *>(subwindow)->getCurrentAxisRect();
        if (axisrect) {
            sd = std::unique_ptr<FFTDialog>(
                    new FFTDialog(FFTDialog::onGraph, this));
            sd->setAxisrect(axisrect);
        }
    } else if (isActiveSubWindow(subwindow, SubWindowType::TableSubWindow)) {
        sd = std::unique_ptr<FFTDialog>(
                new FFTDialog(FFTDialog::onTable, this));
...
void ApplicationWindow::fitBoltzmannSigmoid()
{
    analysis("fitSigmoidal");
}

void ApplicationWindow::fitGaussian()
{
    analysis("fitGauss");
}
...
void ApplicationWindow::showLayerDialog()
{
    ...
    MultiLayer *plot = qobject_cast<MultiLayer
    *>(d_workspace->activeSubWindow()); if (plot->isEmpty()) {
      QMessageBox::warning(
          this, tr("Warning"),
...
MyWidget *ApplicationWindow::clone(MyWidget *w)
{
    ...
    if (isActiveSubWindow(w, SubWindowType::Plot2DSubWindow)) {
        QMessageBox::warning(this, tr("Duplicate window error"),
                             tr("Plot2D windows cannot be duplicated! (not "
                                "implimented yet)"));
        ...
        nw = newNote();
        if (nw)
            qobject_cast<Note *>(nw)->setText(qobject_cast<Note *>(w)->text());
    }
...
void ApplicationWindow::updateWindowStatus(MyWidget *w)
{
    ...
    if (w->status() == MyWidget::Maximized) {
        ...
        foreach (MyWidget *oldMaxWindow, windows) {
            if (oldMaxWindow != w
                && oldMaxWindow->status() == MyWidget::Maximized)
...
void ApplicationWindow::dropEvent(QDropEvent *event)
{
    if (event->mimeData()->hasUrls()) {
        ...
        foreach (QUrl url, urls) {
            ...
            if (ext == "aproj" || ext == "aproj~" || ext == "aproj.gz"
                ...
                openAproj(fileName);
                       ...
                       || ext == "tsv") {
                asciiFiles << fileName;
            } else if (ext == "bmp" || ext == "bw" || ext == "eps"
                       || ext == "epsf" || ext == "epsi" || ext == "exr"
                       || ext == "kra" || ext == "ora" || ext == "pcx"
                       || ext == "psd" || ext == "ras" || ext == "rgb"
                       || ext == "rgba" || ext == "sgi" || ext == "tga"
                       || ext == "xcf" || ext == "dds" || ext == "gif"
                       || ext == "ico" || ext == "jp2" || ext == "jpeg"
                       || ext == "jpg" || ext == "mng" || ext == "pbm"
                       || ext == "pgm" || ext == "pic" || ext == "png"
                       || ext == "ppm" || ext == "svg" || ext == "svgz"
        ...
        if (!asciiFiles.isEmpty()) {
            importASCII(asciiFiles, ImportASCIIDialog::NewTables,
                        columnSeparator, ignoredLines, renameColumns,
                        strip_spaces, simplify_spaces, d_convert_to_numeric,
...
void ApplicationWindow::closeEvent(QCloseEvent *event)
{
    if (!saved) {
        switch (QMessageBox::information(
                this, tr("AlphaPlot"),
                tr("Save changes to project: <p><b> %1 </b> ?")
                        .arg(projectname),
...
void ApplicationWindow::customEvent(QEvent *event)
{
    if (event->type() == SCRIPTING_CHANGE_EVENT) {
        ...
        // it's good programming practice not to assume a particular call path
        // for an event; which means that we don't know for sure at this point
        // whether scriptEnv is connected or not.
...
void ApplicationWindow::showTable(const QString &curve)
{
    ...
    w->deselectAll();
    w->setCellsSelected(0, colIndex, w->d_future_table->rowCount() - 1,
                        colIndex);
    w->showMaximized();
    QList<QTreeWidgetItem *> items =
...
void ApplicationWindow::showWindowContextMenu()
{
    ...
    if (isActiveSubWindow(subwindow, SubWindowType::MatrixSubWindow)) {
        ...
        cm.addAction(IconLoader::load("edit-delete-selection",
...
#else
    const QString locateDefaultHelp = QDir::toNativeSeparators(
            "/usr/share/doc/AlphaPlot/manual/index.html");
#endif
...
{
    QMessageBox::information(
            this, tr("AlphaPlot help files not ready yet!"),
                                     ...
                                     const QString &curveTitle)
{
    if (whichFit == "fitLinear" || whichFit == "fitSigmoidal"
        ...
        if (fitter->setDataFromCurve(PlotColumns::getassociateddatafromstring(
                                             axisrect, curveTitle),
                                     axisrect)) {
            if (whichFit != "fitLinear")
        ...
        std::unique_ptr<Differentiation> diff(
                new Differentiation(this, axisrect,
...
Layout3D *
ApplicationWindow::plot3DMatrix(const Graph3DCommon::Plot3DType &plottype)
{
...
QList<QMdiSubWindow *>
ApplicationWindow::subWindowsListFromTreeRecursive(QList<QMdiSubWindow *> list,
...
void ApplicationWindow::fitMultiPeak(int profile)
{
    ...
    if (layout->getAxisRectList().size() == 0) {
        QMessageBox::warning(
                this, tr("Warning"),
                tr("<h4>There are no plot layouts available in this "
                   "window.</h4>"
                   "<p><h4>Please add a layout and try again!</h4>"));
        ui_->actionDisableGraphTools->setChecked(true);
        return;
    ...
    if (lsvec.size() + curvevec.size() == 0) {
        QMessageBox::warning(this, tr("Warning"),
                             tr("<h4>There are no compatible plots available "
                                "in this layout.</h4>"
                                "<p><h4>Please add Line Special or Line "
                                "Scatter plot to the layout "
                                "and try again!</h4>"));
        ui_->actionDisableGraphTools->setChecked(true);
        return;
    }
                                                 ...
                                                 double y)
{
    ...
    if (multipeakfitvalues_.size() == multiPeakfitpoints_) {
        AxisRect2D *axisrect = multipeakfitvalues_.first()
                                       .first->getxaxis()
                                       ->getaxisrect_axis();
        std::unique_ptr<MultiPeakFit> d_fit(new MultiPeakFit(
                this, axisrect, MultiPeakFit::PeakProfile::Gauss,
                multiPeakfitpoints_));
...
void ApplicationWindow::parseCommandLineArguments(const QStringList &args)
{
    ...
    foreach (str, args) {
        if (str == "-v" || str == "--version") {
            ...
            hide();
            QMessageBox::information(
                    this, tr("AlphaPlot") + " - " + tr("Version"), s);
#else
            ...
            QString s = "\n" + tr("Usage") + ": " + "AlphaPlot ["
                    + tr("options") + "] [" + tr("file") + "_" + tr("name")
                    + "]\n\n";
            s + tr("Valid options are") + ":\n";
            s += "-a " + tr("or")
                    + " --about: " + tr("about AlphaPlot application") + "\n";
            s += "-h " + tr("or")
                    + " --help: " + tr("show command line options") + "\n";
            s += "-l=XX " + tr("or")
            ...
            if (!locales.contains(locale))
                QMessageBox::critical(this, tr("Error"),
                                      tr("<b> %1 </b>: Wrong locale option or "
                                         "no translation available!")
                                              .arg(locale));
        } else if (str.startsWith("--about") || str.startsWith("-a")) {
            QString abt = tr("AlphaPlot is a free cross-platform program for "
                             "two- and "
                             "three-dimensional graphical presentation of data "
                             "sets and for "
                             "data analysis. The plots can be produced from "
                             "data sets stored "
                             "in tables or from analytical functions.")
                    + "\n"
                    + tr("AlphaPlot uses code from SciDAVis, which consisted "
                         "(at the time "
                         "of the fork, i.e. SciDAVis 1.D9) which inturn is the "
                         "fork of "
                         "QtiPlot 0.9-rc.");
#ifdef Q_OS_WIN
            hide();
    ...
    if (!file_name.isEmpty()) {
        ...
        if (fi.isDir()) {
            ...
            QMessageBox::critical(this, tr("File opening error"),
                                  tr("You don't have the permission to open "
                                     "this file: <b>%1</b>")
                                          .arg(file_name));
            return;
        } else if (!fi.exists()) {
                                            ...
                                            const QPoint &p, bool fromFolders)
{
    ...
    if (fromFolders && show_windows_policy != HideAll) {
        cm.addAction(tr("&Show All Windows"), this,
                     SLOT(showAllFolderWindows()));
        cm.addAction(tr("&Hide All Windows"), this,
                     SLOT(hideAllFolderWindows()));
        cm.addSeparator();
    }

    if (!fromFolders || it->parent()) {
        cm.addAction(IconLoader::load("edit-delete", IconLoader::General),
                     tr("&Delete Folder"), this, SLOT(deleteFolder()),
                     Qt::Key_F8);
        cm.addAction(tr("&Rename"), this, SLOT(renameFolderFromMenu()),
                     Qt::Key_F2);
        cm.addSeparator();
    }
...
{
    disconnect(ui_->folderView, SIGNAL(itemChanged(QTreeWidgetItem *, int)),
               this, SLOT(renameFolder(QTreeWidgetItem *)));
...
void ApplicationWindow::addFolder()
{
    ...
    if (fi) {
        ...
        if (parentFolder)
            ui_->folderView->setCurrentItem(
                    parentFolder->folderTreeWidgetItem());
    }
}
...
bool ApplicationWindow::changeFolder(Folder *newFolder, bool force)
{
    ...
    if (!(newFolder->children()).isEmpty()) {
        refreshFolderTreeWidgetItemsRecursive(
                newFolder->folderTreeWidgetItem());
    }
    ...
    if (active_window) {
        ...
        if (active_window_state == MyWidget::Minimized)
            active_window
                    ->showMinimized(); // d_workspace->setActiveWindow() makes
    ...
    if (!(item->folder()->children()).isEmpty()) {
        ...
        for (int i = 0; i < fi->childCount(); i++) {
            ...
            foreach (MyWidget *widget, list) {
                if (!hiddenWindows->contains(widget)
                    && !outWindows->contains(widget)) {
...
void ApplicationWindow::addListViewItem(MyWidget *widget)
{
    ...
    WindowTableWidgetItem *it =
            new WindowTableWidgetItem(ui_->listView, widget);
...
void ApplicationWindow::folderProperties()
{
    ...
    if (!current_folder->parent()) {
        ...
        if (projectname != "untitled") {
            ...
            properties.modified =
                    fileInfo.lastModified().toString(Qt::LocalDate);
            properties.label = "";
        } else {
            properties.path = projectname;
    ...
    if (isActiveSubWindow(window, SubWindowType::MatrixSubWindow)) {
        ...
        properties.content =
                QString(tr("%1 Characters,\n%2 Lines"))
                        .arg(QString::number(
                                static_cast<Note *>(window)->text().count()))
                        .arg("(unavailable)");
...
void ApplicationWindow::addFolderListViewItem(Folder *folder)
{
    ...
    FolderTreeWidgetItem *item =
            new FolderTreeWidgetItem(ui_->listView, folder);
                             ...
                             bool partialMatch, bool subfolders)
{
    if (windowNames || labels) {
        MyWidget *widget = current_folder->findWindow(
                s, windowNames, labels, caseSensitive, partialMatch);
        if (widget) {
            activateWindow(widget);
            return;
        }
        ...
        if (subfolders) {
            bool found = findRecursive(static_cast<FolderTreeWidgetItem *>(
                                               ui_->folderView->currentItem()),
                                       FindWindowItem, s, labels, caseSensitive,
                                       partialMatch);
            if (found)
                return;
        }
    ...
    if (folderNames) {
        Folder *dir =
                current_folder->findSubfolder(s, caseSensitive, partialMatch);
        if (dir) {
        ...
        if (subfolders) {
            bool found = findRecursive(static_cast<FolderTreeWidgetItem *>(
                                               ui_->folderView->currentItem()),
                                       FindFolderItem, s, false, caseSensitive,
                                       partialMatch);
            if (found)
                return;
        }
                                      ...
                                      bool caseSensitive, bool partialMatch)
{
    ...
    for (int i = 0; i < item->childCount(); i++) {
        it = static_cast<FolderTreeWidgetItem *>(item->child(i));
        bool status = findRecursive(it, findItem, s, labels, caseSensitive,
                                    partialMatch);
        if (status) {
            return true;
        }
...
void ApplicationWindow::searchForUpdates()
{
    int choice = QMessageBox::question(
            ...
            tr("AlphaPlot will now try to determine whether a new version of "
               "AlphaPlot is available. Please modify your firewall settings "
               "in "
        ...
        if (version_line.count() == 6) {
            ...
            if (available_version > AlphaPlot::version()) {
                if (QMessageBox::question(
                            ...
                            tr("There is a newer version of AlphaPlot "
                               "available for "
...
{
    return static_cast<FolderTreeWidgetItem *>(
            ui_->folderView->topLevelItem(0));
}

QString ApplicationWindow::getLogInfoText() const
{
    return logInfo;
}
    ...
    if (isActiveSubWindow(widget, SubWindowType::TableSubWindow)) {
        ...
        if (layout->getCurrentAxisRect()) {
            ...
            foreach (QString string, deplist) {
                depend_menu.addAction(string, this,
                                      SLOT(setActiveWindowFromAction()));
            }
            cm.addMenu(&depend_menu);
        }
...
void ApplicationWindow::selectPlotType(int value)
{
    ...
    if (type == Graph::Box) {
        ...
        foreach (QString colname, list)
            collist << table->column(table->colIndex(colname));
        layout->generateHistogram2DPlot(AxisRect2D::BarType::VerticalBars,
                                        false, table, collist, from, to);
        return;
    }
    ...
    if (type == Graph::Spline) {
        if (to - from < 2) {
            QMessageBox::warning(this, tr("Error"),
                                 tr("Please select three or more rows for "
                                    "Spline plotting!"));
            return;
        }
    }
    ...
    switch (type) {
    case Graph::Scatter:
        layout->generateCurve2DPlot(AxisRect2D::LineScatterType::Scatter2D,
    ...
    case Graph::LineSymbols:
        layout->generateCurve2DPlot(
                AxisRect2D::LineScatterType::LineAndScatter2D, table, xcol,
                ycollist, from, to);
        return;
    case Graph::Spline:
        layout->generateCurve2DPlot(AxisRect2D::LineScatterType::Spline2D,
                                    table, xcol, ycollist, from, to);
        return;
    case Graph::VerticalDropLines:
        layout->generateLineSpecial2DPlot(
                AxisRect2D::LineScatterSpecialType::VerticalDropLine2D, table,
                xcol, ycollist, from, to);
        return;
    ...
    case Graph::HorizontalSteps:
        layout->generateLineSpecial2DPlot(
                AxisRect2D::LineScatterSpecialType::HorizontalStep2D, table,
                xcol, ycollist, from, to);
        return;
    case Graph::Area:
        layout->generateLineSpecial2DPlot(
                AxisRect2D::LineScatterSpecialType::Area2D, table, xcol,
                ycollist, from, to);
        return;
    ...
    case Graph::VerticalBars:
        layout->generateBar2DPlot(AxisRect2D::BarType::VerticalBars, table,
                                  xcol, ycollist, from, to);
        return;
    case Graph::VerticalStackedBars:
        layout->generateStakedBar2DPlot(AxisRect2D::BarType::VerticalBars,
                                        table, xcol, ycollist, from, to);
        setAutoScale();
        return;
    case Graph::Channel:
        layout->generateLineSpecialChannel2DPlot(table, xcol, ycollist, from,
                                                 to);
        return;
    default: {
...
void ApplicationWindow::loadIcons()
{
    ...
    ui_->actionPlot2DVerticalDropLines->setIcon(QIcon(
    ...
    ui_->actionPlot2DVerticalStackedBars->setIcon(IconLoader::load(
            "graph2d-vertical-stack-bar", IconLoader::LightDark));
    ui_->actionPlot2DHorizontalBars->setIcon(
            IconLoader::load("graph2d-horizontal-bar", IconLoader::LightDark));
    ui_->actionPlot2DHorizontalStackedBars->setIcon(IconLoader::load(
            "graph2d-horizontal-stack-bar", IconLoader::LightDark));
    ...
    actionResetZoomfactor_->setIcon(IconLoader::load("graph3d-perspective-view",
                                                     IconLoader::LightDark));
    ...
    foreach (QMdiSubWindow *subwindow, subWindowsList()) {
        QList<QTreeWidgetItem *> items = ui_->listView->findItems(
                qobject_cast<MyWidget *>(subwindow)->name(), Qt::MatchExactly,
                0);
        if (isActiveSubWindow(subwindow, SubWindowType::TableSubWindow)) {
            Table *table = qobject_cast<Table *>(subwindow);
            if (items.count() > 0)
        ...
        const ApplicationWindow::SubWindowType &subwindowtype)
{
    ...
    if (subwindow) {
        ...
        case TableSubWindow:
            ...
            break;
        case MatrixSubWindow:
            (qobject_cast<Matrix *>(subwindow)) ? result = true
                                                : result = false;
            break;
        case NoteSubWindow:
        ...
        case Plot2DSubWindow:
            ...
            break;
        case Plot3DSubWindow:
            (qobject_cast<Layout3D *>(subwindow)) ? result = true
                                                  : result = false;
            break;
        }
    }
