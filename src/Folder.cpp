                              ...
                              bool partialMatch)
{
    ...
    foreach (QObject *folder, folderList) {
        ...
        if (partialMatch) {
            if (caseSensitive
                && name.startsWith(subFolderName, Qt::CaseSensitive))
        ...
        } else // partialMatch == false
        {
            if (caseSensitive && name == subFolderName)
                return static_cast<Folder *>(folder);
                     ...
                     && (name.toLower() == subFolderName.toLower()))
                return static_cast<Folder *>(folder);
        }
    }
    // will never reach
...
{
    foreach (MyWidget *window, lstWindows) {
        if (window->inherits(cls) && name == window->name())
            return window;
    }
    ...
    foreach (QObject *folder, children()) {
        MyWidget *window =
                (static_cast<Folder *>(folder))->window(name, cls, true);
        if (window)
            return window;
    }
...
void FolderTreeWidgetItem::setActive(bool status)
{
    ...
    (status) ? setIcon(0, IconLoader::load("folder-open", IconLoader::General))
             : setIcon(0,
                       IconLoader::load("folder-closed", IconLoader::General));
    setSelected(status);
}
    ...
    : QTreeWidget(parent), tableWidgetDeligate(new TableWidgetDelegate())
{
    ...
    connect(tableWidgetDeligate, SIGNAL(invalidFolderName(const QString &)),
            this, SLOT(invalidFolderNameMsgBox(const QString &)));
}

FolderTreeWidget::~FolderTreeWidget()
{
    delete tableWidgetDeligate;
}
