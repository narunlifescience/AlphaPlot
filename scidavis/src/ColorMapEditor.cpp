...
if (row == 0 || row == table->rowCount() - 1)
	{
	QMessageBox::critical(this, tr("SciDAVis - Input Error"), tr("Sorry, you cannot edit this value!"));
	table->blockSignals(true);
	if (!row)
...
if (s.isEmpty() || s.contains(QRegExp("\\D")))
	{
	QMessageBox::critical(this, tr("SciDAVis - Input Error"), tr("Please enter a valid color level value!"));
