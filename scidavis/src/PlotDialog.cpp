...
PlotDialog::PlotDialog(bool showExtended, QWidget *parent, Qt::WFlags fl)
  ...
  if (showExtended) btnMore->toggle();
...
void PlotDialog::showPlotAssociations(QTreeWidgetItem *item, int) {
  ...
  if (item->type() != CurveTreeItem::PlotCurveTreeItem) return;
...
void PlotDialog::changePlotType(int new_curve_type) {
  CurveTreeItem *item = (CurveTreeItem *)listBox->currentItem();
  if (!item || item->type() != CurveTreeItem::PlotCurveTreeItem) return;
  Graph *graph = item->graph();
  if (!graph) return;

  int old_curve_type = item->plotItemType();
  if (boxPlotType->count() == 1 || (old_curve_type == new_curve_type)) return;

  if (old_curve_type == Graph::ColorMap ||
      old_curve_type == Graph::ContourMap || old_curve_type == Graph::GrayMap)
    clearTabWidget();
  else if (old_curve_type == Graph::VectXYAM ||
           old_curve_type == Graph::VectXYXY) {
    clearTabWidget();
    insertTabs(old_curve_type);

    graph->setCurveType(item->plotItemIndex(),
                        (Graph::CurveType)new_curve_type);
...
void PlotDialog::initLayerPage() {
  ...
  connect(boxAntialiasing, SIGNAL(toggled(bool)), this,
          SLOT(updateAntialiasing(bool)));
  ...
  connect(boxBackgroundColor, SIGNAL(clicked()), this,
          SLOT(pickBackgroundColor()));
  connect(boxCanvasColor, SIGNAL(clicked()), this, SLOT(pickCanvasColor()));
  connect(boxBorderWidth, SIGNAL(valueChanged(int)), this,
          SLOT(updateBorder(int)));
}
...
void PlotDialog::initSpectrogramPage() {
  ...
  connect(levelsColorBox, SIGNAL(clicked()), this,
          SLOT(pickContourLinesColor()));
...
void PlotDialog::setMultiLayer(MultiLayer *ml) {
  ...
  for (int i = 0; i < plots.count(); ++i) {
    ...
    LayerItem *layer =
        new LayerItem(g, item, tr("Layer") + QString::number(i + 1));
    item->addChild(layer);
...
void PlotDialog::selectCurve(int index) {
  ...
  if (!layerItem) return;
  if (layerItem->type() != LayerItem::LayerTreeItem) return;
...
void PlotDialog::showStatistics() {
  ...
  if (!it) return;
  if (it->type() != CurveTreeItem::PlotCurveTreeItem) return;
  ...
  Table *t = app->newTable(
      h->dataSize(), 4, tableName,
      tr("Histogram and Probabilities for") + " " + h->title().text());
  if (t) {
    double h_sum = 0.0;
    for (int i = 0; i < h->dataSize(); i++) h_sum += h->y(i);
...
void PlotDialog::changeErrorBarsPlus() {
  ...
  graph->updateErrorBars((QwtErrorPlotCurve *)item->plotItem(),
                         xBox->isChecked(), widthBox->currentText().toInt(),
                         capBox->currentText().toInt(), colorBox->color(),
                         plusBox->isChecked(), minusBox->isChecked(),
...
void PlotDialog::pickErrorBarsColor() {
  ...
  if (!color.isValid() || color == colorBox->color()) return;
  ...
  graph->updateErrorBars(
      (QwtErrorPlotCurve *)item->plotItem(), xBox->isChecked(),
...
void PlotDialog::updateTabWindow(QTreeWidgetItem *currentItem,
  ...
  if (currentItem->type() == CurveTreeItem::PlotCurveTreeItem) {
    ...
    if (previousItem->type() != CurveTreeItem::PlotCurveTreeItem ||
        ((CurveTreeItem *)previousItem)->plotItemType() !=
            curveItem->plotItemType()) {
      clearTabWidget();
      int plot_type = setPlotType(curveItem);
      if (plot_type >= 0) insertTabs(plot_type);
      if (!curvePlotTypeBox->isVisible()) curvePlotTypeBox->show();
    }
...
void PlotDialog::insertTabs(int plot_type) {
  ...
  if (plot_type == Graph::Line) {
    ...
    if (plot_type == Graph::Histogram) {
      ...
      privateTabWidget->setCurrentIndex(
          privateTabWidget->indexOf(histogramPage));
    } else
      privateTabWidget->setCurrentIndex(privateTabWidget->indexOf(linePage));
    ...
    privateTabWidget->addTab(spectrogramPage,
                             tr("Colors") + " / " + tr("Contour"));
    privateTabWidget->setCurrentIndex(
        privateTabWidget->indexOf(spectrogramPage));
  }
}
...
int PlotDialog::setPlotType(CurveTreeItem *item) {
  ...
  if (curveType >= 0) {
    ...
    if (curveType == Graph::ErrorBars)
      ...
      if (curveType == Graph::VectXYAM) boxPlotType->setCurrentIndex(1);
...
bool PlotDialog::acceptParams() {
  if (privateTabWidget->currentWidget() == fontsPage) {
    ...
    if (!boxAll->isChecked()) return true;
  ...
  if (privateTabWidget->currentWidget() == axesPage) {
    ...
    sp->setDisplayMode(QwtPlotSpectrogram::ImageMode,
                       imageGroupBox->isChecked());
    ...
    if (boxConnect->currentIndex() ==
        0)  // no line => we really have a scatter plotter
      boxPlotType->setCurrentIndex(1);
    else {
      int index = item->plotItemIndex();
      graph->setCurveStyle(index, boxConnect->currentIndex());
      QBrush br =
          QBrush(boxAreaColor->color(), boxPattern->getSelectedPattern());
      if (!fillGroupBox->isChecked()) br = QBrush();
    ...
    if (boxSymbolStyle->selectedSymbol() ==
        QwtSymbol::NoSymbol)  // no symbol => line plot
      boxPlotType->setCurrentIndex(0);
    else {
      int size = 2 * boxSymbolSize->value() + 1;
      QBrush br = QBrush(boxFillColor->color(), Qt::SolidPattern);
      if (!boxFillSymbol->isChecked()) br = QBrush();
      QPen pen =
          QPen(boxSymbolColor->color(), boxPenWidth->value(), Qt::SolidLine);
      QwtSymbol s = QwtSymbol(boxSymbolStyle->selectedSymbol(), br, pen,
                              QSize(size, size));
      QwtPlotCurve *curve = (QwtPlotCurve *)plotItem;
      curve->setSymbol(s);
    }
...
void PlotDialog::setPiePenStyle(const Qt::PenStyle &style) {
  if (style == Qt::SolidLine) boxPieLineStyle->setCurrentIndex(0);
  if (style == Qt::DashLine) boxPieLineStyle->setCurrentIndex(1);
  if (style == Qt::DotLine) boxPieLineStyle->setCurrentIndex(2);
  if (style == Qt::DashDotLine) boxPieLineStyle->setCurrentIndex(3);
  if (style == Qt::DashDotDotLine) boxPieLineStyle->setCurrentIndex(4);
}
...
void PlotDialog::pickContourLinesColor() {
  ...
  if (!color.isValid() || color == levelsColorBox->color()) return;

  levelsColorBox->setColor(color);
}

void PlotDialog::updateTreeWidgetItem(QTreeWidgetItem *item) {
  if (item->type() != QTreeWidgetItem::Type) return;
...
void PlotDialog::pickCanvasColor() {
  ...
  if (!c.isValid() || c == boxCanvasColor->color()) return;
...
void PlotDialog::pickBackgroundColor() {
  ...
  if (!c.isValid() || c == boxBackgroundColor->color()) return;
...
void PlotDialog::pickBorderColor() {
  ...
  if (!c.isValid() || c == boxBorderColor->color()) return;
  ...
  if (boxAll->isChecked()) {
    ...
    for (int i = 0; i < allPlots.count(); i++) {
      Graph *g = (Graph *)allPlots.at(i);
      if (g) g->setFrame(boxBorderWidth->value(), c);
    }
  } else {
    LayerItem *item = (LayerItem *)listBox->currentItem();
    if (!item) return;
...
void PlotDialog::updateAntialiasing(bool on) {
  if (privateTabWidget->currentWidget() != layerPage) return;

  if (boxAll->isChecked()) {
    QWidgetList allPlots = d_ml->graphPtrs();
    for (int i = 0; i < allPlots.count(); i++) {
      Graph *g = (Graph *)allPlots.at(i);
      if (g) g->setAntialiasing(on);
    }
  } else {
    LayerItem *item = (LayerItem *)listBox->currentItem();
    if (!item) return;
...
void PlotDialog::updateBorder(int width) {
  ...
  if (boxAll->isChecked()) {
    ...
    for (int i = 0; i < allPlots.count(); i++) {
      Graph *g = (Graph *)allPlots.at(i);
      if (g) g->setFrame(width, boxBorderColor->color());
    }
  } else {
    LayerItem *item = (LayerItem *)listBox->currentItem();
    if (!item) return;
...
void PlotDialog::closeEvent(QCloseEvent *e) {
  ApplicationWindow *app = (ApplicationWindow *)this->parent();
  if (app) app->d_extended_plot_dialog = btnMore->isChecked();
...
CurveTreeItem::CurveTreeItem(const QwtPlotItem *curve, LayerItem *parent,
                             const QString &s)
