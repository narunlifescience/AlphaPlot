...
#define MANUAL_URI                                                           \
  "http://sourceforge.net/projects/scidavis/files/SciDAVis%20Documentation/" \
  "0.1/"
...
void ApplicationWindow::insertTranslatedStrings() {
  if (projectname == "untitled") setWindowTitle(tr("SciDAVis - untitled"));
...
ApplicationWindow *ApplicationWindow::open(const QString &fn) {
...
#else
  {
    QMessageBox::critical(
        this, tr("File opening error"),
        tr("SciDAVis currently does not support Origin import. If you are "
...
ApplicationWindow *ApplicationWindow::openProject(const QString &fn) {
  ...
  if (fn.endsWith(".qti", Qt::CaseInsensitive) ||
    ...
    if (d_file_version > 90) {
      ...
      delete file;
      QMessageBox::critical(this, tr("File opening error"),
                            tr("SciDAVis does not support QtiPlot project "
...
void ApplicationWindow::scriptError(const QString &message,
  ...
  Q_UNUSED(scriptName)
  Q_UNUSED(lineNumber)
  QMessageBox::critical(this, tr("SciDAVis") + " - " + tr("Script Error"),
                        message);
}
  ...
  if (!fn.isEmpty()) {
    ...
    if (fn.contains(".qmt", true) || fn.contains(".qpt", true) ||
      ...
      if ((fileType != "SciDAVis") && (fileType != "QtiPlot")) {
        QMessageBox::critical(
            this, tr("File opening error"),
            tr("The file: <b> %1 </b> was not created using SciDAVis!")
                .arg(fn));
        return;
      }
      ...
      if (fileType == "QtiPlot") {
        ...
        if (d_file_version > 90) {
          QMessageBox::critical(this, tr("File opening error"),
                                tr("SciDAVis does not support QtiPlot template "
                                   "files from versions later than 0.9.0.")
                                    .arg(fn));
          return;
        }
...
#ifdef Q_OS_MAC  // Mac
  QSettings settings(QSettings::IniFormat, QSettings::UserScope, "SciDAVis",
                     "SciDAVis");
#else
  QSettings settings(QSettings::NativeFormat, QSettings::UserScope, "SciDAVis",
                     "SciDAVis");
#endif
...
void ApplicationWindow::about() { SciDAVis::about(); }
...
void ApplicationWindow::closeEvent(QCloseEvent *ce) {
  if (!saved) {
    ...
    switch (QMessageBox::information(this, tr("SciDAVis"), s, tr("Yes"),
                                     tr("No"), tr("Cancel"), 0, 2)) {
      case 0:
        if (!saveProject()) {
          ce->ignore();
        ...
        if (fields.count() >= col + 2) {
          ...
          if (d_file_version < 90 && w->columnType(col) == SciDAVis::Numeric)
            w->setCell(row, col, QLocale::c().toDouble(cell.replace(",", ".")));
                   ...
                   w->columnType(col) == SciDAVis::Numeric)
            w->setCell(row, col, QLocale().toDouble(cell));
          else
            w->setText(row, col, cell);
        }
...
void ApplicationWindow::createActions() {
  ...
  // translations
  actionOpen =
      new QAction(QIcon(QPixmap(":/fileopen.xpm")), tr("&Open") + "...", this);
  ...
  actionOpenTemplate = new QAction(QIcon(QPixmap(":/open_template.xpm")),
                                   tr("Open Temp&late..."), this);
  connect(actionOpenTemplate, SIGNAL(activated()), this, SLOT(openTemplate()));

  actionSaveTemplate = new QAction(QIcon(QPixmap(":/save_template.xpm")),
  ...
  actionLoad =
      new QAction(QIcon(QPixmap(":/import.xpm")), tr("&Import ASCII..."), this);
...
void ApplicationWindow::parseCommandLineArguments(const QStringList &args) {
  ...
  foreach (str, args) {
    if ((str == "-a" || str == "--about") ||
      ...
      s +=
          QObject::tr("Released") + ": " + SciDAVis::releaseDateString() + "\n";
      ...
      QString s = "\n" + tr("Usage") + ": ";
      s += "scidavis [" + tr("options") + "] [" + tr("file") + "_" +
           tr("name") + "]\n\n";
      ...
      s += "-l=XX " + tr("or") + " --lang=XX: " +
           tr("start SciDAVis in language") + " XX ('en', 'fr', 'de', ...)\n";
      s += "-m " + tr("or") + " --manual: " +
           tr("show SciDAVis manual in a standalone window") + "\n";
      s += "-v " + tr("or") + " --version: " +
           tr("print SciDAVis version and release date") + "\n";
      s += "-x " + tr("or") + " --execute: " +
...
void ApplicationWindow::appendProject(const QString &fn) {
  ...
  if (fn.contains(".sciprj") || fn.contains(".qti") ||
    ...
    QMessageBox::critical(
        this, tr("File opening error"),
        tr("The file: <b>%1</b> is not a SciDAVis or Origin project file!")
            .arg(fn));
    return;
  }
...
void ApplicationWindow::searchForUpdates() {
  int choice = QMessageBox::question(
      this, versionString(),
      tr("SciDAVis will now try to determine whether a new version of SciDAVis "
         "is available. Please modify your firewall settings in order to allow "
         "SciDAVis to connect to the internet.") +
    ...
    if (list.count() > 2) {
      ...
      if (available_version > SciDAVis::version()) {
        if (QMessageBox::question(
                ...
                tr("There is a newer version of SciDAVis (%1) available for "
...
QString ApplicationWindow::versionString() { return SciDAVis::versionString(); }
