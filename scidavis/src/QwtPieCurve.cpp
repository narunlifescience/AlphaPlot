...
void QwtPieCurve::drawPie(QPainter *painter, const QwtScaleMap &xMap,
                          const QwtScaleMap &yMap, int from, int to) const {
  Q_UNUSED(xMap);
  Q_UNUSED(yMap);
  // This has to be synced with Graph::plotPie() for now... until we have a
  // clean solution.
