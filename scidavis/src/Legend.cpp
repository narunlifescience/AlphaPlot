...
void Legend::draw(QPainter *p, const QwtScaleMap &xMap, const QwtScaleMap &yMap,
  ...
  QwtArray<long> heights =
      itemsHeight(map.deviceToLayoutY(y), symbolLineLength, width, height);
...
void Legend::setText(const QString &s) { d_text->setText(s); }

void Legend::setFrameStyle(int style) {
  if (d_frame == style) return;

  d_frame = style;
}

void Legend::setBackgroundColor(const QColor &c) {
  if (d_text->backgroundBrush().color() == c) return;
...
void Legend::drawFrame(QPainter *p, int type, const QRect &rect) const {
  ...
  if (type == None) QwtPainter::fillRect(p, rect, d_text->backgroundBrush());
  ...
  if (type == Line) {
    ...
    QRect shadow_right = QRect(rect.right(), rect.y() + d_shadow_size_y,
                               d_shadow_size_x, rect.height() - 1);
    QRect shadow_bottom = QRect(rect.x() + d_shadow_size_x, rect.bottom(),
                                rect.width() - 1, d_shadow_size_y);
...
void Legend::drawSymbols(QPainter *p, const QRect &rect, QwtArray<long> height,
  ...
  for (int i = 0; i < (int)titles.count(); i++) {
    if (titles[i].contains("\\c{") || titles[i].contains("\\l(")) {
      ...
      if (g->curveType(cv) == Graph::VectXYXY ||
          g->curveType(cv) == Graph::VectXYAM)
...
QwtArray<long> Legend::itemsHeight(int y, int symbolLineLength, int &width,
                                   int &height) const {
  int maxL = 0;
    ...
    if (titles[i].contains("\\c{") && (int)cvs.size() > 0) {
      ...
      if (c && c->rtti() != QwtPlotItem::Rtti_PlotSpectrogram) {
        ...
        if (l < 3)
          ...
          l = 15;
        if (l > maxL && c->symbol().style() != QwtSymbol::NoSymbol) maxL = l;
      }
    }

    if (titles[i].contains("\\p{")) maxL = 10;
  }
  return maxL;
}
  ...
  if (s.contains("%(")) {  // curve name specification
    ...
    if (cv >= 0) {
      ...
      if (g) {
        ...
        if (c) s = s.replace(pos, pos2 - pos + 1, c->title().text());
      }
    }
  }
  return s;
