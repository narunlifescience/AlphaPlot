...
int Filter::sortedCurveData(QwtPlotCurve *c, double start, double end,
  ...
  for (i_start = 0; i_start < datasize; i_start++)
    if (c->x(p[i_start]) >= start) break;
  for (i_end = datasize - 1; i_end >= 0; i_end--)
    if (c->x(p[i_end]) <= end) break;
...
int Filter::curveData(QwtPlotCurve *c, double start, double end, double **x,
  ...
  for (i_start = 0; i_start < datasize; i_start++)
    if (c->x(i_start) >= start) break;
  for (i_end = datasize - 1; i_end >= 0; i_end--)
    if (c->x(i_end) <= end) break;
...
QwtPlotCurve *Filter::addResultCurve(double *x, double *y) {
  ...
  Column *xCol =
      new Column(tr("1", "filter table x column name"), SciDAVis::Numeric);
  Column *yCol =
      new Column(tr("2", "filter table y column name"), SciDAVis::Numeric);
  ...
  Table *t = app->newHiddenTable(
      tableName, d_explanation + " " + tr("of") + " " + d_curve->title().text(),
      QList<Column *>() << xCol << yCol);

  DataCurve *c = new DataCurve(t, tableName + "_" + xCol->name(),
                               tableName + "_" + yCol->name());
