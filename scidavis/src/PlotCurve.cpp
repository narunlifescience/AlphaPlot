...
DataCurve::DataCurve(Table *t, const QString &xColName, const QString &name,
    ...
    : PlotCurve(name),
      ...
      d_end_row(endRow) {
  if (t && d_end_row < 0) d_end_row = t->numRows() - 1;
}

void DataCurve::setRowRange(int startRow, int endRow) {
  if (d_start_row == startRow && d_end_row == endRow) return;
...
void DataCurve::updateColumnNames(const QString &oldName,
  ...
  if (updateTableName) {
    ...
    if (lst.size() > 0 && lst[0] == oldName) setTitle(newName + "_" + lst[1]);

    lst = d_x_column.split("_", QString::SkipEmptyParts);
    if (lst.size() > 0 && lst[0] == oldName)
      d_x_column = newName + "_" + lst[1];
  } else {
    if (title().text() == oldName) setTitle(newName);
    if (d_x_column == oldName) d_x_column = newName;
  }
}
...
QList<QVector<double> > DataCurve::convertData(const QList<Column *> &cols,
  ...
  // first valid row's
  // content. We can't change this easily without breaking backwards
  // compatibility (think of
      ...
      /*case Table::Time:
                          {
                                  QTime time;
                                  QString format;
                                  if (g && g->axesType()[axes[i]] ==
         Table::Time) {
                                          QStringList lst =
         g->axisFormatInfo(axes[i]).split(";");
         ...
         static_cast<DateTime2StringFilter *>(col->outputFilter())->format();
                                  ...
                                  if (g)
                                          g->setLabelsDateTimeFormat(axes[i],
         Graph::Time, time.toString() + ";" + format);
                                  break;
                          }
                  case Table::Date:
                          {
         ...
         static_cast<DateTime2StringFilter *>(col->outputFilter())->format();
                                  ...
                                  if (g)
                                          g->setLabelsDateTimeFormat(axes[i],
         Graph::Date, date.toString("YYYY-MM-DD") + ";" + format);
                                  break;
          }*/
      case Table::DateTime: {
        QDateTime datetime;
        ...
        if (format.isEmpty())
          format = static_cast<DateTime2StringFilter *>(col->outputFilter())
                       ->format();

        reference_dates.push_back(QDate());
        reference_times.push_back(QTime());
        if (g)
          g->setLabelsDateTimeFormat(
              axes[i], Graph::DateTime,
              datetime.toString("YYYY-MM-DDTHH:MM:SS") + ";" + format);
        break;
      }
      case Table::Text:
        if (g) g->setLabelsTextFormat(axes[i], col, d_start_row, end_row);
        reference_dates.push_back(QDate());
        reference_times.push_back(QTime());
        break;
      default:
  ...
  for (int i = 0; i < valid_rows.size(); i++)
    for (int j = 0; j < cols.size(); j++) switch (cols[j]->columnMode()) {
        case Table::Text:
          result[j][i] = static_cast<double>(valid_rows[i] + 1);
          break;
        /*case Table::Time:
                                result[j][i] =
           reference_times[j].msecsTo(cols[j]->timeAt(valid_rows[i]));
                                break;
                        case Table::Date:
                                result[j][i] =
           reference_dates[j].daysTo(cols[j]->dateAt(valid_rows[i]));
            break;*/
        case Table::DateTime: {
          QDateTime dt = cols[j]->dateTimeAt(valid_rows[i]);
          result[j][i] =
              double(dt.date().toJulianDay()) +
...
bool DataCurve::loadData() {
  ...
  QList<QVector<double> > points =
      convertData(d_type == Graph::HorizontalBars
                      ? (QList<Column *>() << y_col_ptr << x_col_ptr)
...
void DataCurve::removeErrorBars(DataCurve *c) {
  if (!c || d_error_bars.isEmpty()) return;

  int index = d_error_bars.indexOf(c);
  if (index >= 0 && index < d_error_bars.size()) d_error_bars.removeAt(index);
}

void DataCurve::clearErrorBars() {
  if (d_error_bars.isEmpty()) return;
...
int DataCurve::tableRow(int point) { return d_index_to_row.value(point, -1); }

QwtDoubleRect PlotCurve::boundingRect() const {
  QwtDoubleRect r = QwtPlotCurve::boundingRect();
  if (symbol().style() == QwtSymbol::NoSymbol) return r;
  ...
  return QwtDoubleRect(d_x_left, d_y_top, qAbs(d_x_right - d_x_left),
                       qAbs(d_y_bottom - d_y_top));
}
