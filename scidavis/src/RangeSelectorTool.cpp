...
RangeSelectorTool::RangeSelectorTool(Graph *graph, const QObject *status_target,
  ...
  */
  d_active_marker.setSymbol(
      QwtSymbol(QwtSymbol::Ellipse, QBrush(QColor(255, 255, 0, 128)),
  ...
  if (status_target)
    connect(this, SIGNAL(statusText(const QString &)), status_target,
            status_slot);
  emit statusText(
      tr("Click or use Ctrl+arrow key to select range (arrows select active "
         "cursor)!"));
}
...
void RangeSelectorTool::pointSelected(const QPoint &pos) {
  ...
  if (curve == d_selected_curve)
    ...
    if (curve->x(point) == min_x)
      ...
      second_x = min_x;
    if (second_x ==
        max_x) {  // start at selected point and try larger indices first
      for (int i = 0; i < n; ++i)
...
void RangeSelectorTool::setSelectedCurve(QwtPlotCurve *curve) {
  if (!curve || d_selected_curve == curve) return;
  d_selected_curve = curve;
  d_active_point = 0;
  d_inactive_point = d_selected_curve->dataSize() - 1;
  d_active_marker.setValue(d_selected_curve->x(d_active_point),
...
void RangeSelectorTool::setActivePoint(int point) {
  if (point == d_active_point) return;
  d_active_point = point;
  d_active_marker.setValue(d_selected_curve->x(d_active_point),
                           d_selected_curve->y(d_active_point));
  emitStatusText();
  ...
  if (((PlotCurve *)d_selected_curve)->type() == Graph::Function) {
    emit statusText(
        QString("%1 <=> %2[%3]: x=%4; y=%5")
            ...
            .arg(QLocale().toString(d_selected_curve->x(d_active_point), 'G',
                                    15))
            .arg(QLocale().toString(d_selected_curve->y(d_active_point), 'G',
                                    15)));
  } else {
    ...
    emit statusText(
        QString("%1 <=> %2[%3]: x=%4; y=%5")
            ...
            .arg(d_selected_curve->title().text())
            .arg(row + 1)
            .arg(t->text(row, t->colIndex(((DataCurve *)d_selected_curve)
                                              ->xColumnName())))
