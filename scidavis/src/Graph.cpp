    ...
    for (int i = 0; i < lst.size(); i++) {
      ...
      if (i >= otherCurves.size()) {
        ...
        for (int k = otherCurves.size() - 1; k >= 0; k--) {
          ...
          if (w->colPlotDesignation(index) == SciDAVis::Y) ycol = index;
        }
...
void Graph::createTable(const QwtPlotCurve *curve) {
  ...
  Column *xCol =
      new Column(tr("1", "curve data table x column name"), SciDAVis::Numeric);
  Column *yCol =
      new Column(tr("2", "curve data table y column name"), SciDAVis::Numeric);
