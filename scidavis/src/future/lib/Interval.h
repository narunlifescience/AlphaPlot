...
class IntervalBase {
 public:
  ...
  /**
   * This function returns an invalid interval if the two intervals do not
   * intersect.
   */
  static Interval<T> intersection(const Interval<T>& first,
                                  const Interval<T>& second) {
    return Interval<T>(qMax(first.start(), second.start()),
                       qMin(first.end(), second.end()));
  }
  ...
  static Interval<T> merge(const Interval<T>& a, const Interval<T>& b) {
    if (!(a.intersects(b) || a.touches(b))) return a;
    return Interval<T>(qMin(a.start(), b.start()), qMax(a.end(), b.end()));
  }
  //! Subtract an interval from another
  static QList<Interval<T> > subtract(const Interval<T>& src_iv,
                                      const Interval<T>& minus_iv) {
    QList<Interval<T> > list;
    if ((src_iv == minus_iv) || (minus_iv.contains(src_iv))) return list;
  ...
  static QList<Interval<T> > split(const Interval<T>& i, T before) {
    ...
    if (before < i.start() || before > i.end()) {
      ...
      if (left.isValid()) list.append(left);
      if (right.isValid()) list.append(right);
    }
    return list;
  }
  ...
  //! Restrict all intervals in the list to their intersection with a given
  //! interval
  /**
   ...
   */
  static void subtractIntervalFromList(QList<Interval<T> >* list,
                                       Interval<T> i) {
    ...
    for (int c = 0; c < list->size(); c++) {
      ...
      if (temp_list.isEmpty())
        ...
        if (temp_list.size() > 1) list->insert(c, temp_list.at(1));
      }
    }
  }
 ...
 *	self explanatory.
 *
 *	For the template argument (T), only numerical types ((unsigned) short,
 *(unsigned) int,
...
class Interval<float> : public IntervalBase<float> {
  ...
  float size() const {
    return IntervalBase<float>::d_end - IntervalBase<float>::d_start;
  }
  bool isValid() const {
    return (IntervalBase<float>::d_start <= IntervalBase<float>::d_end);
  }
...
class Interval<double> : public IntervalBase<double> {
  ...
  double size() const {
    return IntervalBase<double>::d_end - IntervalBase<double>::d_start;
  }
  bool isValid() const {
    return (IntervalBase<double>::d_start <= IntervalBase<double>::d_end);
  }
...
class Interval<long double> : public IntervalBase<long double> {
  ...
  Interval(long double start, long double end)
      : IntervalBase<long double>(start, end) {}
  Interval(const Interval<long double>& other)
      : IntervalBase<long double>(other) {}
  long double size() const {
    return IntervalBase<long double>::d_end -
           IntervalBase<long double>::d_start;
  }
  bool isValid() const {
    return (IntervalBase<long double>::d_start <=
