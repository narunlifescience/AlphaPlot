...
class AbstractAspect : public QObject {
  ...
  */
   ...
   * The only requirement is that the root Aspect reimplements undoStack() to
   * get the
   * undo stack from somewhere (the default implementation just delegates to
   * parentAspect()).
   */
 ...
 public slots:
   ...
   * - \%t - creationTime()
   *
   * Additionally, you can use the construct %C{<text>}. <text> is only shown in
   * the caption
 ...
 protected:
  ...
  //! given to the user.
  /**
   * This will cause statusInfo() to be emitted. Typically, this will cause the
   * specified string
   * to be displayed in a status bar, a log window or some similar non-blocking
   * way so as not to
