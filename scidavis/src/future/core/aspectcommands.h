...
class AspectNameChangeCmd : public QUndoCommand {
 public:
  AspectNameChangeCmd(AbstractAspect::Private *target, const QString &new_name)
    ...
    setText(
        QObject::tr("%1: rename to %2").arg(d_target->name()).arg(new_name));
  }
...
class AspectChildRemoveCmd : public QUndoCommand {
 public:
  ...
  ~AspectChildRemoveCmd() {
    if (d_removed && !d_detach) delete d_child;
  }
...
class AspectChildAddCmd : public AspectChildRemoveCmd {
 public:
  AspectChildAddCmd(AbstractAspect::Private *target, AbstractAspect *child,
    ...
    setText(
        QObject::tr("%1: add %2").arg(d_target->name()).arg(d_child->name()));
    d_index = index;
  }
...
class AspectChildMoveCmd : public QUndoCommand {
 public:
  ...
  virtual void redo() {
    ...
    // to be mapped to QAbstractItemModel::layoutAboutToBeChanged() and
    // ::layoutChanged().
