...
void SelectionMoveResizer::add(QWidget *target) {
  if (target->parentWidget() != parent()) return;
  d_widgets << target;
  ...
  switch (op) {
    ...
    case Resize_E:
      return QRect(QPoint(rect.right() - handler_size + 1,
                          rect.center().y() - handler_size / 2 + 1),
                   QSize(handler_size, handler_size));
    case Resize_SE:
      return QRect(QPoint(rect.right() - handler_size + 1,
    ...
    case Resize_W:
      return QRect(
          QPoint(rect.left(), rect.center().y() - handler_size / 2 + 1),
          QSize(handler_size, handler_size));
    case Resize_NW:
...
QRect SelectionMoveResizer::operateOn(const QRect in) {
  ...
  if (in == d_bounding_rect) return boundary_out;
...
void SelectionMoveResizer::paintEvent(QPaintEvent *e) {
  ...
  p.setPen(white_pen);
  for (int i = 0; i < 8; i++) p.drawRect(handlerRect(drawn_rect, (Operation)i));
...
bool SelectionMoveResizer::eventFilter(QObject *o, QEvent *e) {
  switch (e->type()) {
    case QEvent::Resize:
      ...
      return false;
    case QEvent::Move:
      if ((QWidget *)o != parentWidget()) recalcBoundingRect();
      return false;
    default:
      return false;
  }
