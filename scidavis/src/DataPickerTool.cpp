...
DataPickerTool::DataPickerTool(Graph *graph, ApplicationWindow *app, Mode mode,
  ...
  d_selection_marker.setSymbol(QwtSymbol(QwtSymbol::Ellipse,
  ...
  switch (d_mode) {
    case Display:
      emit statusText(
          tr("Click on plot or move cursor to display coordinates!"));
      break;
    case Move:
...
void DataPickerTool::append(const QPoint &pos) {
  ...
  setSelection((QwtPlotCurve *)d_graph->plotWidget()->curve(curve),
               point_index);
...
void DataPickerTool::setSelection(QwtPlotCurve *curve, int point_index) {
  if (curve == d_selected_curve && point_index == d_selected_point) return;
...
bool DataPickerTool::eventFilter(QObject *obj, QEvent *event) {
  switch (event->type()) {
    ...
    case QEvent::KeyPress:
      if (keyEventFilter((QKeyEvent *)event)) return true;
      break;
    default:
      break;
  }
...
bool DataPickerTool::keyEventFilter(QKeyEvent *ke) {
  ...
  switch (ke->key()) {
    case Qt::Key_Enter:
    case Qt::Key_Return:
      if (d_selected_curve) emit selected(d_selected_curve, d_selected_point);
      return true;
    ...
    case Qt::Key_Minus:
      if (d_graph) {
        if (d_selected_curve) {
          int n_points = d_selected_curve->dataSize();
          setSelection(d_selected_curve,
                       (d_selected_point - 1 + n_points) % n_points);
          d_graph->plotWidget()->replot();
        } else
...
void DataPickerTool::removePoint() {
  ...
  if (((PlotCurve *)d_selected_curve)->type() == Graph::Function) {
    QMessageBox::critical(
        const_cast<Graph *>(d_graph), tr("Remove point error"),
  ...
  if (t->columnType(col) == Table::Numeric) {
    t->column(col)->setValueAt(
        ((DataCurve *)d_selected_curve)->tableRow(d_selected_point), 0.0);
    t->column(col)->setInvalid(
        ((DataCurve *)d_selected_curve)->tableRow(d_selected_point), true);
  } else {
    QMessageBox::warning(
        const_cast<Graph *>(d_graph), tr("Warning"),
...
void DataPickerTool::move(const QPoint &point) {
  if (d_mode == Move && d_selected_curve) {
    ...
    double new_x_val = d_graph->plotWidget()->invTransform(
        d_selected_curve->xAxis(), d_move_target_pos.x());
    double new_y_val = d_graph->plotWidget()->invTransform(
        d_selected_curve->yAxis(), d_move_target_pos.y());
