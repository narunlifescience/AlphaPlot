...
VectorCurve::VectorCurve(VectorStyle style, Table *t, const QString &xColName,
                         QString name, const QString &endCol1,
...
void VectorCurve::drawArrowHead(QPainter *p, int xs, int ys, int xe,
  ...
  if (filledArrow) p->setBrush(QBrush(pen.color(), Qt::SolidPattern));
...
void VectorCurve::setVectorEnd(const QString &xColName,
  ...
  if (d_end_x_a == xColName && d_end_y_m == yColName) return;
...
int VectorCurve::width() { return pen.width(); }

void VectorCurve::setWidth(int w) { pen.setWidth(w); }

QColor VectorCurve::color() { return pen.color(); }

void VectorCurve::setColor(const QColor &c) {
  if (pen.color() != c) pen.setColor(c);
}

void VectorCurve::setHeadLength(int l) {
  if (d_headLength != l) d_headLength = l;
}

void VectorCurve::setHeadAngle(int a) {
  if (d_headAngle != a) d_headAngle = a;
}

void VectorCurve::fillArrowHead(bool fill) {
  if (filledArrow != fill) filledArrow = fill;
}
...
QwtDoubleRect VectorCurve::boundingRect() const {
  ...
  if (d_type == Graph::VectXYXY)
    ...
    for (int i = 0; i < rows; i++) {
      ...
      switch (d_position) {
        case Tail:
          rect |= QwtDoubleRect(x_i, y_i, mag * cos(angle), mag * sin(angle))
                      .normalized();
          break;
        case Middle: {
          QwtDoubleRect rect_i(0, 0, fabs(mag * cos(angle)),
                               fabs(mag * sin(angle)));
...
void VectorCurve::updateColumnNames(const QString &oldName,
  ...
  if (updateTableName) {
    ...
    if (lst[0] == oldName) setTitle(newName + "_" + lst[1]);

    lst = d_x_column.split("_", QString::SkipEmptyParts);
    if (lst[0] == oldName) d_x_column = newName + "_" + lst[1];

    lst = d_end_x_a.split("_", QString::SkipEmptyParts);
    if (lst[0] == oldName) d_end_x_a = newName + "_" + lst[1];

    lst = d_end_y_m.split("_", QString::SkipEmptyParts);
    if (lst[0] == oldName) d_end_y_m = newName + "_" + lst[1];
  } else {
    if (title().text() == oldName) setTitle(newName);
    if (d_x_column == oldName) d_x_column = newName;
    if (d_end_x_a == oldName) d_end_x_a = newName;
    if (d_end_y_m == oldName) d_end_y_m = newName;
  }
}
...
VectorCurve::~VectorCurve() { delete vectorEnd; }
