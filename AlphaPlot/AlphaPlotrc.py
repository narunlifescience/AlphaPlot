...
# File                 : AlphaPlotrc.py                                    #
# Project              : AlphaPlot                                         #
# Description          : default configuration file of AlphaPlot' Python   #
#                        environment                                       #
...
def import_to_global(modname, attrs=None, math=False):
	"""
		...
		If math==True, also registers functions with AlphaPlot' math function list.
	"""
	...
	for name in attrs:
		...
		# make functions available in AlphaPlot' math function list
		if math and callable(f): __main__.AlphaPlot.mathFunctions[name] = f
...
# make Qt API available (it gets imported in any case by the AlphaPlot module)
global QtGui
...
# import AlphaPlot' classes to the global namespace (particularly useful for fits)
for name in dir(__main__.AlphaPlot):
	setattr(__main__, name, getattr(__main__.AlphaPlot, name))
...
for name in appImports:
	setattr(__main__,name,getattr(__main__.AlphaPlot.app,name))
...
__main__.AlphaPlot.Column.__getitem__ = __column_getitem
...
__main__.AlphaPlot.Column.__setitem__ = __column_setitem

# import utility module
import sys
sys.path.append(".")
try:
	import_to_global("AlphaPlotUtil")
	print "AlphaPlotUtil successfully imported"
except(ImportError): 
	print "failed to import AlphaPlotUtil"
