...
class ScriptEdit : public QTextEdit, public scripted {
 ...
 private:
  ...
  //! True iff we are inside evaluate(), execute() or executeAll() there were
  //! errors.
  bool d_error;
  //! True iff the text is programmatically changed and handleContentsChange()
  //! should do nothing.
  bool d_changing_fmt;
 ...
 private slots:
  ...
  /**
   * After insertion, the text cursor will have the error message selected,
