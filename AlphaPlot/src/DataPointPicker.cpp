 ...
 *
 * While DataPickerTool does the actual selection work, DataPointPicker provides
 * the framework
 * necessary for script-driven usage. In particular, the event-oriented
 * interface of DataPickerTool
 * doesn't fit very well into the typical imparative structure of simple
 * scripts. DataPointPicker
 * bridges this gap by starting a new event loop (see QEventLoop) which allows
 * the user to pick a
 * data point while the script is still running within the main event loop. The
 * inner event loop
 * exits when the user has selected a data point, pick() returns its coordinates
 * and control flows
 * back to the caller.
 */
...
DataPointPicker::~DataPointPicker() { m_picking_loop.exit(1); }

/**
 * \brief Let the user pick a single data point and return its coordinates.
 *
 * Starts a new event loop, which runs until either the user has selected a
 * point or the
 * DataPointPicker is destructed.
 */
QPointF DataPointPicker::pick() {
  if (m_graph->isPiePlot() || !m_graph->validCurvesDataSize()) return m_result;
  DataPickerTool *tool =
      new DataPickerTool(m_graph, m_app, DataPickerTool::Display);
