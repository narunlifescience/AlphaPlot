 ...
 * some of them relying on AlphaPlot-specific QwtPlotCurve subclasses for parts
...
class Graph : public QWidget {
 ...
 public:
  ...
  void exportPainter(QPaintDevice& paintDevice, bool keepAspect = false,
                     QRect rect = QRect());
  void exportPainter(QPainter& painter, bool keepAspect = false,
                     QRect rect = QRect(), QSize size = QSize());
 public slots:
  ...
  void plotPie(Table* w, const QString& name, int startRow = 0,
               int endRow = -1);
  //! Used when restoring a pie plot from a project file
  void plotPie(Table* w, const QString& name, const QPen& pen, int brush,
               int size, int firstColor, int startRow = 0, int endRow = -1,
               bool visible = true);
  void removePie();
  ...
  bool insertCurvesList(Table* w, const QStringList& names, int style,
                        int lWidth, int sSize, int startRow = 0,
                        int endRow = -1);
  bool insertCurve(Table* w, const QString& name, int style, int startRow = 0,
                   int endRow = -1);
  bool insertCurve(Table* w, int xcol, const QString& name, int style);
  bool insertCurve(Table* w, const QString& xColName, const QString& yColName,
  ...
  /**
   * The option to disable updating is provided so as not to break the project
   * opening code
  ...
  //! \name Error Bars
  //@{
  bool addErrorBars(const QString& xColName, const QString& yColName,
                    Table* errTable, const QString& errColName, int type = 1,
                    int width = 1, int cap = 8,
                    const QColor& color = QColor(Qt::black),
                    bool through = true, bool minus = true, bool plus = true);

  bool addErrorBars(const QString& yColName, Table* errTable,
                    const QString& errColName, int type = 1, int width = 1,
                    int cap = 8, const QColor& color = QColor(Qt::black),
                    bool through = true, bool minus = true, bool plus = true);

  void updateErrorBars(QwtErrorPlotCurve* er, bool xErr, int width, int cap,
  ...
  void setLabelsTextFormat(int axis, const Column* column, int startRow,
                           int endRow);
  ...
  bool addFunctionCurve(ApplicationWindow* parent, int type,
                        const QStringList& formulas, const QString& var,
                        QList<double>& ranges, int points,
  ...
  void updateVectorsLayout(int curve, const QColor& color, int width,
  ...
  bool plotHistogram(Table* w, QStringList names, int startRow = 0,
                     int endRow = -1);
