 ...
 *
 * This class is only meant to simplify implementation of a restricted subtype
 * of filter.
 * It should not be instantiated directly. You should always either derive from
 * AbstractFilter or (if necessary) provide an actual (non-abstract)
 * implementation.
 *
 * The trick here is that, in a sense, the filter is its own output port. This
 * means you
 * can implement a complete filter in only one class and don't have to
 * coordinate data
 * transfer between a filter class and a data source class.
 * Additionaly, AbstractSimpleFilter offers some useful convenience methods
 * which make writing
 * filters as painless as possible.
 *
 * For the data type of the output, all types supported by AbstractColumn
 * (currently double, QString and
 * QDateTime) are supported.
 *
 * \section tutorial1 Tutorial, Step 1
 * The simplest filter you can write assumes there's also only one input port
 * and rows on the
 * input correspond 1:1 to rows in the output. All you need to specify is what
 * data type you
 * want to have (in this example double) on the input port and how to compute
 * the output values:
 *
 * \code
 ...
 * 06			return (source->dataType() == AlphaPlot::TypeDouble);
 * 07		}
 * 08	public:
 * 09		virtual AlphaPlot::ColumnDataType dataType() const { return
 ...
 * Reimplementing inputAcceptable() makes sure that the data source really is of
 * type
 * double (lines 5 to 7). Otherwise, the source will be rejected by
 ...
 *
 * Equivalently, you can write 1:1-filters for QString or QDateTime inputs by
 * checking for
 * AlphaPlot::TypeQString or AlphaPlot::TypeQDateTime in line 6. You would then
 * use
 * AbstractColumn::textAt(row) or AbstractColumn::dateTimeAt(row) in line 13 to
 ...
 * Now for something slightly more interesting: a filter that uses only every
 * second row of its
 * input. We no longer have a 1:1 correspondence between input and output rows,
 * so we'll have
