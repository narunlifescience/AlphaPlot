/***************************************************************************
    ...
    Project              : AlphaPlot
    Description          : Commands to be called by Column to modify
 Column::Private
    --------------------------------------------------------------------
...
ColumnSetModeCmd::~ColumnSetModeCmd() {
  ...
  if (d_conversion_filter) delete d_conversion_filter;
}
...
void ColumnSetModeCmd::redo() {
  if (!d_executed) {
    ...
    d_col->replaceModeData(d_mode, d_new_type, d_new_data, d_new_in_filter,
                           d_new_out_filter, d_new_validity);
  }
  d_undone = false;
}
...
void ColumnSetModeCmd::undo() {
  // reset to old values
  d_col->replaceModeData(d_old_mode, d_old_type, d_old_data, d_old_in_filter,
                         d_old_out_filter, d_old_validity);
...
void ColumnPartialCopyCmd::redo() {
  if (d_src_backup == 0) {
    // copy the relevant rows of source and destination column into backup
    // columns
...
///////////////////////////////////////////////////////////////////////////
ColumnInsertEmptyRowsCmd::ColumnInsertEmptyRowsCmd(Column::Private* col,
...
void ColumnInsertEmptyRowsCmd::redo() { d_col->insertRows(d_before, d_count); }

void ColumnInsertEmptyRowsCmd::undo() { d_col->removeRows(d_before, d_count); }
...
///////////////////////////////////////////////////////////////////////////
ColumnRemoveRowsCmd::ColumnRemoveRowsCmd(Column::Private* col, int first,
...
ColumnSetPlotDesignationCmd::ColumnSetPlotDesignationCmd(
    Column::Private* col, AlphaPlot::PlotDesignation pd, QUndoCommand* parent)
...
void ColumnClearMasksCmd::undo() { d_col->replaceMasking(d_masking); }
...
ColumnSetInvalidCmd::ColumnSetInvalidCmd(Column::Private* col,
                                         Interval<int> interval, bool invalid,
                                         QUndoCommand* parent)
    : QUndoCommand(parent),
      d_col(col),
      d_interval(interval),
...
ColumnSetMaskedCmd::ColumnSetMaskedCmd(Column::Private* col,
                                       Interval<int> interval, bool masked,
                                       QUndoCommand* parent)
...
void ColumnSetMaskedCmd::undo() { d_col->replaceMasking(d_masking); }
...
void ColumnSetFormulaCmd::undo() { d_col->replaceFormulas(d_formulas); }
...
///////////////////////////////////////////////////////////////////////////
ColumnClearFormulasCmd::ColumnClearFormulasCmd(Column::Private* col,
                                               QUndoCommand* parent)
    : QUndoCommand(parent), d_col(col) {
...
void ColumnClearFormulasCmd::undo() { d_col->replaceFormulas(d_formulas); }
...
///////////////////////////////////////////////////////////////////////////
ColumnSetTextCmd::ColumnSetTextCmd(Column::Private* col, int row,
...
///////////////////////////////////////////////////////////////////////////
ColumnSetValueCmd::ColumnSetValueCmd(Column::Private* col, int row,
                                     double new_value, QUndoCommand* parent)
    : QUndoCommand(parent), d_col(col), d_row(row), d_new_value(new_value) {
...
///////////////////////////////////////////////////////////////////////////
ColumnSetDateTimeCmd::ColumnSetDateTimeCmd(Column::Private* col, int row,
...
ColumnReplaceDateTimesCmd::ColumnReplaceDateTimesCmd(
    Column::Private* col, int first, const QList<QDateTime>& new_values,
    QUndoCommand* parent)
    : QUndoCommand(parent),
      d_col(col),
      d_first(first),
