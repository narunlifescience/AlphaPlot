...
ImportOPJ::ImportOPJ(ApplicationWindow *app, const QString &filename)
  ...
  importGraphs(opj);
  importNotes(opj);
  if (filename.endsWith(".opj", Qt::CaseInsensitive)) createProjectTree(opj);
...
bool ImportOPJ::createProjectTree(const OriginFile &opj) {
  ...
  tree<Origin::ProjectNode>::iterator root =
      projectTree->begin(projectTree->begin());
  ...
  for (tree<Origin::ProjectNode>::iterator sib = projectTree->begin(root);
    ...
    if (sib->type == Origin::ProjectNode::Folder) {
      Folder *f =
          new Folder(parent.value(projectTree->parent(sib)), sib->name.c_str());
      parent[sib] = f;
      ...
      FolderListItem *fi =
          new FolderListItem(mw->current_folder->folderListItem(), f);
      if (fi) f->setFolderListItem(fi);
    } else {
      QString name = sib->name.c_str();
      if (sib->type == Origin::ProjectNode::Note) {
        QRegExp rx("^@\\((\\S+)\\)$");
        if (rx.indexIn(name) == 0) name = rx.cap(1);
      }
      QString nodetype;
      switch (sib->type) {
      ...
      if (w) {
        ...
        if (f) {
          // removeWindow  uses QList.removeAll, so remove w before adding it to
          // its folder
...
int ImportOPJ::translateOrigin2AlphaPlotLineStyle(int linestyle) {
  int AlphaPlotstyle = 0;
  switch (linestyle) {
  ...
  return AlphaPlotstyle;
}

bool ImportOPJ::importTables(const OriginFile &opj) {
  int visible_count = 0;
  int AlphaPlot_scaling_factor = 10;  // in Origin width is measured in
                                     // characters while in AlphaPlot - pixels
                                     // --- need to be accurate
  for (unsigned int s = 0; s < opj.spreadCount(); ++s) {
    ...
    if (spread.hidden || spread.loose) mw->hideWindow(table);

    table->setWindowLabel(spread.label.c_str());
    for (int j = 0; j < columnCount; ++j) {
      Origin::SpreadColumn column = spread.columns[j];
      Column *AlphaPlot_column = table->column(j);

      QString name(column.name.c_str());
      AlphaPlot_column->setName(name.replace(QRegExp(".*_"), ""));
      if (column.command.size() > 0)
        AlphaPlot_column->setFormula(Interval<int>(0, maxrows),
                                    QString(column.command.c_str()));
      AlphaPlot_column->setComment(QString(column.comment.c_str()));
      table->setColumnWidth(j, (int)column.width * AlphaPlot_scaling_factor);

      switch (column.type) {
        case Origin::SpreadColumn::X:
          table->setColPlotDesignation(j, AlphaPlot::X);
          break;
        case Origin::SpreadColumn::Y:
          table->setColPlotDesignation(j, AlphaPlot::Y);
          break;
        case Origin::SpreadColumn::Z:
          table->setColPlotDesignation(j, AlphaPlot::Z);
          break;
        case Origin::SpreadColumn::XErr:
          table->setColPlotDesignation(j, AlphaPlot::xErr);
          break;
        case Origin::SpreadColumn::YErr:
          table->setColPlotDesignation(j, AlphaPlot::yErr);
          break;
        case Origin::SpreadColumn::Label:
        default:
          table->setColPlotDesignation(j, AlphaPlot::noDesignation);
      }
      ...
      switch (column.valueType) {
        ...
        case Origin::TextNumeric: {
          ...
          for (int i = 0; i < std::min((int)column.data.size(), maxrows); ++i) {
            ...
            if (value.type() == typeid(double)) {
              ...
              AlphaPlot_column->setValueAt(i, datavalue);
            }
          }
          int f;
          if (column.numericDisplayType == 0) {
            ...
            Double2StringFilter *filter = static_cast<Double2StringFilter *>(
                table->column(j)->outputFilter());
        ...
        case Origin::Text:
          for (int i = 0; i < min((int)column.data.size(), maxrows); ++i)
            AlphaPlot_column->setTextAt(
                i, boost::get<string>(column.data[i]).c_str());
          table->column(j)->setColumnMode(AlphaPlot::Text);
          break;
        case Origin::Date: {
          switch (column.valueTypeSpecification) {
            case -128:
          ...
          for (int i = 0; i < min((int)column.data.size(), maxrows); ++i)
            AlphaPlot_column->setValueAt(i, boost::get<double>(column.data[i]));
          table->column(j)->setColumnMode(AlphaPlot::DateTime);
          DateTime2StringFilter *filter = static_cast<DateTime2StringFilter *>(
              AlphaPlot_column->outputFilter());
        ...
        case Origin::Month: {
          ...
          for (int i = 0; i < min((int)column.data.size(), maxrows); ++i)
            AlphaPlot_column->setValueAt(i, boost::get<double>(column.data[i]));
          table->column(j)->setColumnMode(AlphaPlot::Month);
          DateTime2StringFilter *filter = static_cast<DateTime2StringFilter *>(
              table->column(j)->outputFilter());
          filter->setFormat(format);
          break;
        ...
        case Origin::Day: {
          ...
          for (int i = 0; i < min((int)column.data.size(), maxrows); ++i)
            AlphaPlot_column->setValueAt(i, boost::get<double>(column.data[i]));
          table->column(j)->setColumnMode(AlphaPlot::Day);
          DateTime2StringFilter *filter = static_cast<DateTime2StringFilter *>(
              table->column(j)->outputFilter());
          filter->setFormat(format);
          break;
    ...
    if (!(spread.hidden || spread.loose) || opj.version() != 7.5) {
      ...
      if (opj.version() >= 6.0) {
        ...
        int dy =
            table->parentWidget()->frameGeometry().height() - table->height();
        table->parentWidget()->move(QPoint(
            visible_count * dx + xoffset * OBJECTXOFFSET, visible_count * dy));
        visible_count++;
      }
    }
  }
  ...
  for (unsigned int s = 0; s < opj.matrixCount(); ++s) {
    ...
    Matrix->setColumnsWidth(matrix.width * AlphaPlot_scaling_factor);
// TODO
#if 0
...
// cascade the matrices
    ...
    int dx = 100;
    int dy = 100;
    Matrix->parentWidget()->move(QPoint(
        visible_count * dx + xoffset * OBJECTXOFFSET, visible_count * dy));
    visible_count++;
  }

  if (visible_count > 0) xoffset++;
  return true;
}
  ...
  for (unsigned int g = 0; g < opj.graphCount(); ++g) {
    ...
    for (unsigned int l = 0; l < layers; ++l) {
      ...
      if (!layer.legend.text.empty()) {
        graph->newLegend(
            parseOriginText(QString::fromLocal8Bit(layer.legend.text.c_str())));
      }
      // add texts
      for (unsigned int i = 0; i < layer.texts.size(); ++i) {
        graph->newLegend(parseOriginText(
            QString::fromLocal8Bit(layer.texts[i].text.c_str())));
      }
      int auto_color = 0;
      int auto_color1 = 0;
      int style = 0;
      for (unsigned int c = 0; c < layer.curves.size(); ++c) {
        ...
        switch (data[0].toAscii()) {
          ...
          case 'F': {
            ...
            QList<double> ranges;
            int s = opj.functionIndex(
                data.right(data.length() - 2).toStdString().c_str());
            ...
            graph->addFunctionCurve(mw, type, formulas, "x", ranges,
        ...
        cl.filledArea =
            (_curve.fillArea || style == Graph::VerticalBars ||
        ...
        if (style == Graph::VerticalBars || style == Graph::HorizontalBars) {
          ...
          if (b) b->setGap(qRound(100 - _curve.symbolSize * 10));
        } else if (style == Graph::Histogram) {
      ...
      if (style == Graph::HorizontalBars) {
        graph->setScale(0, layer.xAxis.min, layer.xAxis.max, layer.xAxis.step,
                        layer.xAxis.majorTicks, layer.xAxis.minorTicks,
                        layer.xAxis.scale);
        graph->setScale(2, layer.yAxis.min, layer.yAxis.max, layer.yAxis.step,
                        layer.yAxis.majorTicks, layer.yAxis.minorTicks,
                        layer.yAxis.scale);
      } else {
      ...
      grid->setMajPenX(QPen(
          ColorBox::color(layer.xAxis.majorGrid.color),
          ceil(layer.xAxis.majorGrid.width),
          Graph::getPenStyle(translateOrigin2AlphaPlotLineStyle(
              (Origin::GraphCurve::LineStyle)layer.xAxis.majorGrid.style))));
      grid->setMinPenX(QPen(
          ColorBox::color(layer.xAxis.minorGrid.color),
          ceil(layer.xAxis.minorGrid.width),
          Graph::getPenStyle(translateOrigin2AlphaPlotLineStyle(
              (Origin::GraphCurve::LineStyle)layer.xAxis.minorGrid.style))));
      grid->setMajPenY(QPen(
          ColorBox::color(layer.yAxis.majorGrid.color),
          ceil(layer.yAxis.majorGrid.width),
          Graph::getPenStyle(translateOrigin2AlphaPlotLineStyle(
              (Origin::GraphCurve::LineStyle)layer.yAxis.majorGrid.style))));
      grid->setMinPenY(QPen(
          ColorBox::color(layer.yAxis.minorGrid.color),
      ...
      graph->setXAxisTitle(parseOriginText(
          QString::fromLocal8Bit(formats[0].label.text.c_str())));
      graph->setYAxisTitle(parseOriginText(
          QString::fromLocal8Bit(formats[2].label.text.c_str())));
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        QString tableName =
            data.right(data.length() - 2) + "_" + ticks[i].columnName.c_str();
        ...
        graph->showAxis(
            i, type, tableName, mw->table(tableName), !(formats[i].hidden),
      ...
      graph->setAutoscaleFonts(
          mw->autoScaleFonts);  // restore user defined fonts behaviour
...
QString ImportOPJ::parseOriginTags(const QString &str) {
  ...
  QString rtag[] = {"</b>",   "</i>",   "</u>",   "</font>",
                    "</sup>", "</sub>", "</font>"};
  QRegExp rxtags[7];
  for (int i = 0; i < 7; ++i) rxtags[i].setPattern(rxstr[i] + "[^\\(\\)]*\\)");
  ...
  for (int i = 0; i < 6; ++i) line.replace(QRegExp(rxstr[i]), ltag[i]);
