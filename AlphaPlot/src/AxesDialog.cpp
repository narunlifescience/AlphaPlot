...
AxesDialog::AxesDialog(QWidget *parent, Qt::WFlags fl) : QDialog(parent, fl) {
...
void AxesDialog::initScalesPage() {
  ...
  connect(boxScaleType, SIGNAL(activated(int)), this,
          SLOT(updateMinorTicksList(int)));
  ...
  connect(btnMajor, SIGNAL(toggled(bool)), boxMajorValue,
...
void AxesDialog::initAxesPage() {
  ...
  axesTitlesList->setMaximumWidth(axesTitlesList->iconSize().width() + width +
                                  50);
  // resize the list to the maximum width
  ...
  connect(buttonLabelFont, SIGNAL(clicked()), this,
          SLOT(customAxisLabelFont()));

  connect(axesTitlesList, SIGNAL(currentRowChanged(int)), this,
          SLOT(updateShowBox(int)));
  connect(axesTitlesList, SIGNAL(currentRowChanged(int)), this,
          SLOT(updateAxisColor(int)));
  connect(axesTitlesList, SIGNAL(currentRowChanged(int)), this,
          SLOT(updateTitleBox(int)));
  connect(axesTitlesList, SIGNAL(currentRowChanged(int)), this,
          SLOT(setTicksType(int)));
  connect(axesTitlesList, SIGNAL(currentRowChanged(int)), this,
          SLOT(setAxisType(int)));
  connect(axesTitlesList, SIGNAL(currentRowChanged(int)), this,
          SLOT(setBaselineDist(int)));
  connect(axesTitlesList, SIGNAL(currentRowChanged(int)), this,
          SLOT(updateLabelsFormat(int)));

  connect(boxShowLabels, SIGNAL(clicked(bool)), this,
          SLOT(updateTickLabelsList(bool)));
  ...
  connect(boxMajorTicksType, SIGNAL(activated(int)), this,
          SLOT(updateMajTicksType(int)));
  connect(boxMinorTicksType, SIGNAL(activated(int)), this,
          SLOT(updateMinTicksType(int)));

  connect(boxShowAxis, SIGNAL(clicked()), this, SLOT(showAxis()));
  connect(boxFormat, SIGNAL(activated(int)), this,
          SLOT(setLabelsNumericFormat(int)));

  connect(btnAxesFont, SIGNAL(clicked()), this, SLOT(customAxisFont()));
  connect(boxBaseline, SIGNAL(valueChanged(int)), this,
          SLOT(changeBaselineDist(int)));
  connect(boxAxisType, SIGNAL(activated(int)), this,
          SLOT(showAxisFormatOptions(int)));
...
void AxesDialog::drawFrame(bool framed) {
  ...
  d_graph->drawCanvasFrame(framed, boxFrameWidth->value(),
                           boxFrameColor->color());
}
...
void AxesDialog::showAxis() {
  ...
  bool labels = false;
  if (tickLabelsOn[axis] == "1") labels = true;
  ...
  showAxis(axis, currentSelectedAxisType(), boxColName->currentText(), ok,
           ...
           boxShowLabels->isChecked(), boxAxisColor->color(),
...
void AxesDialog::customAxisFont() {
  ...
  switch (axesTitlesList->currentRow()) {
    case 0: {
      ...
      if (okF) xBottomFont = fnt;
      break;
    }
    case 1: {
      axis = QwtPlot::yLeft;
      fnt = QFontDialog::getFont(&okF, yLeftFont, this);
      if (okF) yLeftFont = fnt;
      break;
    }
    case 2: {
      axis = QwtPlot::xTop;
...
void AxesDialog::updateGrid() {
  if (generalDialog->currentWidget() != gridPage) return;
  ...
  if (axesGridList->currentRow() == 1) {
    ...
    grid->setMajPenX(QPen(ColorBox::color(boxColorMajor->currentIndex()),
                          boxWidthMajor->value(),
                          Graph::getPenStyle(boxTypeMajor->currentIndex())));
    grid->setMinPenX(QPen(ColorBox::color(boxColorMinor->currentIndex()),
                          boxWidthMinor->value(),
    ...
    grid->setMajPenY(QPen(ColorBox::color(boxColorMajor->currentIndex()),
                          boxWidthMajor->value(),
                          Graph::getPenStyle(boxTypeMajor->currentIndex())));
    grid->setMinPenY(QPen(ColorBox::color(boxColorMinor->currentIndex()),
                          boxWidthMinor->value(),
                          Graph::getPenStyle(boxTypeMinor->currentIndex())));
  }
...
void AxesDialog::showGridOptions(int axis) {
  ...
  disconnect(boxMajorGrid, SIGNAL(toggled(bool)), this,
             SLOT(majorGridEnabled(bool)));
  disconnect(boxMinorGrid, SIGNAL(toggled(bool)), this,
             SLOT(minorGridEnabled(bool)));
...
void AxesDialog::changeBaselineDist(int baseline) {
  ...
  showAxis(axis, currentSelectedAxisType(), formatInfo[axis],
           ...
           boxMinorTicksType->currentIndex(), boxShowLabels->isChecked(),
           boxAxisColor->color(), boxFormat->currentIndex(),
...
bool AxesDialog::updatePlot() {
  if (generalDialog->currentWidget() == (QWidget *)scalesPage) {
    ...
    d_graph->setScale(a, start, end, stp, boxMajorValue->value(),
                      boxMinorValue->currentText().toInt(),
    ...
    if (format == Graph::Numeric) {
      ...
      if (lst.size() < 2 || lst[0].isEmpty()) {
        ...
        if (format == Graph::Time)
          ...
          lst << QDateTime(QDate(1, 1, 1), QTime(0, 0, 0))
    ...
    d_graph->changeTicksLength(boxMinorTicksLength->value(),
                               boxMajorTicksLength->value());
...
void AxesDialog::pickAxisColor() {
  ...
  if (!c.isValid() || c == boxAxisColor->color()) return;
  ...
  showAxis(axis, currentSelectedAxisType(), formatInfo[axis],
           boxShowAxis->isChecked(), boxMajorTicksType->currentIndex(),
           boxMinorTicksType->currentIndex(), boxShowLabels->isChecked(), c,
           boxFormat->currentIndex(), boxPrecision->value(), boxAngle->value(),
           boxBaseline->value(), formula, boxAxisNumColor->color());
}
...
void AxesDialog::pickAxisNumColor() {
  ...
  if (!c.isValid() || c == boxAxisNumColor->color()) return;
  ...
  showAxis(axis, currentSelectedAxisType(), formatInfo[axis],
           ...
           boxMinorTicksType->currentIndex(), boxShowLabels->isChecked(),
           boxAxisColor->color(), boxFormat->currentIndex(),
           boxPrecision->value(), boxAngle->value(), boxBaseline->value(),
           formula, c);
}
...
void AxesDialog::updateMajTicksType(int) {
  ...
  showAxis(axis, currentSelectedAxisType(), formatInfo[axis],
           boxShowAxis->isChecked(), type, boxMinorTicksType->currentIndex(),
           boxShowLabels->isChecked(), boxAxisColor->color(),
           boxFormat->currentIndex(), boxPrecision->value(), boxAngle->value(),
           boxBaseline->value(), formula, boxAxisNumColor->color());
}
...
void AxesDialog::updateMinTicksType(int) {
  ...
  showAxis(axis, currentSelectedAxisType(), formatInfo[axis],
           boxShowAxis->isChecked(), boxMajorTicksType->currentIndex(), type,
           boxShowLabels->isChecked(), boxAxisColor->color(),
           boxFormat->currentIndex(), boxPrecision->value(), boxAngle->value(),
           boxBaseline->value(), formula, boxAxisNumColor->color());
}
...
void AxesDialog::updateTickLabelsList(bool on) {
  ...
  if (tickLabelsOn[axis] == QString::number(on)) return;
...
void AxesDialog::setLabelsNumericFormat(int) {
  ...
  showAxis(axis, type, formatInfo[axis], boxShowAxis->isChecked(),
           boxMajorTicksType->currentIndex(), boxMinorTicksType->currentIndex(),
           boxShowLabels->isChecked(), boxAxisColor->color(), format, prec,
...
void AxesDialog::updateLabelsFormat(int) {
  if (currentSelectedAxisType() != Graph::Numeric) return;
...
void AxesDialog::customAxisLabelFont() {
  ...
  if (okF && fnt != oldFont) d_graph->setAxisTitleFont(axis, fnt);
}
...
void AxesDialog::updateMinorTicksList(int scaleType) {
  ...
  int a = Graph::mapToQwtAxis(axesList->currentRow());
  boxMinorValue->setEditText(
      QString::number(d_graph->plotWidget()->axisMaxMinor(a)));
}

void AxesDialog::showAxis(int axis, int type, const QString &labelsColName,
                          bool axisOn, int majTicksType, int minTicksType,
                          bool labelsOn, const QColor &c, int format, int prec,
                          int rotation, int baselineDist,
                          const QString &formula, const QColor &labelsColor) {
  ApplicationWindow *app = qobject_cast<ApplicationWindow *>(parent());
  if (!app) return;

  Table *w = app->table(labelsColName);
  if ((type == Graph::Txt || type == Graph::ColHeader) && !w) return;

  if (!d_graph) return;
  d_graph->showAxis(axis, type, labelsColName, w, axisOn, majTicksType,
                    minTicksType, labelsOn, c, format, prec, rotation,
                    baselineDist, formula, labelsColor);
}
