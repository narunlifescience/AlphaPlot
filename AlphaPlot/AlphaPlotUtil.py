...
"""
	Some classes and functions that are particularly useful when doing calculations in AlphaPlot;
	maybe also outside of it.
"""

import math, AlphaPlot
...
class value:
	"""
		...
		construct a value from a AlphaPlot table, like this:
		value(table, valcol, errcol, row)
		where table is a AlphaPlot.Table instance, valcol the column containing the actual value, errcol the column
	...
	def __init__(self,val,var=None,sigma=True,row=None):
		if val.__class__==AlphaPlot.Table:
...
def lookup(tab, col, txt):
	"""
	  lookup(tab, col, txt):
	  search column col of AlphaPlot table tab for a cell with text content txt,
...
def exportTableToTeX(t, filename=None):
	...
	if not filename:
		filename=QFileDialog.getSaveFileName(AlphaPlot.app,"AlphaPlot - Export TeX table","","All files *;;TeX documents (*.tex *.TEX);;");
	f=open(filename,'w')
