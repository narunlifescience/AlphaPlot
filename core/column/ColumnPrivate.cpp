...
ColumnPrivate::ColumnPrivate(Column * owner, SciDAVis::ColumnMode mode)
 : AbstractColumn(owner ? owner->name() : ""), d_owner(owner)
{
	d_column_mode = mode;
	switch(mode)
	{		
...
void ColumnPrivate::setColumnMode(SciDAVis::ColumnMode mode)
{
	...
	if(d_owner) emit d_owner->modeAboutToChange(d_owner);
	...
	if(d_owner) emit d_owner->modeChanged(d_owner);
}
...
{
	if(d_owner) emit d_owner->dataAboutToChange(d_owner);
	d_data = data;
	d_validity = validity;
	if(d_owner) emit d_owner->dataChanged(d_owner);
}
...
void ColumnPrivate::insertRows(int before, int count)
{
	...
	if(d_owner) emit d_owner->rowsAboutToBeInserted(d_owner, before, count);
	...
	if(d_owner) emit d_owner->rowsInserted(d_owner, before, count);
}
...
void ColumnPrivate::removeRows(int first, int count)
{
	...
	if(d_owner) emit d_owner->rowsAboutToBeRemoved(d_owner, first, count);
	...
	if(d_owner) emit d_owner->rowsRemoved(d_owner, first, count);
}

void ColumnPrivate::setPlotDesignation(SciDAVis::PlotDesignation pd)
{
	if(d_owner) emit d_owner->plotDesignationAboutToChange(d_owner);
	d_plot_designation = pd; 
	if(d_owner) emit d_owner->plotDesignationChanged(d_owner);
}
...
{
	if(d_owner) emit d_owner->maskingAboutToChange(d_owner);	
	d_masking.clear();
	if(d_owner) emit d_owner->maskingChanged(d_owner);	
}
...
{
	if(d_owner) emit d_owner->dataChanged(d_owner); // all cells must be repainted
	if(d_owner) emit d_owner->descriptionChanged(d_owner); // the icon for the type changed
}
