...
if (g->isPiePlot())
	...
	for (i=0;i<g->curves();i++)
		...
		if (style == VerticalBars || style == HorizontalBars)
        	...
        	qCopy(y, y + n, temp.begin());
			QwtSingleArrayData dat(x[0], temp, n);
			c->setData(dat);
			}
		else
...
for (int j = 0; j <(int)names.count(); j++)
	...
	if (size>0)
		...
		QVector<double> temp(size);
       	qCopy(y, y + size, temp.begin());
		QwtSingleArrayData data(double(j+1), temp, size);
...
if (size>0)
	...
	if (c)
		...
		QVector<double> temp(size);
        qCopy(y, y + size, temp.begin());
		QwtSingleArrayData dat(c->x(0), temp, size);
		c->setData(dat);
		}
	delete[] y;
	}
...
QVector<double> temp(size);
qCopy(y, y + size, temp.begin());
QwtSingleArrayData dat(l[1].toDouble(), temp, size);
c->setData(dat);
delete[] y;
