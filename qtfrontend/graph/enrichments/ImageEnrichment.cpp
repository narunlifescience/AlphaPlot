...
#include "ImageEnrichment.h"
#include <QPainter>
#include <QImageReader>

ImageEnrichment::ImageEnrichment(const QString& fn):
	d_pos(QPoint(0,0)),
	d_x_right(0),
    d_y_bottom(0)
{
...
void ImageEnrichment::draw (QPainter *p, const QwtScaleMap &xMap, const QwtScaleMap &yMap, const QRect &) const
{
...
void ImageEnrichment::setSize(const QSize& size)
{
    d_size = size;
    updateBoundingRect();
}

void ImageEnrichment::setOrigin(const QPoint& p)
{
    d_pos = p;
...
void ImageEnrichment::setRect(int x, int y, int w, int h)
{
...
void ImageEnrichment::setBoundingRect(double left, double top, double right, double bottom)
{
...
void ImageEnrichment::updateBoundingRect()
{
    if (!plot())
        return;
...
QwtDoubleRect ImageEnrichment::boundingRect() const
{
    return QwtDoubleRect(xValue(), yValue(), qAbs(d_x_right - xValue()), qAbs(d_y_bottom - yValue()));
}

QRect ImageEnrichment::rect() const
{
