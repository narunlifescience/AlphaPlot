/***************************************************************************
    File                 : LineEnrichment.cpp
    Project              : SciDAVis
    --------------------------------------------------------------------
    Copyright            : (C) 2006 by Ion Vasilief
    Email (use @ for *)  : ion_vasilief*yahoo.fr
    Description          : Draws lines and arrows.
...
void LineEnrichment::draw(QPainter *p, const QwtScaleMap &xMap, const QwtScaleMap &yMap, const QRect &) const
{
...
double LineEnrichment::theta(int xs, int ys, int xe, int ye) const
{
double t;
if (xe == xs)
	{
...
double LineEnrichment::dist(int x, int y)
{
	if (!plot())
		return -1.0;
...
void LineEnrichment::setColor(const QColor& c)
{
if (linePen().color() == c)
	return;
...
void LineEnrichment::setWidth(int w)
{
if (linePen().width() == w)
	return;
...
void LineEnrichment::setStyle(Qt::PenStyle style)
{
...
void LineEnrichment::setHeadLength(int l)
{
if (d_head_length == l)
	return;
...
void LineEnrichment::setHeadAngle(int a)
{
if (d_head_angle == a)
	return;
...
void LineEnrichment::fillArrowHead(bool fill)
{
if (d_fill_head == fill)
	return;
...
void LineEnrichment::setStartPoint(const QPoint& p)
{
if (d_start == p)
	return;
...
void LineEnrichment::setEndPoint(const QPoint& p)
{
if (d_end == p)
	return;
...
QPoint LineEnrichment::startPoint() const
{
    if (!plot())
		return QPoint();
...
QwtDoublePoint LineEnrichment::startPointCoord()
{
	return QwtDoublePoint(d_rect.left(), d_rect.top());
}

void LineEnrichment::setStartPoint(double x, double y)
{
...
QPoint LineEnrichment::endPoint() const
{
    if (!plot())
		return QPoint();
...
void LineEnrichment::setEndPoint(double x, double y)
{
...
QwtDoublePoint LineEnrichment::endPointCoord()
{
return QwtDoublePoint(d_rect.right(), d_rect.bottom());
}

void LineEnrichment::setBoundingRect(double xs, double ys, double xe, double ye)
{
...
QwtDoubleRect LineEnrichment::boundingRect() const
{
	const QwtScaleMap &xMap = plot()->canvasMap(xAxis());
	const QwtScaleMap &yMap = plot()->canvasMap(yAxis());
...
void LineEnrichment::updateBoundingRect()
{
const QwtScaleMap &xMap = plot()->canvasMap(xAxis());
const QwtScaleMap &yMap = plot()->canvasMap(yAxis());
...
void LineEnrichment::setEditable(bool yes)
{
	if (yes == d_editable)
		return;
	if (yes) {
...
bool LineEnrichment::eventFilter(QObject *, QEvent *e)
{
	switch(e->type()) {
		...
		case QEvent::MouseButtonDblClick:
			{
				...
				if (me->button() != Qt::LeftButton)
					return false;
				LineEnrichmentDialog *ld = new LineEnrichmentDialog(this, plot()->window(), Qt::Tool);
